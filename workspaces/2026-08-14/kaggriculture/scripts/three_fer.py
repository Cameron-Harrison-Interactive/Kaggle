#!/usr/bin/env python3
"""three_fer.py — leftover walker + tetsu/seat1 counter + distinctive path.
...
# --------------------------------------------------------------------------
def compile_three(mod, walker, path, seed=1):
    tapes = {}
    reports = {}
    leftovers = {}
    visits_by_seat = {}
    ...
    for seat in (0, 1):
        ...
        if path.get("style") and path["style"] not in ("off", "greedy", "p0"):
            tape = apply_alt_path(
                tape, visits,
                style=path["style"],
...
def staged_catalog(quick=False):
    ...
    quick: w1 + px + c_tetsu_tomato + the 3-fer of those three.
    """
    if quick:
        return [
            {"name": "BASE", "walker": WALKERS[0], "path": PATHS[0], "counter": COUNTERS[0]},
            {"name": "w1", "walker": WALKERS[1], "path": PATHS[0], "counter": COUNTERS[0]},
            {"name": "px", "walker": WALKERS[0], "path": PATHS[1], "counter": COUNTERS[0]},
            {"name": "c_tetsu_tomato", "walker": WALKERS[0], "path": PATHS[0], "counter": COUNTERS[1]},
            {"name": "w1+px+c_tetsu_tomato", "walker": WALKERS[1], "path": PATHS[1], "counter": COUNTERS[1]},
        ]
    out = []
    # singles
    out.append({"name": "BASE", "walker": WALKERS[0], "path": PATHS[0], "counter": COUNTERS[0]})
    for w in WALKERS[1:]:
        out.append({"name": w["name"], "walker": w, "path": PATHS[0], "counter": COUNTERS[0]})
    for p in PATHS[1:]:
        out.append({"name": p["name"], "walker": WALKERS[0], "path": p, "counter": COUNTERS[0]})
    for c in COUNTERS[1:]:
        out.append({"name": c["name"], "walker": WALKERS[0], "path": PATHS[0], "counter": c})
    ...
    for p in (PATHS[1], PATHS[3]):  # xfirst + zig
        for c in (COUNTERS[1], COUNTERS[4]):  # both-seat tomato, seat1 both
            out.append({"name": f"w1+{p['name']}+{c['name']}",
                        "walker": WALKERS[1], "path": p, "counter": c})
    # de-dupe
    seen, uniq = set(), []
    for v in out:
...
def _init(mod_path, tetsu_path, our_refs):
    _W["mod"] = rc.load_v18(mod_path)
    _W["v18"] = _load_mod(mod_path, "v18_live").agent
...
def _eval_variant(task):
    ...
    try:
        ...
        agents = {
            s: make_three_agent(tapes[s], mod, v["counter"]["name"], _W["refs"])
            for s in (0, 1)
        }
        # keep-gate vs v18
        v18 = _W["v18"]
        ...
        for gs in gate_seeds:
            for seat in (0, 1):
                a = agents[seat]
                if seat == 0:
                    x, y = rc.battle(a, v18, gs, 0)
                else:
                    y, x = rc.battle(v18, a, gs, 1)
        ...
        rec = {
            ...
            "pass_ok": pass_ok,
            "v18": {"avg": kg["avg"], "wins": kg["wins"], "games": kg["games"],
                    "losses": kg["losses"], "by": kg["by"]},
        ...
        flag = "SHIP" if ship else ("keep" if pass_ok and kg["losses"] == 0 else "no")
        print(
            ...
            f"v18 {kg['avg']:+.0f} W {kg['wins']}/{kg['games']} | {tet_s} | "
            f"{rec['time_s']}s",
            flush=True,
        )
...
def main():
    ...
    ap.add_argument("--gate-seeds", default="1,2,3", help="v18 keep-gate seeds")
    ap.add_argument("--procs", type=int, default=0)
    ap.add_argument("--diagnose-only", action="store_true")
    ap.add_argument("--quick", action="store_true",
                    help="5 candidates only (BASE, w1, px, c_tetsu_tomato, 3-fer)")
    ...
    print("[three-fer] this is NOT a cartesian. leftover walker + tetsu "
          "farm-keyed counter + equivalent-length path rewrite.", flush=True)
    print("[three-fer] ship gate: seat0 PASS>=$167,978  animals>=13  "
          "0 losses vs v18. Keep v18 live unless SHIP=YES.", flush=True)
    ...
    if not ship_yes:
        print("[three-fer] nothing cleared the ship gate. HI_AgriBot_v18 stays live.",
              flush=True)
