...
def plan_market(S, day):
    ...
    # ---- 4) WAR HERD 6/6/8 (user directive): fert engine + egg anchor +
    # milk/wool denial. NO wheat floor — feed comes from the desk (buy <=21
    # off enemy dumps), so the strb treadmill keeps every tile it had. ----
    goose_t = min(8, day) if 2 <= day <= 8 else (8 if day > 8 else 0)
    cow_t = min(6, max(1, day - 1)) if 2 <= day <= 7 else 6
    sheep_t = min(6, max(0, day - 3)) if 3 <= day <= 9 else 6
