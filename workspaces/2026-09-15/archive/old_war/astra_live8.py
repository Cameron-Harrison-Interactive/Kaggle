...
def agent(obs, configuration=None):
    ...
    if day >= 2 and not final_day:
        ...
        # TRACE-04 measured cap: min(12, ...) throttled the seed stock to 12
        # standing seeds while 55-78 tiles sat empty d14-29 with idle workers
        # (3 of the 12 slots were dead MELON seeds crop_score had abandoned).
        # Crew-scaled cap: 2 seeds per planter (8 hands -> 18) lets PLANT jobs
        # (lowest priority, slack-only) fill idle decisions. room still gates
        # tending capacity and crop_score>0 still gates every purchase.
        need = max(0, min(2 * (desired_hands + 1), slots, room) - standing_seeds)
    ...
    try:
        with open("/tmp/astra_econ8.txt", "a") as _lf:
            _lf.write(
