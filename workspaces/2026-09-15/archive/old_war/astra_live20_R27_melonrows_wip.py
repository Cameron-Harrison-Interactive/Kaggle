...
def agent(obs, configuration=None):
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        if pos is not None and quadrant(pos) == "NW" and animal_line(pos):
            if (day <= 6 and vseeds.get("MELON", 0) > 0
                    and sum(1 for p, _t in plants
                            if p != pos and quadrant(p) == "NW"
                            and animal_line(p)
                            and _t.get("kind") == "PLANT"
                            and _t.get("crop") == "MELON") < 6):
                return "MELON"
            return None  # post-window rows wait for animals
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        for q in quads:
            if q == "NW":
                ...
                # south team: pure crops, 3 workers, line tiles included
                roles += [("Q", q), ("Q", q), ("Q", q)]
        qi = 0
    ...
    if day <= 1:
        _m_gap_n, _m_res = 6, 150   # the NW row wave (opening wallet)
