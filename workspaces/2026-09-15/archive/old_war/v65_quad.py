...
def plan_market(S, day):
    ...
    # USER QUAD CREW MATH: 3 workers per quad (covers crops + 5 animals in
    # that quad's line), +1 per 5 animals beyond the base 5/quad. Fibonacci
    # peak hires when counted work + walk overflows the crew's hours.
    nq_ = len(S["quadrants"])
    base_crew = 3 * nq_ + max(0, -(-(max(0, n_an - 5 * nq_)) // 5))
    try:
        _tk = build_tasks(S, day)
    except Exception:
        _tk = []
    work_est = len(_tk) + 10            # + builds/plants/weeds
    peak_crew = -(-int(work_est * 2.8) // 20)   # serpentine sweeps are walk-heavy
    want = max(5, min(16, max(base_crew, peak_crew)))
    if day <= 2:
        want = 8                        # user's measured opener (open_hires=8)
    cost = sum(FIB[i] for i in range(want))
    floor = 150 if day < 20 else 600
    while want > 8 and money < cost + floor:
        want -= 1
        cost = sum(FIB[i] for i in range(want))
    if money >= cost + (100 if day < 20 else 400):
        for _ in range(want):
            emit(["HIRE"])
        money -= cost
    else:
    ...
    # main target = wheat: flood wheat-heavy enemies any day, everyone from d16
    wheat_heavy_enemy = (en_w >= 8 and en_w >= 1.2 * max(1, my_w)) or day >= 16
    ...
    # war wheat filler d18-26: fill every empty tile — but ONLY when the
    # enemy is wheat-heavy (the flood is a weapon; vs balanced books the
    # extra volume just crashes the price under our own sales).
    if 18 <= day <= 26:
        en_w_ = S.get("enemy_wheat", 0)
        my_w_ = sum(1 for pl in S["plants"] if pl["crop"] == "WHEAT")
        if en_w_ >= 8 and en_w_ >= 1.2 * max(1, my_w_):
            targets["WHEAT"] = max(targets.get("WHEAT", 0), 40)
    ...
    # ---- 4) WAR HERD 6/6/8 = 5 per quad (fert engine + egg anchor) ----
    # USER OPENING: 3 geese first (fert $100 base, most crash-stable product
    # in the game + $50 eggs daily) = expansion capital. Cows once cash flows
    # (d5+), sheep last (d8+). Growth stage by stage, never day-0 animals.
    goose_t = 3 if day >= 1 else 0
    if day >= 5:
        goose_t = min(8, 3 + (day - 5) // 2)
    cow_t = min(6, day - 4) if day >= 5 else 0
    sheep_t = min(6, day - 7) if day >= 8 else 0
    tgt = {"GOOSE": goose_t, "COW": cow_t, "SHEEP": sheep_t}
    have = {}
    for a in animals:
    ...
    elif nq == 3 and day >= 10 and money + sell_rev >= 6400 and sum(targets.values()) > 45:
...
def build_tasks(S, day):
    tasks = []
    # ENGINE-EXACT SCHEDULE (from source): product accrues on interval days
    # regardless; the care bonus only lands on FED production days; 2 unfed
    # days = escape. So: FEED on production days + survival; CARE only on
    # production days; FERT daily.
    for a in S["animals"]:
        an = ANIMALS[a["kind"]]
        dsf = (day + 1) - a["placed"] - an["first_yield_day"]
        prod = dsf >= 0 and dsf % an["interval"] == 0
        survival = a["unfed"] >= 1
        if not a["fed"] and (prod or survival):
            tasks.append(("FEED", a["pos"], "WHEAT"))
        if prod and not a["cared"]:
...
def plan_day(S, day):
    ...
    # ---- QUAD-BANDED SERPENTINE (the football play, user design): every
    # unit owns a quadrant (3/quad, extras to the heaviest); within the quad
    # the rows split into contiguous bands; each hand sweeps its rows in
    # serpentine order servicing crops AND the animal line in one pass. No
    # dedicated servers, no dead space, no cross-quad walks. ----
    routes = {i: [] for i in range(n_units)}

    def _qof(p):
        return ("N" if p[1] < 5 else "S") + ("W" if p[0] < 5 else "E")

    owned_q = sorted(S["quadrants"]) or ["NW"]
    qpool = {q: {} for q in owned_q}      # quad -> row -> [(serp, task)]
    for t in tasks:
        kind, pos, arg = t
        q = _qof(pos)
        if q not in qpool:
        ...
        r = pos[1] - y0
        sx = (pos[0] - x0) if r % 2 == 0 else (4 - (pos[0] - x0))
        qpool[q].setdefault(r, []).append((sx, (kind, pos, arg)))
    # ROUTES THAT FIT IN A DAY: per quad, order tasks (survival first,
    # serpentine within priority), cut into chunks of <= 14 tasks (14 work +
    # ~14 walk fits a 21h day with margin). Quad's own units take its chunks
    # first; leftovers to underloaded units. An unfinished route's tail is
    # how crops and animals die — nobody owns work they cannot finish.
    # WALK-AWARE CHUNKS: an hour is ONE step or ONE action. Route cost =
    # manhattan steps between stops + one action per stop (+2 shed margin).
    # Build greedily nearest-neighbor from the quad's shed corner, survival
    # tasks (FEED/WATER) seeded first so they lead every chunk.
    prio = {"FEED": 0, "WATER": 1, "CARE": 2, "COLLECT": 3, "HARVESTA": 4,
            "HARVESTC": 5, "FERTILIZE": 6, "DIGP": 7}
    chunks = []
    for q in owned_q:
        allq = []
        for r in sorted(qpool.get(q, {}).keys()):
            allq.extend(qpool[q].get(r, []))
        surv = sorted([e for e in allq if prio.get(e[1][0], 8) <= 1],
                      key=lambda e: e[0])
        rest = [e for e in allq if prio.get(e[1][0], 8) > 1]
        y0 = 0 if q[0] == "N" else 5
        x0 = 0 if q[1] == "W" else 5
        corner = (x0 + 2, y0 + 2)
        budget = 18
        ...
        cost = 0
        pos = corner
        pool = list(surv) + list(rest)
        while pool:
            # prefer survival tasks in serp order until exhausted, else nearest
            if surv:
                e = surv.pop(0)
                if e not in pool:
                    continue
                ...
                e = min(pool, key=lambda e2: _d(pos, e2[1][1]) + prio.get(e2[1][0], 8) * 0.01)
                pool.remove(e)
            kind, tpos, arg = e[1]
            step = _d(pos, tpos)
            if cur and cost + step + 1 > budget:
                chunks.append([q, cur])
                cur = []
                cost = 0
                pos = corner
                step = _d(pos, tpos)
            cur.append((kind, tpos, arg))
            cost += step + 1
            pos = tpos
        if cur:
            chunks.append([q, cur])
    load = {q: sum(len(v) for v in qpool.get(q, {}).values()) for q in owned_q}
    assign = {}
    counts = {q: 0 for q in owned_q}
    for i in range(n_units):
        # proportional to load: heavy quads take the peak hands (user: hire
        # Fibonacci when there is more work), empty quads get units last
        q = min(owned_q,
                key=lambda qq: (counts[qq] / (1 + load[qq]), owned_q.index(qq)))
        assign[i] = q
        counts[q] += 1
    sim["unit_quad"] = dict(assign)
    owner = {}
    for q in owned_q:
        qchunks = [k for k, c in enumerate(chunks) if c[0] == q]
        mates = [i for i in range(n_units) if assign[i] == q]
        for j, k in enumerate(qchunks):
            if j < len(mates):
                owner[mates[j]] = k
    left = [k for k in range(len(chunks)) if k not in owner.values()]
    free = [i for i in range(n_units) if i not in owner]
    for i, k in zip(free, left):
        owner[i] = k
    # orphaned chunks (more chunks than units): append to the least-loaded
    # owner — a long day for one unit beats a dead crop at a chunk's tail
    still = [k for k in left if k not in owner.values()]
    for k in still:
        i_min = min(range(n_units), key=lambda i: len(routes[i]))
        routes[i_min].extend(chunks[k][1])
    for i in range(n_units):
        k = owner.get(i)
        if k is None:
            continue
        _, seq = chunks[k]
        for (kind, pos, arg) in seq:
            routes[i].append((kind, pos, arg))
    ...
    # ANIMAL LINES (user spec): 5 per quad in a straight line on the quad's
    # shed-side row (N quads: y=3, S quads: y=6) — one band-owner covers all
    # five in a single pass. Quad quotas realize the 6/6/8 herd.
    QUOTA = {"NW": {"COW": 2, "SHEEP": 1, "GOOSE": 2},
    ...
    def _line_cells(q):
        y = 3 if q[0] == "N" else 6
        x0 = 0 if q[1] == "W" else 5
        return [(x, y) for x in range(x0, x0 + 5)]

    q_have = {q: {"COW": 0, "SHEEP": 0, "GOOSE": 0} for q in QUOTA}
    for a in S["animals"]:
        qq = _qof(a["pos"])
        if qq in q_have:
            q_have[qq][a["kind"]] += 1
    ...
    def _place_kind(kind):
        struct = ANIMALS[kind]["structure"]
        # pass 1: existing matching empty structure on any quota line
        for q in sorted(QUOTA):
            if q not in S["quadrants"] or q_have[q][kind] >= QUOTA[q][kind]:
                continue
            for c in _line_cells(q):
                if (c in reserved or c in SHED_TILES):
                    continue
                if (c in sim["struct_at"] and sim["struct_at"][c] == struct
                        and c not in sim["animal_at"]):
                    reserved.add(c)
                    backlog.append(["PLACE", c, kind, -1])
                    q_have[q][kind] += 1
                    return True
        # pass 2: first free line cell (build) in a quota quad
        for q in sorted(QUOTA):
            if q not in S["quadrants"] or q_have[q][kind] >= QUOTA[q][kind]:
                continue
            for c in _line_cells(q):
    ...
    for s in S["structures"]:
        ...
        for kind in ("COW", "SHEEP", "GOOSE"):
            if shed_avail(kind, 24) > 0 and ANIMALS[kind]["structure"] == s["kind"]:
                ...
                break
    # PER-QUAD PLANTING (user system): crops rotate across quads so every
    # quad fills evenly and each quad's hands plant their own tiles.
    plant_jobs = []
    # CAPACITY-TIED WATER-LEVEL FILL: measured ~4 tended tiles per hand.
    # Never plant beyond what the crew can service (sprawl = unwatered =
    # deaths); fill quads evenly (water level) so bands stay dense.
    crew_cap = 4 * max(1, want)
    # USER STAGES: half the first quad to start, then grow every stage
    # (~6 tiles/day) until every quad is full, then maintain.
    nq_cap = 25 * len(S["quadrants"])
    stage_cap = 12 + 6 * max(0, day - 4)
    cur_planted = len(S["plants"])
    room_total = max(0, min(crew_cap, stage_cap, nq_cap) - cur_planted)
    q_cycle = sorted(S["quadrants"])
    qi = 0
    used_cells = []
    for crop in ("CARROT", "STRAWBERRY", "MELON", "TOMATO", "WHEAT"):
        n = int(seeds_avail(crop, 5))
        for _ in range(n):
            if len(plant_jobs) >= room_total:
                break
            placed = False
            for _try in range(len(q_cycle)):
                q = q_cycle[(qi + _try) % len(q_cycle)]
                y0 = 0 if q[0] == "N" else 5
                x0 = 0 if q[1] == "W" else 5
                row_order = [2, 1, 0, 4] if q[0] == "N" else [7, 8, 9, 5]
                cands = []
                for ry in row_order:
                    for x in range(x0, x0 + 5):
                        c = (x, ry)
                        if (c in sim["empty_at"] and c not in reserved
                                and c not in used_cells):
                            cands.append(c)
                if cands:
                    used_cells.append(cands[0])
                    plant_jobs.append(["PLANT", cands[0], crop, -1])
                    qi = (qi + _try + 1) % len(q_cycle)
                    placed = True
                    break
            if not placed:
                break
    backlog.extend(plant_jobs)
    for w in sorted(sim["weed_at"], key=lambda c: _d(c, (4, 4)))[:30]:
    ...
    def unit_step(i, h):
        ...
        # ---- fixed route ----
        rt = routes[i]
        if sum(sim["inv"][i].values()) >= 10 and ptr[i] < len(rt):
            rt.insert(ptr[i], ("DROPC", None, None))
        while ptr[i] < len(rt):
            ...
            if kind == "HARVESTA":
                ...
                if u == pos:
                    ...
                    if sim.get("harv_units", 0) + yv > 200:
                        ptr[i] += 1; continue
                    sim["harv_units"] = sim.get("harv_units", 0) + yv
            ...
            if kind == "HARVESTC":
                ...
                if u == pos:
                    pl = sim["plant_at"][pos]
                    if sim.get("harv_units", 0) + pl["yield"] > 200:
                        ptr[i] += 1; continue
                    sim["harv_units"] = sim.get("harv_units", 0) + pl["yield"]
        ...
        for t in backlog:                 # release my stale claims first
            if t[3] == i:
                t[3] = -1
        _uq = sim.get("unit_quad", {}).get(i)
        for t in sorted(backlog, key=lambda t: 0 if (t[1] and _uq and
                (("N" if t[1][1] < 5 else "S") + ("W" if t[1][0] < 5 else "E")) == _uq)
                else 1):
            ...
            if kind == "PLANT":
                ...
                if avail <= 0:
                    ...
                    continue
                if h >= 23:
                    continue              # h22 plants still get the same-day WATER we insert
                t[3] = i
                if u == pos:
...
def agent(obs, config=None):
    ...
    # ---- THE FLOOD (every hour): fill leftover order slots with mega-sells.
    # Shed drains hourly -> nothing harvested is ever lost to EOD overflow,
    # and every unit converts to cash the same hour we take it.
    if hour >= 1:
        shed_ = dict((obs.get("private", {}) or {}).get("shed", {}) or {})
        extra = [["SELL", it, 10 ** 6] for it in
                 ("MELON", "STRAWBERRY", "MILK", "WOOL", "EGG", "CARROT")]
        if shed_.get("FERTILIZER", 0) > 12:
            extra.append(["SELL", "FERTILIZER", shed_["FERTILIZER"] - 12])
        for o in extra:
            if len(mkt) >= 10:
                break
            mkt.append(o)

        # ---- WHEAT DESK (market-maker): own the wheat book. ----
        # Wheat is the only item with BOTH a BUY_PRODUCT hook and a town-drain
        # rebound (~31/day consumption). Top bots run this desk; we run it
        # first and BIGGER: buy their dumps (support), trickle the rips (cap).
        # The band keeps wheat pinned ~21-27 so the enemy's late $40+ premium
        # never materializes, and we pocket the spread every cycle.
        if 2 <= day <= 26:
            mk_ = obs.get("market", {}) or {}
            pxs_ = mk_.get("prices", {}) or {}
            pxw = float(pxs_.get("WHEAT", 25))
            n_an_ = 0
            for row_ in obs["farms"][p].get("tiles", []):
                for t_ in row_:
                    if isinstance(t_, dict) and "animal" in t_:
                        n_an_ += 1
            keep_ = 0 if day >= 29 else (n_an_ if day >= 28 else max(6, n_an_ * 2))
            book_ = shed_.get("WHEAT", 0) - keep_
            total_shed_ = sum(shed_.values())
            money_ = float(obs["farms"][p].get("money", 0))
            desk = None
            if pxw <= 21 and day <= 25:
                # BUY THE DIP: they dumped; we take it all off the market.
                room_ = max(0, 90 - total_shed_)
                qty_ = min(60 if pxw <= 12 else 40, room_)
                if money_ >= 1500:
                    qty_ = min(qty_, int((money_ - 1500) // max(1.0, pxw)))
                    if qty_ > 0:
                        desk = ["BUY_PRODUCT", "WHEAT", qty_]
            elif pxw >= 27 and book_ > 0:
                # TRICKLE THE RIP: cap the price, convert the book slowly.
                desk = ["SELL", "WHEAT", min(book_, 12)]
            elif shed_.get("WHEAT", 0) > keep_ + 70:
                # shed-cap safety: book max 70
                desk = ["SELL", "WHEAT", shed_.get("WHEAT", 0) - keep_ - 70]
            if desk:
                if len(mkt) < 10:
                    mkt.append(desk)
                else:
                    # replace the last flood mega-sell: the desk pays more
                    mkt[9] = desk
    farm = obs["farms"][p]
