...
def plan_market(S, day):
    ...
    # ---- MATH-DRIVEN CREW (measured: 2.87 ops/animal/day + clustered walk
    # ~= 4 hours per animal -> ~5 animals per hire; field tasks ~1.5h each) ----
    ...
    # economy: pre-d20 spend to the floor (growth); post-d20 bank (score = bank)
    ...
    targets["WHEAT"] = max(targets.get("WHEAT", 0), min(46, 14 + 2 * herd))
    if 18 <= day <= 26:
    ...
    # ---- 4) ANIMALS TO 20 (meta parity; feed covered by the wheat floor) ----
    sheep_t = min(6, max(0, day - 1)) if day <= 8 else 6
    cow_t = min(10, max(1, day)) if day <= 10 else 10
    tgt = {"GOOSE": (4 if day >= 2 else 0),
           "COW": cow_t,
           "SHEEP": sheep_t}
    have = {}
    for a in animals:
