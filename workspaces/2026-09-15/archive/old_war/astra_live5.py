...
def agent(obs, configuration=None):
    ...
    if plant_choices and not final_day:
        # TRACE-04: crops capped at 13-29 while 52-78 tiles sat empty with 12
        # seeds held — PLANT at priority 340 lost every scoring race to
        # WATER(740)/FEED(760)/HARVEST(900) once the herd grew. Planting is
        # the engine's core throughput; it must outrank routine tending.
        for pos in crop_empty:
            jobs.append(("PLANT", pos, None, 720))
        for pos in crop_weeds:
            jobs.append(("CLEAR", pos, None, 640))
    ...
    try:
        with open("/tmp/astra_econ5.txt", "a") as _lf:
            _lf.write(
