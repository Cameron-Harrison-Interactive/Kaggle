...
def agent(obs, config=None):
    ...
    # ---- v10: tetsu-class d0 all-in (census: HIRE5, COW2+SHEEP2,
    # WHEAT7+MELON12, BUY wheat 22 feed stock, EOD $27) ----
    if day == 0:
        if n_hands < 5:
            for _ in range(5 - n_hands):
                market.append(["HIRE"])
        if shed.get("COW", 0) == 0 and n_cows < 2:
            market.append(["BUY_ANIMAL", "COW", 2])
        if shed.get("SHEEP", 0) == 0 and n_sheep < 2:
            market.append(["BUY_ANIMAL", "SHEEP", 2])
        if seeds.get("MELON", 0) < 12 and money > 1000:
            market.append(["BUY_SEED", "MELON", 12 - int(seeds.get("MELON", 0))])
        if seeds.get("WHEAT", 0) < 10 and money > 60:
            market.append(["BUY_SEED", "WHEAT", 28 - int(seeds.get("WHEAT", 0))])
        _w0 = ((obs.get("market") or {}).get("prices") or {}).get("WHEAT", 40) or 40
        if shed.get("WHEAT", 0) < 22 and money > 900:
            market.append(["BUY_PRODUCT", "WHEAT", min(22, int(money // max(1, int(_w0) + 1)))])
    ...
    if (5 <= day <= 20 and n_geese < TARGET_GEESE and money > 420
    ...
    # wheat dip-buy (CD market-making layer): absorb cheap evening dumps,
    # resell at premium via the price-aware sell block below.
    _wx = ((obs.get("market") or {}).get("prices") or {}).get("WHEAT", 40) or 40
    _wload = sum(v for v in shed.values() if isinstance(v, (int, float)))
    if (3 <= day <= 26 and _wx < 33 and money > 1500 and _wload < 70):
        _wq = min(80, int((money - 1200) // max(1, int(_wx) + 1)))
        if _wq > 0:
            market.append(["BUY_PRODUCT", "WHEAT", _wq])
    ...
    def plan_crop():
        ...
        if (seeds.get("STRAWBERRY", 0) > 0 and 2 <= day <= 12
                and strb_tiles < 24 and wheat_tiles >= 6):
            return "STRAWBERRY"
    ...
    if day >= 4 and coops_total < TARGET_GEESE:
        for c in COOP_SPOTS:
            if tiles[c[1]][c[0]] is None:
                jobs.append((c[0], c[1], "BUILD_COOP", 260.0, None))
