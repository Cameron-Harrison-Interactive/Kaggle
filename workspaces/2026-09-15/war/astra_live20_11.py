...
R51b v2: THE CARE CREW - a 3rd NW animal worker appended LAST in the
role list (thin-crew days trim it first, never SW), +1 hire on solvent
days. v1 (role inside NW's block) measured the mechanism: animal rev
+$7.9k (care 44->56%, fert 62->74%) but the SW pop + wage cash cost
$13k of crops. (The R51 ranch-cow expansion measured -4.3/-6.3/-12.2k -
SE opens too late for a cow - and is NOT in this file.)
"""
...
def agent(obs, configuration=None):
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        # R51b (ladder replays): the CARE CREW role - a 3rd NW animal
        # worker. Ladder care coverage ~50% / fert ~55% forfeits ~$20-40k
        # of care bonuses per game (engine: daily care+feed = cow 3
        # units/production vs 1, sheep 4, goose 2). The reorder family
        # is closed (measured negative) - this is the prescribed
        # MORE-LABOR fix. The role is appended LAST and the trim pops
        # from the end, so a thin-crew day (hire failed: cash+180) drops
        # the care role ITSELF, never SW's second worker (the R30
        # lesson). The +1 hire below funds it on solvent days; measured
        # v1 (role inside NW's block): animal rev +$7.9k but the SW pop
        # + wage cash ate $13k of crops = -5.1k net.
        _care_crew = (day >= 8 and len(quads) == 4 and sum(
            1 for _p, _t in animals
            if quadrant(_p) in ("NW", "NE")) >= 8)
        if _care_crew:
            roles.append(("A", "NW"))
        qi = 0
    ...
    # R51b: fund the care-crew role (3rd NW A-worker) - same gate as the
    # role; on a cash-tight morning the hire (and role) simply skip.
    if (day >= 8 and sum(1 for _p, _t in animals
                         if quadrant(_p) in ("NW", "NE")) >= 8
            and len(owned) == 4):
        desired_hands += 1
