"""
R41 (09-14, user design - THE SWAP: factory-first opening, late melon
wave) - MEASURED, REJECTED: solo 67,015/80,296/69,683 = 72.3k vs
live20_6's 99,959 on the same seeds (-27.6k). The autopsy (tape s42):
the factory DID form (30 standing by d10) but still converted only
132 units (~4/tile - the water+tick pairing, not the schedule, is the
blocker); the wave's $9-12k d10-12 dump was STRUCTURAL - without it SW
opened d19 (was d16), SE d22 (was d16), and the whole mid-game ran 3-6
days late; the late wave (d15-17) yielded 25 units/$6.6k vs the early
wave's 140/$27k; the blueprint's d1-3 seed buys ate the NE budget and
NE slid to d8-10. THREE ARCHITECTURES NOW MEASURED: wave+coverage
(20_6, 96.9k) > wave+fert-bolt-on (R40, 93.9k) > factory-first (this,
72.3k). The tick economy needs tetsu's LITERAL labor model (water
every plant every day + daily care), not a schedule swap. KEPT FOR THE
RECORD; live20_6 [v26] stands.
"""
...
def agent(obs, configuration=None):
    ...
    if war:
        ...
        strb_target = min(45, max(33, opp_strb))
    else:
        strb_target = 38 if quoted("STRAWBERRY") >= 180 else 33
    ...
    if not final_day:
        for pos, tile in plants:
            ...
            if int(tile.get("fertilized_until_day", -1)) >= day:
                continue
            if harvestable(tile) and crop not in ("TOMATO", "STRAWBERRY"):
                # R41: ongoing crops stay planted through daily harvest;
                # the spray covers their next 2 ticks (+2 units each)
                continue
    ...
    fert_targets = {
        pos for _, pos in fert_candidates[:min(14, fert_total)]
    }
    ...
    if ("SE" in owned and not final_day
        ...
        if se_fill_n >= 3:
            proj_t = Counter(counts)
            proj_t["TOMATO"] += se_fill_n
            proj_m = Counter(counts)
            proj_m["MELON"] += se_fill_n
            s_wheat = max(1e-9, crop_score("WHEAT"))
            if crop_score("TOMATO", proj_t) >= 2 * s_wheat:
                se_fill_tomato = True
            elif crop_score("MELON", proj_m) > s_wheat:
                melon_cap += se_fill_n
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        if quad == "NW" and not war:
            # R41 (user swap): the STRB FACTORY owns NW's crop tiles
            # from day 1 - ticks d11-20 under the full fert program
            # (3 units per watered+ferted tick; the tetsu machine).
            # The melon wave is GONE from the opening; it replants LATE
            # (d15-17 on freed ground - engine: first yield +10, so a
            # d19 melon would yield 1 unit on the final day; d16-17 is
            # the last paying window). Then the wheat feed desk.
            if (vseeds.get("STRAWBERRY", 0) > 0
        ...
        if (not final_day and vseeds.get("STRAWBERRY", 0) > 0
                and counts["STRAWBERRY"] < STRB_TARGET
                and True):  # R41: factory ground everywhere, incl. NW
            return "STRAWBERRY"
        # R41: the LATE WAVE - melons return on freed ground d15-17
        # (first yield d25-27; 3-6 units into the endgame px)
        if (15 <= day <= 17 and vseeds.get("MELON", 0) > 0
                and counts["MELON"] < 12
                and day + 10 <= total_days - 2):
            return "MELON"
        # R33 (user): measured tomato fill - SE only, gate open, seeds
        # in pocket, and the first yield (day+8) lands before the
        # whistle. The gate (see the R33 block above) read the live
        # market and the opponent before this ever fires.
        if (quad == "SE" and se_fill_tomato
        ...
        live = [
            ...
            # R33: tomato seeds only ever flow to the gated SE fill -
            # never to the open argmax (price crashed, opp runs them,
            # or the window closed)
            and not (c == "TOMATO" and quad != "SE")
        ]
        if filler:
                ...
                for _ in range(held_animals[kind]):
                    ...
                    for pos_s in sites:
                        if pos_s in used:
                            continue
                        q = quadrant(pos_s)
                        if qcount[q] >= (8 if q in ("NW", "SW") else 10):
                            continue
                        t_s = tile_map[pos_s]
                ...
                for pos_a, tile in mine:
                    if not tile.get("fed_today") and not final_day:
                        ...
                        if ((quoted(ANIMALS[tile["animal"]][2]) <= 10
                            ...
                            continue
                        if w["busy"] + distance(w["pos"], pos_a) + 1 <= budget:
                            w["plan"].append(("FEED", pos_a, None))
                            w["busy"] += distance(w["pos"], pos_a) + 1
                            w["pos"] = pos_a
                        else:
    ...
    if (
        ...
        if len(owned) == 1:
            _animal_budget = 0
        elif 2 <= day <= 15 and counts["STRAWBERRY"] < STRB_TARGET:
            _animal_budget = 0
        if (density >= (.90 if day <= 13 else .95)
                and money >= land_cost + 150 + _fill_cost + _animal_budget
                and herd_ready
    ...
    if day >= 2 and not final_day:
        ...
        # R33 (user): tomato fill seeds - bought only when the gate
        # read the market and said YES (2x wheat churn, net of seed).
        # Toward SE's empty tiles (12-stock pacing from the measured
        # fill), AFTER the replacement-seed invariant above, $150
        # reserve (R32d pacing).
        if (se_fill_tomato and day + 8 <= total_days - 2
                and len(orders) < max_orders):
            tom_room = min(se_fill_n, 12)
            tom_need = max(0, tom_room - seeds.get("TOMATO", 0))
    ...
    targets = {
        ...
        "GOOSE": min(6, 2 + max(0, day - 3)) + (
            2 if (herd.get("COW", 0) >= 6 and herd.get("SHEEP", 0) >= 6)
            else 0),
        ...
        "COW": min(6, max(0, day - 7)),
        "SHEEP": min(6, max(0, day - 8)),
    }
    ...
    if day <= 18 and not final_day:
        ...
        _north_cap = sum(8 if q == "NW" else 10 for q in _north)
        # R38: SW row animals never count against the north cap
        _herd_total = sum(
            1 for p, _t in animals
            if quadrant(p) in ("NW", "NE")) + int(
                sum(held_animals.values()))
        for kind in ("GOOSE", "COW", "SHEEP"):
    ...
    # R38 MEASURED VERDICT: the SW row is DISABLED. Two strikes:
    # (1) first cut -71.7k on 303 (north A-workers walked south for
    # installs; the SW fallback worker's day exploded under a full
    # feed/collect line); (2) with light cadence + own-worker installs
    # + row-of-4, STILL -31.9k on 303: at crew 14 there is no spare
    # labor - the row's ~4 animal-tiles cannot pay for the strb ticks
    # and collections they displace in a rich-price game (best case
    # +$1-2k, downside -30k+). Revisit ONLY with a paying 15th hand.
    if (15 <= day <= 22 and not final_day and "SW" in owned
            and len(orders) < max_orders
            and False  # R38: disabled - see verdict above
            and not state.get("sw_row")):
        _sw_best, _sw_exp = None, 0.0
        for kind in ("COW", "SHEEP", "GOOSE"):
            cost, structure, product, first, interval, cap = ANIMALS[kind]
            if opp_animals.get(kind, 0) > 0:
                continue
            if day + first > total_days - 2:
                continue
            if herd[kind] + held_animals[kind] < targets[kind]:
                continue
            n_prod = (total_days - 2 - day - first) // interval + 1
            if n_prod < 2:
                continue
            exp = min(n_prod, 4) * quoted(product) - 2 * cost
            if exp > _sw_exp:
                _sw_best, _sw_exp = kind, exp
        if _sw_best is not None:
            state["sw_row"] = _sw_best
            # R38: a ROW OF FOUR (user's primary ask). The 8-animal
            # form is labor-negative at crew 14 (measured 303: the
            # fallback worker's day exploded, strb factory untended).
            state["sw_target"] = 4
    if state.get("sw_row") and day <= 24 and not final_day:
        _swk = state["sw_row"]
        _sw_installed = sum(
    ...
    if day <= 1:
        _m_gap_n, _m_res = 15, 150  # the NW crop-tile wave (opening wallet)
    ...
    # R41: the late wave's seeds - bought once the factory is seeded
    if (14 <= day <= 15 and not final_day
            and counts["MELON"] + seeds["MELON"] < 12):
        _mw = 12 - counts["MELON"] - seeds["MELON"]
        _mq = min(_mw, int(max(0, virtual_cash - 400) // 80))
        if _mq > 0 and buy_order(["BUY_SEED", "MELON", _mq], 80 * _mq, 400):
            seeds["MELON"] += _mq
    if (1 <= day <= 15
            and not final_day):
        strb_gap = STRB_TARGET - counts["STRAWBERRY"] - seeds["STRAWBERRY"]
        if strb_gap > 0:
            qty = min(8, strb_gap, int(max(0, virtual_cash - 150) // 100))
            if qty > 0 and buy_order(
    ...
    if 25 <= day <= 27 and not final_day:
        ...
        if _egap > 0:
            ...
            _eq = min(_egap, 8 * max(0, total_days - 2 - day),
                      int(max(0, virtual_cash - 150) // 10))
            if _eq > 0 and buy_order(["BUY_SEED", "WHEAT", _eq],
                                      10 * _eq, 150):
                seeds["WHEAT"] += _eq
    ...
    if day >= 1 and not final_day:
        ...
        filler_crop = ("CARROT" if ((day <= 6 and len(owned) > 1)
                                    or carrot_target) else "WHEAT")
        filler_seeds = sum(
