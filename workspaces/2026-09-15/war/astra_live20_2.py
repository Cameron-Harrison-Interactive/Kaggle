"""
R35 (09-14, user design - PLAN 2: THE D0 OPENING) - MEASURED, REJECTED:
solo 74,241/78,470/91,204 = 81.3k vs live20_1's 99.9k on the same seeds
(-18.6k). Mechanism (census s42): the d0 order slots went to land+goose+
NE-fill, so the melon wave delayed to d2 and shrank to 12 tiles; NE's 25
carrot tiles produced 2 UNITS all game (crew ~9 cannot tend 38 tiles
across 2 quads through d0-3 - they died unwatered); cash sat at $236-$493
through d10-12 and the herd ramp starved (milk 66 vs 100 units). The $3k
start funds NE OR the full opening economy, not both - and day-0 LABOR is
the second wall. KEPT FOR THE RECORD; live20_1 [78bdd357] stands.

R35 design (for the record): day-0 wallet buys NE ($1,000) + ONE goose
($300) + the wave ($1,280) + NE's carrot opener ($300); goose #2 from d1
as cash allows; everything else = the R34 design (steady herd from d4,
mix-first NE, late strb factory d10-15, SE 3rd worker).
...
def agent(obs, configuration=None):
    ...
    if (
        ...
        if len(owned) == 1:
            ...
            _animal_budget = 0
        # R35 (user, live20_2): NE AT D0 - the opening wallet opens the
        # second quad the same day (density is meaningless before the
        # first planting; the money bar still must clear land + NE's
        # opener seeds + the $150 buffer, ~$1,450 at d0 prices).
        if ((density >= (.90 if day <= 13 else .95)
             or (day <= 1 and len(owned) == 1))
    ...
    targets = {
        ...
        # R35 (user, live20_2 = plan 2): ONE goose at d0 - the $3k start
        # buys NE ($1,000) + 1 goose ($300) + the wave ($1,280) + NE's
        # carrot opener ($300) and still clears wages (~$2,880 total).
        # Goose #2 returns from d1 as soon as cash allows the $650
        # reserve (the NE carrot cash d2-3 funds it by ~d3-4).
        "GOOSE": (1 if day == 0 else min(6, 2 + max(0, day - 3))) + (
    ...
    if day <= 18 and not final_day:
        ...
        if candidates:
            ...
            qty = 1 if day == 0 and kind == "GOOSE" else 1
            qty = max(1, qty)
