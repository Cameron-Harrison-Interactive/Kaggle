"""
R48 (09-14, user whole-commit - COWS-FIRST CRASH RAMP) - MEASURED,
REJECTED BOTH WAYS: solo 77.4k (-20.4k) AND head-to-head WORSE (vs
elite tape median -110,030 vs v28's -98,292; vs tetsu -110,304 vs
-92,680). The build: 3 cows d0 (fert $500/day from d1) + wheat start
+ cheap early crew + NE ~d3 + 30-tile double wave + sheep-8 herd +
strb after. WHY THE CRASH-FIRST THESIS FAILS: (1) melon first yield
is d10 - the tapes ALREADY dump at the physics limit; you cannot be
the first seller earlier than first yield; (2) our 30-tile dump cut
OUR realized px (tiles 16-30 at ~$120 vs 1-15 at ~$250) without
hurting theirs - they harvest their ticks before our flood lands;
(3) denial only denies vs opponents who DEPEND on a market (the
elite tape monetizes the same first draw - they ARE the first
seller); (4) the crashed markets we left thin (strb/milk/wool)
stayed RICH for them (they scored 146-175k vs ~128k vs v28 - our
crash fed them). KEPT FOR THE RECORD; live20_8 [v28] stands.
"""
...
def agent(obs, configuration=None):
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        if quad == "NW" and not war:
            ...
            if (vseeds.get("WHEAT", 0) > 0 and day <= 1):
                return "WHEAT"
            if (1 <= day <= 12 and vseeds.get("MELON", 0) > 0
                    and counts["MELON"] < 15):
                return "MELON"
        ...
        # R48 (user): NE opens ~d3 and its ground runs the SECOND wave
        # - 30 melon tiles dumping through d10-19 (crash-first: their
        # tape dumps 12 tiles at d10; we arrive bigger and earlier-ish,
        # then the ground hands over to the strb factory).
        if (quad == "NE" and 3 <= day <= 8
    ...
    targets = {
        ...
        # R48 (user whole-commit): 3 cows at d0 - fert ($100/u daily,
        # 5 animals from d1) + milk d8 fund the ramp; sheep push to 8
        # for wool-market coverage; cows settle at 4 (north cap 18).
        "COW": (3 if day == 0 else min(4, 3 + max(0, day - 2))),
        "SHEEP": min(8, max(0, day - 5)),
    }
