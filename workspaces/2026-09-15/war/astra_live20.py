"""
WIP 09-11 (evening): ZONED TEAM ROUTING + NORTH HERD (astra_live20) -
NOT a submission candidate. Progress this session: 20k -> 65-70k solo
(baseline v21: 97-109k). KEPT (all measured): teams of 3/quad (north:
2 crop + 1-2 animal; south: 3 crop, no animals); herd concentrated in
the NORTH rows (8/quad, 16 total; NW has exactly 8 non-shed row spots);
feed guarantee (feeding never depends on crew size - crop worker or
farmer runs the line first); wage reserve relaxes when the herd needs
its crew (the $178-vs-$181 miss = zero hands = herd escaped d3);
cargo-aware installs; melon early boost d2-10 (+15k); growth reserve
1100->400 (+4-8k). FIXED CATASTROPHES: opening herd escape (d3 night),
install cargo-strand, staffing-trim leaving no crop worker.
OPENING (swept 09-11 night): COW+SHEEP start + wheat8/carrot4/melon6 - best d1-5 ramp; melon boost is LOAD-BEARING (+20k final). TWO ANIMAL ROWS + NW-FIRST (09-11 late): herd 16 by d14-16, NE@d10 funded, solo 63-80k avg 70.6k. STALL SOLVED 09-12 (core-crew wages + tick-water priority): 42 $76k (best seed), 202 melons back, 6-seed avg 66.7k. 5 A-WORKERS / 18-HERD (09-12): 71.1k gate avg (42 best-ever 76.6k); fert at baseline parity; milk/wool closing. REMAINING GAPS (next, in order): herd mix stuck thin post-NE (milk 60
vs 114, wool 33 vs 82 - growth buys crawl; check the milk/fert price
gate + one-kind-per-turn), strb 137 vs 172, missed waters ~530 (standing
is higher; ZONE_CAP 8/quad-worker may need 10), dead plants ~52.
Compare: astra_live19.py [75f03b70] = submitted v21 (the live line).
...
def agent(obs, configuration=None):
    ...
    strb_target = 45 if war else min(45, max(33, opp_strb))
    ...
    def harvestable(tile):
        ...
        if (crop == "MELON" and age >= 9 and units >= 4
                and money < 600):
            return True
    ...
    targets = {
        ...
        "COW": (min(4, 1 + max(0, day - 4)) if 5 <= day <= 9
                else min(6, max(0, (day - 2) // 2))),
        "SHEEP": (min(3, max(0, day - 4)) if 5 <= day <= 9
                  else min(6, max(0, (day - 4) // 2))),
    }
    ...
    if ((day >= 10 or len(owned) >= 2) and day <= 16
            and not final_day):
    ...
    if 25 <= day <= 27 and not final_day:
        ...
        if _egap > 0:
            _eq = min(_egap, int(max(0, virtual_cash - 150) // 10))
