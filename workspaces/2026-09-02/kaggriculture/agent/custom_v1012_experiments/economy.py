...
def plan_market_orders(board, cfg, spy=None):
    ...
    if len(orders) < 10 and hour <= 3:  # buy early in the day
    ...
    for (x, y), role in layout.items():
        ...
        if role == "CARROT":
            ...
            continue
        # S62: STRAW/CROP tiles follow the live shop-adaptive crop choice —
        # when straw drain is low, desired_crop_for returns WHEAT there and
        # we must buy WHEAT seeds (not $100 straw seeds we'll never plant).
