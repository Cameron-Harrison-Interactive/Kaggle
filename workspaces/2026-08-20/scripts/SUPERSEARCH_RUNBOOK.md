...
## three-fer — READ THIS FIRST (2026-08-13)

The v20 surgical 3,060-variant run **finished**. Verdict: **no improver**.
Keep `HI_AgriBot_v18` live. Do **not** launch another cartesian of idle /
leftover-harvest / feed / splice — those dims are measured dead.

`scripts/three_fer.py` is the next one-off (minutes, not hours):

1. **leftover walker** — real day-27/28/29 walk to the 7 leftover wheat
   (the PASS-on-tile harvest never fired because workers never stand on
   row 8–9).
2. **tetsu / seat1 counter** — runtime overlay keyed off the opponent's
   **visible farm** (crop counts, quads, plant-tile Jaccard vs our layout).
   No mirror-clone, no sabotage.
3. **distinctive path** — equivalent-length Manhattan rewrite of existing
   MOVE sequences so the walk is not a Yubo/Gbining copy, same anchors.
...
Ship gate unchanged: seed1 seat0 vs PASS ≥ $167,978, animals ≥ 13, 0 losses
vs v18 on seeds 1-3 both seats. `--build-agent` is a no-op unless SHIP=YES.
Outputs land in `data/three_fer/` (`diagnose.json`, `ledger.jsonl`, `report.json`).

---

## v20 — READ THIS FIRST (2026-08-13)

The 10,800-variant cartesian run **finished**. Result: **zero variants beat
base ($167,978)**. Crop swaps, hire scaling, dropping animals, fill, early
plant, and full-route water recompiles all lost. The printed "champion"
`crop0+h1.0+a13+w_young+f8+e0+fr0` was a reconstruction bug — finalists were
compiled without their parameters, so every finalist was the base tape.

`supersearch_turbo.py` is now a **surgical** search. Same launch command.
It starts from the proven tape and searches real routing (odd-day splices,
path style, idle WATER, leftover harvest, cheap feed). Resume is ON by
default. Wipe the old cache once:
