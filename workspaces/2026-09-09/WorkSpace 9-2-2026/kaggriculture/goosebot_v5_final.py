...
def agent(obs, config=None):
    ...
    if day <= 2 and seeds.get("STRAWBERRY", 0) < 4 and money > 1600:
        market.append(["BUY_SEED", "STRAWBERRY", 4 - int(seeds.get("STRAWBERRY", 0))])
    ...
    if unlocked < 2 and 5 <= day <= 20 and money > 2000:
    ...
    if (1 <= day <= 8 and n_cows < 4 and money > 500
            and pastures_total > n_cows):
        market.append(["BUY_ANIMAL", "COW", 1])
    elif (9 <= day <= 23 and n_cows < TARGET_COWS and money > 800
            and pastures_empty and hour <= 12 and shed.get("COW", 0) == 0
            and wheat_secure and n_geese >= 4):
        market.append(["BUY_ANIMAL", "COW", 1])
    if (9 <= day <= 23 and n_sheep < TARGET_SHEEP and money > 800
    ...
    for i in range(n_units):
        ...
        if is_feeder:
            ...
            if k_feeders > 1:
                _sz = (len(animals_all) + k_feeders - 1) // k_feeders
                my_animals = list(animals_all[(my_idx - 1) * _sz: my_idx * _sz]) \
                    or list(animals_all[-_sz:])
            else:
                my_animals = list(animals_all)
        ...
        if is_fert_runner and act is None:
            if inv.get("FERTILIZER", 0) > 0 and strb_needs_fert:
                t = take_nearest(px, py, strb_needs_fert)
                act = (["FERTILIZE"] if (px, py) == t
                       else _gb_move(px, py, t[0], t[1]))
            elif inv.get("FERTILIZER", 0) >= 4 or (inv.get("FERTILIZER", 0) > 0 and hour >= 20):
                ts = min(SHED_TILES,
                         key=lambda s: abs(s[0] - px) + abs(s[1] - py))
        ...
        if act is None:
            ...
            if weed_cand:
            ...
            elif (day >= 1 and pastures_total < TARGET_COWS + TARGET_SHEEP
                ...
                p_max = max(12 + strb_tiles + melon_tiles, (5 * (1 + target_hands) - 2 * n_animals) // 2)
                ...
                if (len(plants_all) < p_max and planted_today < day_cap
                    ...
                    if cand:
                        t = take_nearest(px, py, cand)
                        claimed.add(t)
                        if (seeds.get("MELON", 0) > 0 and 1 <= day <= 5
                                and melon_tiles < 8):
                            crop = "MELON"
                        elif (seeds.get("STRAWBERRY", 0) > 0 and 2 <= day <= 9
                                and strb_tiles < 4
                                and wheat_tiles < 8):
                            crop = "STRAWBERRY"
