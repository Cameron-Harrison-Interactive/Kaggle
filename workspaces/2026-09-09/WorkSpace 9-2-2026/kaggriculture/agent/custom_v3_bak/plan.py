...
# NE (y 0..4, x 5..9): rows 0-2 crop wheat, row 3 pasture, row 4 coop.
# Strawberry lives in SW/SE where it doesn't displace wheat volume.
...
# SW (y 5..9, x 0..4): mix — first 2 rows STRAWBERRY (high ROI ongoing),
# rest CROP (wheat) for volume.
SW_LAYOUT = {}
for y in range(5, 10):
    for x in range(5):
        if (x, y) in SHED_SET:
            ...
            SW_LAYOUT[(x, y)] = "STRAW"    # 5 tiles per row × 2 = ~9 strawberry (minus shed access)
        else:
            SW_LAYOUT[(x, y)] = "CROP"

# SE (y 5..9, x 5..9): 1 row strawberry then wheat
SE_LAYOUT = {}
for y in range(5, 10):
    for x in range(5, 10):
        if (x, y) in SHED_SET:
...
def desired_crop_for(x, y):
    ...
    role. WHEAT for CROP tiles, STRAWBERRY for STRAW tiles."""
