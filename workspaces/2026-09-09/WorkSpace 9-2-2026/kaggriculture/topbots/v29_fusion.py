...
# ============================================================
# FUSION LAYER (Harrison Interactive, 2026-09-08):
#   - keep tape buys (hires/seeds/animals/land/wheat/fert) + tape labor
#   - strip tape SELL dumps (they synchronize with clones and crash prices)
#   - adaptive volume sells with floors (front-run the mirror crash)
#   - wheat carry overlay (buy <=33, sell >=42, never touch feed buffer)
# ============================================================
_FLOOR = {"MILK": 25, "WOOL": 40, "STRAWBERRY": 60, "FERTILIZER": 15,
          "EGG": 20, "CARROT": 15, "MELON": 80, "TOMATO": 40}
_FUS = {}

def _fusion_sells(obs):
    """Return our sell orders + carry orders for this step (list of market orders)."""
    try:
        p = int(obs["player"])
        farm = obs["farms"][p]
        ...
        for row in farm["tiles"]:
            for tile in row:
                if isinstance(tile, dict) and "animal" in tile:
                    n_an += 1
        # volume sells at floors (never dump below)
        vals = []
        for it, fl in _FLOOR.items():
            q = shed.get(it, 0)
            if q > 0 and prices.get(it, 0) >= fl:
                keep = 15 if it == "FERTILIZER" else 0
                if q - keep > 0:
                    vals.append((q * prices.get(it, 0), ["SELL", it, q - keep]))
        vals.sort(key=lambda v: -v[0])
        orders.extend(o for _, o in vals[:6])
        # wheat: carry + surplus above feed buffer
        pw = prices.get("WHEAT", 25)
        sw = shed.get("WHEAT", 0)
        feed_buf = max(10, int(n_an * 2.5))
        shed_used = sum(shed.values())
        if day >= 27:
            if sw > 0 and pw >= 30:
                orders.append(["SELL", "WHEAT", max(0, sw - (n_an if day == 27 else 0))])
        elif day >= 12 and pw >= 42 and sw > feed_buf:
            orders.append(["SELL", "WHEAT", min(sw - feed_buf, 25)])
        return orders
    except Exception:
        return []

def _fusion_buys(obs):
    """Carry buys in spare slots."""
    try:
        p = int(obs["player"])
        farm = obs["farms"][p]
        shed = obs.get("private", {}).get("shed", {}) or {}
        ...
        shed_used = sum(shed.values())
        out = []
        if day <= 22 and pw <= 33 and money >= 2200 and shed_used <= 62:
            room = 62 - shed_used
            q = min(20, room, int((money - 2000) // max(1, pw)))
            if q > 0:
                out.append(["BUY_PRODUCT", "WHEAT", q])
        return out
    except Exception:
        return []
...
def agent(observation, configuration=None):
    # tape first, always safe
    base = _agent_core(observation, configuration)
    try:
        market = [list(o) for o in base["market"]]
        # STRICTLY ADDITIVE: tape orders first (its economy depends on its own
        # cash-flow choreography); our sells/carry only fill free slots.
        ours = _fusion_sells(observation) + _fusion_buys(observation)
        out = market + ours
        return {"farmer": base["farmer"], "hands": base["hands"], "market": out[:10]}
    except Exception:
        return base
...
# FINAL entrypoint binding (last callable in module dict):
