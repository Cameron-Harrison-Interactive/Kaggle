...
VERSION = "HI_AgriBot_v18.8_CrashDump"
import base64
import copy
import json
import math
...
_CRASH_GATE = {
    "enabled": True,
    "start_day": 14,
    "price_ratio": 0.75,   # straw price < 75% of base (120) = 90
    "min_sell": 6,
    "horizon": 48,         # look up to 2 days ahead for planned straw sells
}
...
def _repay_crash_debt(obs, action):
    """At the planned sell step, reduce the tape's STRAWBERRY SELL by the
    amount we already front-ran (net zero total sold)."""
    try:
        seat = _seat(obs)
        step = int(obs.get("step", 0) or 0)
        debts = _CRASH_DEBT.get(seat, {})
        due = debts.pop(step, None)
        if not due:
            return action
        market = []
        for raw in action.get("market", []) or []:
            ...
            if len(o) >= 3 and o[0] == "SELL" and o[1] == "STRAWBERRY":
                q = max(0, int(o[2]))
                red = min(q, int(due.get("STRAWBERRY", 0) or 0))
                due["STRAWBERRY"] = int(due.get("STRAWBERRY", 0) or 0) - red
                q -= red
                if q <= 0:
                    continue
                o[2] = q
...
def _adapt_market(obs, action):
    """Family-gated early straw dump (v18.8).

    Live losses: straw $196@d15 -> $20@d20 -> $1@d25. The tape sells straw
    on a fixed schedule; when the price is CRASHING we pull the next planned
    STRAWBERRY SELL forward and repay it at the original step (debt netting,
    same as clone preemption). Mirrors with healthy markets never trigger:
    gate needs price < 75% of base AND falling AND opp straw-heavy.
    """
    try:
        if not _CRASH_GATE["enabled"]:
            return action
        day = int(obs.get("day", int(obs.get("step", 0) or 0) // 24) or 0)
        if day < _CRASH_GATE["start_day"]:
            return action
        m = _mem_for(obs)
        if m.get("mode") not in ("anti_straw", "default", "mirror"):
            return action
        market = _get(obs, "market", {}) or {}
        prices = _get(market, "prices", {}) or {}
        px = float(_get(prices, "STRAWBERRY", 120) or 120)
        base = float(_MARKET_PARAMS["STRAWBERRY"][0])
        if px >= base * _CRASH_GATE["price_ratio"]:
            return action
        # falling? compare with 24h ago
        ph = m.get("price_hist", {})
        hist = ph.get("STRAWBERRY", []) or []
        if len(hist) >= 2 and hist[-1] >= hist[-2]:
            return action
        # opp straw-heavy?
        opp = _opp_farm(obs)
        if _count_crop(opp, "STRAWBERRY") < 12:
            return action
        # find the next planned straw sell in our tape within horizon
        actions = _SEAT1_ACTIONS if _seat(obs) == 1 else _SEAT0_ACTIONS
        step = int(obs.get("step", 0) or 0)
        planned = None
        for look in range(1, _CRASH_GATE["horizon"] + 1):
            fs = step + look
            if fs >= len(actions):
                break
            for o in actions[fs].get("market") or []:
                if len(o) >= 3 and o[0] == "SELL" and o[1] == "STRAWBERRY":
                    q = max(0, int(o[2]))
                    if q >= _CRASH_GATE["min_sell"]:
                        planned = (fs, q)
                        break
            if planned:
                break
        if not planned:
            return action
        # pull it forward: sell from shed now, repay at planned step
        shed = _get(_get(obs, "private", {}) or {}, "shed", {}) or {}
        avail = max(0, int(shed.get("STRAWBERRY", 0) or 0))
        if avail < _CRASH_GATE["min_sell"]:
            return action
        target = min(avail, planned[1])
        market_act = list(action.get("market") or [])
        if len(market_act) >= 10:
            return action
        market_act.append(["SELL", "STRAWBERRY", target])
        action["market"] = market_act[:10]
        # record debt to repay at the planned step (runtime, per-seat —
        # never mutate the shared tape)
        try:
            seat = _seat(obs)
            due = _CRASH_DEBT.setdefault(seat, {}).setdefault(planned[0], {})
            due["STRAWBERRY"] = due.get("STRAWBERRY", 0) + target
        except Exception:
            pass
    except Exception:
        pass
...
def _race_liquidation(obs, action):
    ...
    try:
        ...
        PREMIUM = ("STRAWBERRY", "MELON", "TOMATO", "CARROT", "EGG", "MILK",
                   "WOOL", "FERTILIZER")
        _BASEPX = {"STRAWBERRY": 120, "MELON": 250, "TOMATO": 60, "CARROT": 35,
                   "EGG": 50, "MILK": 160, "WOOL": 200, "FERTILIZER": 100}
        _px = (obs.get("market") or {}).get("prices") or {}
        for item in PREMIUM:
            ...
            if slots_used >= 10:
                break
            if day < 12:
                # adapeak window: only sell lines still near base price;
                # never dump into an already-crashed line before d12
                p = float(_px.get(item, _BASEPX[item]) or _BASEPX[item])
                if p < 0.9 * _BASEPX[item]:
                    continue
