...
def agent(obs, config=None):
    ...
    target_sheep = 0 if wool_flood > 10 else 1
    ...
    if day <= 22 and pw <= 33 and money >= 2200 and shed_room >= 30:
        q = min(60, shed_room - 20, int((money - 1800) // max(1, pw)))
        if q > 0:
    ...
    if hour <= 1:
        ...
        if day <= 22 and total_animals < 8:
            ...
            if 0 <= day <= 16 and animals.get("SHEEP", 0) + shed.get("SHEEP", 0) < target_sheep:
        ...
        if len(farm["unlocked_quadrants"]) == 1 and day >= 6 and money >= 1300:
            o_land.append(["BUY_LAND"]); money -= 1000
        elif len(farm["unlocked_quadrants"]) == 2 and day >= 10 and money >= 2700:
