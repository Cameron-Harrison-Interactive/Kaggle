...
# ---------------------------------------------------------------------------
# melon-early overlay: the blob's melon sells fire late (post-crash $19 in
# episode 106047657 vs JOSHNA's early $169). Sell MELON into the h1-h3
# premium band whenever price >= 100 and no melon sell is already posted.
# Market-only: never drops the blob's own orders, never touches labor.
# ---------------------------------------------------------------------------

_MELON_ROUTE_AGENT = agent


def agent(obs):
    action = _MELON_ROUTE_AGENT(obs)
    try:
        hour = obs.get("hour")
        if hour is not None and 1 <= int(hour) <= 3:
            prices = (obs.get("market") or {}).get("prices") or {}
            mp = prices.get("MELON") or 0
            if mp >= 100:
                m = list((action or {}).get("market") or [])
                has_melon_sell = any(isinstance(o, list) and len(o) > 1
                                     and o[0] == "SELL" and o[1] == "MELON" for o in m)
                if not has_melon_sell:
                    shed = (obs.get("private") or {}).get("shed") or {}
                    q = int(shed.get("MELON", 0) or 0)
                    if q > 0 and len(m) < 10:
                        m.append(["SELL", "MELON", q])
...
# Kaggle entrypoint: guaranteed last callable.
