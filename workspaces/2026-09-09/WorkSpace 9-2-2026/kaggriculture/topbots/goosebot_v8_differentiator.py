...
def agent(obs, config=None):
    ...
    if (2 <= day <= 22 and n_sheep < TARGET_SHEEP
            ...
            and (pastures_empty or sheep_early
                 or (pastures_total - n_cows - n_sheep) > 0) and hour <= 12
    ...
    try:
        _opp = farms_list[1 - seat] if len(farms_list) > 1 else None
        _opp_strb = 0
        if _opp and day >= 4:
            for _row in (_opp.get("tiles") or []):
                for _t in _row:
        ...
        _opp_strb = 0
    _strb_cap = 18 if _opp_strb >= 20 else 33
    _carrot_cap = 16 if _opp_strb >= 20 else 12

    def plan_crop():
        if seeds.get("MELON", 0) > 0 and day <= 5 and melon_tiles < 12:
            return "MELON"
        if (seeds.get("STRAWBERRY", 0) > 0 and 2 <= day <= 12
                and strb_tiles < _strb_cap and wheat_tiles >= 6):
            return "STRAWBERRY"
        if (seeds.get("CARROT", 0) > 0 and day <= 26
                and wheat_tiles >= wheat_floor and carrot_tiles < _carrot_cap):
            return "CARROT"
