...
def agent(obs, config=None):
    ...
    # v37 CREWS: animal tasks cluster at the pen; field tasks spread wide.
    # Split hands once per day: nearest K to the pen = ANIMAL CREW (they service
    # pen + logistics all day); the rest = FIELD CREW (crops only). Kills the
    # pen<->field commute churn that burned ~200 walk steps/day.
    st.setdefault("crew_state", None)
    cst = st["crew_state"]
    ...
    n_ani_tasks = sum(1 for t in field if not t[5] and is_animal_task(t))
    if cst is None or cst.get("day") != day or cst.get("n_units") != len(units) or cst.get("nt") // 3 != n_ani_tasks // 3:
        # pen centroid = mean position of animal tasks (or shed)
        pts = [t[1] for t in field if not t[5] and is_animal_task(t)] or [(4, 4)]
        pcx = sum(p[0] for p in pts) / len(pts); pcy = sum(p[1] for p in pts) / len(pts)
        # crew size: ~1 per 12 daily animal-task-instances, 2..5
        k = max(2, min(5, (n_ani_tasks * 4) // 24)) if len(units) > 3 else 0
        hands_idx = list(range(1, len(units)))
        hands_idx.sort(key=lambda i: _d(units[i], (pcx, pcy)))
        crew = set(hands_idx[:k])
        cst = {"day": day, "n_units": len(units), "nt": n_ani_tasks, "crew": crew}
        st["crew_state"] = cst
    crew = cst["crew"]
    ...
    for i, u in enumerate(units):
        ...
        if chosen is None:
            if i == 0:
                ...
                if i in crew:
                    # ANIMAL CREW: pen + logistics; field only if idle
                    ani = [t for t in field if not t[5] and is_animal_task(t)]
                    chosen = best_task(ani, u, i) or best_task(logi, u, i) \
                             or best_task([t for t in field if not t[5] and not is_animal_task(t)], u, i) \
                             or best_task(field, u, i)
                else:
                    # FIELD CREW: crops; sector fallback stays
                    fld = [t for t in field if not t[5] and not is_animal_task(t)]
                    chosen = best_task(fld, u, i, restrict_sector=sector(u)) \
                             or best_task(fld, u, i, restrict_sector=(i - 1) % n_hands) \
                             or best_task(fld, u, i) \
                             or best_task(field, u, i)
        if chosen is None:
