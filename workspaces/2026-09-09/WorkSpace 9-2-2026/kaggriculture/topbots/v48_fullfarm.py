...
def plan_market(S, day):
    ...
    if 3 <= day <= 19:
        base_s = (32 if pxs >= 120 else 20) if day >= 10 else (20 if pxs >= 120 else 16)
        targets["STRAWBERRY"] = base_s   # treadmill; d19 last sow prods by d29
    if day >= 8 and pxt >= 50 and pxs < 150:
        targets["TOMATO"] = 6
    # bounded board fill (day-gated): every empty tile gets a crop from d10
    if day >= 10 and len(S["quadrants"]) >= 3:
        if pxt >= 60 and 10 <= day <= 21:
