...
def agent(obs, config=None):
    ...
    if st["v43"] is None or st["v43"].get("day") != day:
        routes = {}
        ...
        if len(units) == 1:
            ...
            # pen crew: scales with animal load; pickups pre-scheduled at head
            k = max(2, min(4, len(ani) // 6)) if len(hands) > 3 else max(1, len(hands) - 1)
            pen, rest = hands[:k], hands[k:]
            ...
            for ii, i in enumerate(pen):
                ...
                routes[i] = seq
            # field crew: stable index arcs (fan-out from center, no reassign)
            if fld and rest:
                ...
                for jj, i in enumerate(rest):
                    rem = sorted(arcs[jj], key=lambda t: _d(units[i], t[1]))
                    seq = []
                    if any(t[2] == "FERTILIZE" for t in rem) and shed.get("FERTILIZER", 0) > 0:
                        seq.append(("PICKUPF", (4, 4)))
                    seq += [(t[2], tuple(t[1])) for t in rem]
                    routes[i] = seq
            for i in hands:
                routes.setdefault(i, [])
            # farmer: central logistics + spillover pen work
            fseq = []
            if ani and shed.get("WHEAT", 0) > 0:
                fseq.append(("PICKUPW", (4, 4)))
            fseq += [(t[2], tuple(t[1])) for t in ani[:3]]
            routes[0] = fseq
        st["v43"] = {"day": day, "routes": routes, "ptr": {}}
    v43 = st["v43"]
            ...
            for t in (field + logi):
                if not t[5] and tuple(t[1]) == tuple(pos) and t[2] == op and valid(t):
                    ...
                    if need and carrying[i].get(need, 0) <= 0 and op != "PICKUP":
                        ...
                        continue
                    return t
            if op in ("PICKUPW", "PICKUPF"):
                item = "WHEAT" if op == "PICKUPW" else "FERTILIZER"
                if tuple(u) in ((4,4),(5,4),(4,5),(5,5)):
                    for t in logi + field:
                        if not t[5] and t[2] == "PICKUP" and t[3] == item:
                            return t
                return None
            p += 1
            v43ptr[i] = p
