...
def agent(obs, config=None):
    ...
    # ============ v40 PLAN-THEN-COMMIT ============
    # Build full-day routes at dawn: arcs + NN chains (the offline-optimal
    # pattern). Units execute their route; re-plan only when route exhausts,
    # a task goes invalid (skip), or an URGENT deadline appears late-day.
    ...
    def _build_routes():
        openf = [t for t in field if not t[5]]
        ani = [t for t in openf if _is_ani(t)]
        fld = [t for t in openf if not _is_ani(t)]
        routes = {}
        if len(units) == 1:
            routes[0] = [t[1] for t in (logi + openf)]
            return routes
        # farmer: logistics (feed pickups etc.) + spillover
        routes[0] = [t[1] for t in logi[:6]]
        hands = list(range(1, len(units)))
        # pen crew: 2-4 nearest hands to pen centroid
        if ani:
            pcx = sum(t[1][0] for t in ani) / len(ani); pcy = sum(t[1][1] for t in ani) / len(ani)
            k = max(2, min(4, len(ani) // 6)) if len(hands) > 4 else max(1, len(hands) // 2)
            hs = sorted(hands, key=lambda i: _d(units[i], (pcx, pcy)))
            pen = set(hs[:k]); field_h = [i for i in hands if i not in pen]
            for i in hs[:k]:
                pos = units[i]
                rem = sorted(ani, key=lambda t: _d(pos, t[1]))
                routes[i] = [t[1] for t in rem]
        else:
            field_h = hands
        # field crew: angular arcs around field centroid, unit->arc by proximity, NN chains
        if fld:
            ...
            for i in field_h:
                ...
                pos = units[i]
                rem = sorted(arc, key=lambda t: _d(pos, t[1]))
                routes[i] = [t[1] for t in rem]
        for i in hands:
            routes.setdefault(i, [])
        return routes

    st.setdefault("day_plan", None)
    dp = st["day_plan"]
    if dp is None or dp.get("day") != day or dp.get("n_units") != len(units):
        dp = {"day": day, "n_units": len(units), "routes": _build_routes(), "ptr": {}}
        st["day_plan"] = dp
    routes, rptr = dp["routes"], dp.setdefault("ptr", {})

    def _next_route_task(i, u):
        rt = routes.get(i, [])
        p = rptr.get(i, 0)
        while p < len(rt):
            tgt = rt[p]
            # find live task at this position (any op) — task objects are rebuilt per step
            cands = [t for t in (field + logi) if not t[5] and tuple(t[1]) == tuple(tgt)]
            if cands:
                # prefer carrying-feasible; else keep for later
                ok = [t for t in cands if not (t[4] and carrying[i].get(t[4], 0) <= 0 and t[2] != "PICKUP")]
                if ok:
                    return ok[0]
                # task needs carrying we lack: defer (stay, fallback may pickup)
                return None
            p += 1
            rptr[i] = p
        return None
    ...
    # URGENT: unfed animals / danger-water late in the day override routes
    urgent = []
    if hour >= 14:
        for t in field:
            if t[5]: continue
            if t[2] == "FEED" or (t[2] == "WATER" and t[0] == 0):
                urgent.append(t)
