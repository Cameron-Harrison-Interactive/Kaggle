...
def agent(obs, config=None):
    ...
    if hour >= 4:
        ...
        for item, keep in (("WHEAT", wheat_keep), ("CARROT", 0), ("EGG", 0),
            ...
            if q > 0:
                market.append(["SELL", item, q])
    # blueprint hire ramp: hands are FIRED nightly -> rehire every day.
    # $150k economies run 9-14 hands/day; we were capped at 4.
    if hour <= 3 or n_hands == 0:
        _ht = int(farm.get("hires_today") or 0)
        _ramp = 4 if day < 2 else 7 if day < 6 else 10 if day < 12 else 14
        _gap = _ramp - _ht
        _room = 10 - len(market)
        if _gap > 0 and money > 80 and _room > 0:
            for _ in range(min(_gap, _room if hour >= 1 else 3)):
                market.append(["HIRE"])
