...
def agent(obs, config=None):
    ...
    if hour <= 1:
        ...
        if day >= 12 and n_tiles >= 40 and farm["money"] >= 3000: want = 14
        if day >= 27: want = 6
        have = len(farm["hands"])
    ...
    # ============ v42 TAPE ENGINE: checkpoint route planning ============
    # Every CHECKPOINT hour (0,6,12,18) build full routes from live state:
    #   1) carrying-feasible chains (wheat pickups precede feeds)
    #   2) pen crew (nearest units) services animals; field crew gets arcs
    #   3) NN ordering within each route
    # Between checkpoints units execute their route; urgent interrupts
    # (unfed h>=16, danger-water h>=18) override. Route task = live task
    # objects matched by (pos, op) so intraday state is always respected.
    ANI_OPS = {"FEED", "CARE", "COLLECT_FERTILIZER"}
    def _tile_at(pos):
        x, y = pos
        ...
        return None
    def _is_ani(t):
        if t[2] in ANI_OPS: return True
        if t[2] == "HARVEST":
            tl = _tile_at(t[1])
            return isinstance(tl, dict) and "animal" in tl
        return False
    def _shedtile(u):
        return min(((4,4),(5,4),(4,5),(5,5)), key=lambda c: _d(u, c))

    def _build_routes():
        openf = [t for t in (field + logi) if not t[5]]
        ani = [t for t in openf if _is_ani(t)]
        fld = [t for t in openf if not _is_ani(t)]
        routes = {}
        if len(units) == 1:
            routes[0] = [(t[2], tuple(t[1])) for t in (logi + openf)]
            return routes
        # ---- farmer: logistics master (pickups + deposits + feed support)
        fseq = []
        # pickup wheat if feeds exist and shed has wheat
        n_feed = sum(1 for t in ani if t[2] == "FEED")
        if n_feed and shed.get("WHEAT", 0) > 0:
            fseq.append(("PICKUPW", _shedtile(units[0])))
        fseq += [(t[2], tuple(t[1])) for t in ani[: max(4, n_feed // 2)]]
        routes[0] = fseq
        # ---- pen crew: 2-4 hands nearest the pen; they pick up wheat first
        hands = list(range(1, len(units)))
        pen = []
        if ani:
            pcx = sum(t[1][0] for t in ani) / len(ani); pcy = sum(t[1][1] for t in ani) / len(ani)
            k = max(2, min(4, (len(ani) + 5) // 8)) if len(hands) > 4 else max(1, len(hands) // 2)
            hs = sorted(hands, key=lambda i: _d(units[i], (pcx, pcy)))
            pen = hs[:k]
            per = max(1, (len(ani) + k - 1) // k)
            for ii, i in enumerate(pen):
                my = sorted(ani[ii * per:(ii + 1) * per], key=lambda t: _d(units[i], t[1]))
                seq = []
                if any(t[2] == "FEED" for t in my) and shed.get("WHEAT", 0) > 0:
                    seq.append(("PICKUPW", _shedtile(units[i])))
                seq += [(t[2], tuple(t[1])) for t in my]
                routes[i] = seq
        field_h = [i for i in hands if i not in pen]
        # ---- field crew: angular arcs + NN
        if fld:
            cx = sum(t[1][0] for t in fld) / len(fld); cy = sum(t[1][1] for t in fld) / len(fld)
            order = sorted(fld, key=lambda t: math.atan2(t[1][1] - cy, t[1][0] - cx))
            k = max(1, len(field_h))
            arcs = [order[j::k] for j in range(k)]
            acen = [(sum(t[1][0] for t in a) / max(1, len(a)), sum(t[1][1] for t in a) / max(1, len(a))) for a in arcs]
            pairs = sorted((_d(units[i], acen[j]), i, j) for i in field_h for j in range(k))
            tu, tj, umap = set(), set(), {}
            for dd, i, j in pairs:
                if i in tu or j in tj: continue
                tu.add(i); tj.add(j); umap[i] = j
            for i in field_h:
                j = umap.get(i)
                arc = arcs[j] if j is not None and j < len(arcs) else fld
                rem = sorted(arc, key=lambda t: _d(units[i], t[1]))
                routes[i] = [(t[2], tuple(t[1])) for t in rem]
        for i in hands:
            routes.setdefault(i, [])
        return routes

    CHECKPOINTS = {0, 6, 12, 18}
    st.setdefault("v42", {})
    v42 = st["v42"]
    if hour in CHECKPOINTS or v42.get("sig") != (day, len(units), hour // 6):
        v42["routes"] = _build_routes()
        v42["ptr"] = {}
        v42["sig"] = (day, len(units), hour // 6)
    v42routes, v42ptr = v42["routes"], v42.setdefault("ptr", {})
    ...
    def _route_next(i, u):
        ...
        while p < len(rt):
            ...
            # live task at (pos, op)?
            for t in (field + logi):
                if not t[5] and tuple(t[1]) == tuple(pos) and t[2] == op and valid(t):
                    need = t[4]
                    if need and carrying[i].get(need, 0) <= 0 and op != "PICKUP":
                        return None  # carrying gap: checkpoint will fix
                    return t
            # special: PICKUPW pseudo-op
            if op == "PICKUPW" and _is_shed(u) and shed.get("WHEAT", 0) > 0 and carrying[i].get("WHEAT", 0) <= 0:
                for t in logi + field:
                    if not t[5] and t[2] == "PICKUP" and t[3] == "WHEAT" and tuple(t[1]) == tuple(pos):
                        return t
            p += 1
            v42ptr[i] = p
        return None
    def _is_shed(u):
        return tuple(u) in ((4,4),(5,4),(4,5),(5,5))

    # urgent set computed once per step
    urgent = [t for t in field if not t[5] and ((t[2] == "FEED" and hour >= 16) or (t[2] == "WATER" and t[0] == 0 and hour >= 18))]
    ...
    for i, u in enumerate(units):
        ...
        if lk is not None:
            ...
            if chosen is None:
                locks.pop(i, None)
        if chosen is None and urgent:
            cu = [t for t in urgent if not t[5] and (not t[4] or carrying[i].get(t[4], 0) > 0)]
            if cu:
                chosen = min(cu, key=lambda t: _d(u, t[1]))
                urgent.remove(chosen)
        if chosen is None:
            chosen = _route_next(i, u)
        if chosen is None:
            if i == 0:
