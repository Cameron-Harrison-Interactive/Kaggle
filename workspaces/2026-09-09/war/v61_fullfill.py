...
def plan_market(S, day):
    ...
    if pxw >= 36:
        # FULL-FILL (user directive): we open all 4 quads, the meta line never
        # opens the 4th — plant to capacity, win the volume war at the floor
        if day == 0:
            targets = {"WHEAT": 6}
        ...
        targets = {"WHEAT": 18 if n_an >= 4 else (8 if day == 0 else 12)}
    if day <= 1:
        targets["MELON"] = 14 if pxm >= 150 else 0   # wave 1
    elif 10 <= day <= 17 and pxm >= 150:
        targets["MELON"] = 14   # wave-2 rotation; d17 = last sow that ripens by d29
    if 3 <= day <= 19:
        base_s = (42 if pxs >= 150 else 28) if day >= 10 else (24 if pxs >= 120 else 18)
    ...
    if 18 <= day <= 26:
        # FULL FILL: every empty tile plants; carrot overflow only while its
        # price holds (gate self-disarms once our own flood crashes it)
        targets["WHEAT"] = max(targets.get("WHEAT", 0), 40)
        if px.get("CARROT", 35) >= 28:
            room = n_empty + 10
            targets["CARROT"] = max(targets.get("CARROT", 0),
                                    min(24, max(0, room - targets.get("WHEAT", 0))))
    ...
    pipe_cap = {"STRAWBERRY": 44, "TOMATO": 10}
    for crop in order:
        ...
        if CROPS[crop]["ongoing"]:
            ...
            if crop == "WHEAT" and 15 <= day <= 19 and money >= 2000:
                gap += 25   # stockpile: late-book insurance for the d20-27 fill
