...
def plan_market(S, day):
    ...
    # ramp from ZERO — never spend the day-0/1 seed budget on animals
    goose_t = min(8, day - 1) if 2 <= day <= 9 else (8 if day > 9 else 0)
    cow_t = min(6, day - 2) if 3 <= day <= 8 else (6 if day > 8 else 0)
    sheep_t = min(6, day - 4) if 5 <= day <= 10 else (6 if day > 10 else 0)
...
def plan_day(S, day):
    ...
    for i in range(n_units):
        q = min(owned_q,
                key=lambda qq: (counts[qq] >= 3, counts[qq] / (1 + load[qq]), owned_q.index(qq)))
        assign[i] = q
        counts[q] += 1
    ...
    room_total = max(0, crew_cap - cur_planted)
