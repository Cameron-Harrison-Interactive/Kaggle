...
def plan_market(S, day):
    ...
    if 18 <= day <= 26:
        # war fillers: every empty tile earns. carrot = 3-day cycle (sown d26
        # still harvests d29); wheat = feed + flood volume through d26.
        targets["CARROT"] = 26
...
def plan_day(S, day):
    ...
    if field:
        ...
        n_planters = 2 if n_units >= 10 else (1 if n_units >= 6 else 0)
        planters = set(units_by_c[-n_planters:]) if n_planters else set()
        avail = [i for i in range(1, n_units)
                 if i not in (servers if ani else []) and i not in planters]
        if not avail:
