...
def plan_market(S, day):
    ...
    # main target = wheat: flood wheat-heavy enemies any day, and EVERYONE
    # from d16 (their late-game wheat scarcity premium is the profit engine)
    ...
    # ---- 3.2) SEED BUFFER: pre-stock tomorrow's plantings so tiles never
    # wait on seeds (wheat ~10/day churn, strb ~4/day treadmill)
    if day <= 25 and money >= 800:
        buf = []
        if seeds.get("WHEAT", 0) < 12:
            buf.append(("WHEAT", min(12 - seeds.get("WHEAT", 0), 12)))
        if seeds.get("STRAWBERRY", 0) < 6:
            buf.append(("STRAWBERRY", min(6 - seeds.get("STRAWBERRY", 0), 6)))
        cost_b = sum(CROPS[c]["seed"] * n for c, n in buf)
        if cost_b and money - cost_b >= 400:
            for c, n in buf:
                idx = emit(["BUY_SEED", c, n])
                arrive("seeds", c, n, idx)
                money -= CROPS[c]["seed"] * n
                seeds[c] = seeds.get(c, 0) + n

    # ---- 4) ANIMALS (money left after feed/crew/seeds) ----
    # WAR ANIMAL RAMP: animals are the early money engine (eggs d2+, wool d6+,
    # milk d8+) and the fertilizer source. Fixed ladder, safe cash gates below.
    sheep_t = min(5, max(0, day - 1)) if day <= 7 else 5
    cow_t = min(9, max(1, day)) if day <= 9 else 9
    tgt = {"GOOSE": (3 if day >= 2 else 0),
           "COW": cow_t,
           "SHEEP": sheep_t}
    have = {}
    for a in animals:
        ...
        while ptr[i] < len(rt):
            ...
            if kind == "HARVESTA":
                ...
                if u == pos:
                    ...
                    # WAR CAP: total harvests today stay under the shed limit
...
def agent(obs, config=None):
    ...
    # Shed drains hourly -> nothing overflows at EOD, and every unit we
    # harvest converts to cash the same hour at the best remaining price.
    if hour >= 1:
        farm_ = obs["farms"][p]
        shed_ = dict((obs.get("private", {}) or {}).get("shed", {}) or {})
        n_an_ = 0
        for row_ in farm_.get("tiles", []):
            for t_ in row_:
        ...
        if shed_.get("FERTILIZER", 0) > 8:
            extra.append(["SELL", "FERTILIZER", shed_["FERTILIZER"] - 8])
        nw = shed_.get("WHEAT", 0) - keep_
        if nw > 0:
            extra.append(["SELL", "WHEAT", nw])
        for o in extra:
            if len(mkt) >= 10:
                break
            mkt.append(o)
