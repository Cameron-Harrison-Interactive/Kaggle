...
def plan_day(S, day):
    ...
    # ---- field ZONES (the football play): every hand OWNS a quadrant —
    # stable geography day over day, load-balanced within the zone, idle
    # hands cover the heaviest zone. Replaces angular wedges that re-sliced
    # the whole farm whenever crew size or crop mix shifted. ----
    ...
    if field:
        ...
        zones = {}
        for t in field:
            zones.setdefault(zone_of(t[1]), []).append(t)
        znames = sorted(zones, key=lambda z: (-len(zones[z]), z))   # heaviest first
        K = len(avail)
        # proportional hand allocation (largest remainder), min 1 per zone
        tot = max(1, len(field))
        alloc = {z: K * len(zones[z]) // tot for z in znames}
        used = sum(alloc.values())
        for z in sorted(znames, key=lambda z: -(K * len(zones[z]) / tot - alloc[z])):
            if used >= K:
                break
            if alloc[z] < len(zones[z]):
                alloc[z] += 1
                used += 1
        while used > K:
            z = min(znames, key=lambda z: (alloc[z] <= 1, len(zones[z]) * 1000 - alloc[z]))
            if alloc[z] <= 1:
                break
            alloc[z] -= 1
            used -= 1
        # hands -> zones in STABLE index order onto heaviest zones first
        hand_zone = {}
        zi = 0
        for i in avail:
            while zi < len(znames) and alloc[znames[zi]] <= 0:
                zi += 1
            if zi >= len(znames):
                break
            hand_zone[i] = znames[zi]
            alloc[znames[zi]] -= 1
        for i in avail:   # spare hands cover the heaviest zone
            if i not in hand_zone:
                hand_zone[i] = znames[0]
        zone_hands = {}
        for i, z in hand_zone.items():
            zone_hands.setdefault(z, []).append(i)
        for z, hands in zone_hands.items():
            ts = zones.get(z, [])
            if not ts:
                continue
            cx = sum(t[1][0] for t in ts) / len(ts)
            cy = sum(t[1][1] for t in ts) / len(ts)
            ts_sorted = sorted(ts, key=lambda t: math.atan2(t[1][1] - cy, t[1][0] - cx))
            n_t = len(ts_sorted)
            for j, i in enumerate(hands):
                wedge = ts_sorted[j * n_t // len(hands):(j + 1) * n_t // len(hands)]
                if not wedge:
                    continue
