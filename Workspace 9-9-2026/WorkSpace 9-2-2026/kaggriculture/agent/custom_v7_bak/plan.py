...
# NE (y 0..4, x 5..9): row 0 MELON (plant D1, harvest D13, replant to D25),
# row 1 STRAWBERRY (unlocks NE D0, plant D1 = full 4 productions D11-D17),
# row 2 CROP wheat (feed), row 3 PASTURE, row 4 COOP.
NE_LAYOUT = {}
for y in range(5):
    for x in range(5, 10):
        if (x, y) in SHED_SET:
...
# SW (y 5..9, x 0..4): row 5 STRAWBERRY, rows 6+8 MELON, rows 7+9 CROP (wheat)
SW_LAYOUT = {}
for y in range(5, 10):
    for x in range(5):
        if (x, y) in SHED_SET:
...
# SE (y 5..9, x 5..9): row 5 STRAWBERRY, rows 6+8 MELON, rows 7+9 CROP
SE_LAYOUT = {}
for y in range(5, 10):
    for x in range(5, 10):
        if (x, y) in SHED_SET:
