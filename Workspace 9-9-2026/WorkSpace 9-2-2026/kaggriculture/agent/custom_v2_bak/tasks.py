...
def generate_tasks(board):
    ...
    for (x, y) in board.animals():
        ...
        # CARE: only worth doing if we can also feed today
        # (care banks a bonus consumed on next production day if fed)
        if not t["cared_today"] and t["fed_today"]:
            tasks.append(CareTask((x, y)))

    # ---------- plants (water, harvest) ----------
    for (x, y) in board.plants():
        ...
        if not t["watered_today"]:
            ...
            # In-window watering must OUT-priority pre-emptive harvest (78) so
            # we always squeeze the last +1 yield before pulling. Otherwise
            # we routinely harvest yield=3 wheat on day 4 without watering it
            # to 4 first.
            if urgent:
                pri = 85
            elif in_window:
                pri = 82   # > harvest_pre_decay (78), < feed (90)
            else:
                pri = 40
            tasks.append(WaterTask((x, y), priority=pri))

        # HARVEST when ripe. For one-time crops we WAIT for yield to fill
        # (or the plant to hit its last day) — harvesting at yield=1 is a
        # catastrophic bug (throws away 5 free units).
        if t["yield_units"] > 0:
            if not cd["ongoing"]:
                yu = t["yield_units"]
                max_y = cd["max_yield"]
                # Without fertilizer, MAX attainable yield is 1 + #window_days.
                # window = ceil(max_day/2) .. max_day, so window_len = max_day - ws + 1.
                ws = (cd["max_day"] + 1) // 2
                plain_max = 1 + (cd["max_day"] - ws + 1)  # e.g. WHEAT=4, CARROT=3, MELON=4
                # If plant was fertilized this life we can reach cd["max_yield"].
                ...
                if age >= cd["first"]:
                    if yu >= target or age >= last_water_day:
                        # Higher pri when about to decay past window
                        pri = 78 if age >= last_water_day and yu < target else 70
                        tasks.append(HarvestTask((x, y), priority=pri))
            else:
                # ongoing: harvest whenever yield gets close to max_held to
                # free capacity for the next production. Also harvest late in
                # game to bank value before end.
                if age >= cd["first"] and t["yield_units"] >= max(2, cd["max_yield"] - 1):
                    tasks.append(HarvestTask((x, y), priority=65))
...
def generate_planting_tasks(board):
    """Plant `desired_crop_for(x,y)` on each empty CROP-role tile,
    respecting the seed count we own.

    Priority scales with tile REMOTENESS: distant tiles get bumped so they
    don't perpetually lose to nearby WATER/HARVEST tasks. Without this, hands
    grind on the NW field forever and SE tiles are never planted."""
    from .plan import full_layout, desired_crop_for
    from .board import SHED_TILES
    from .pathing import nearest as _nearest
    tasks = []
    layout = full_layout()
    ...
    for crop, tiles in per_crop.items():
        # NW-first, so the seed budget goes to the closest tiles first
        tiles.sort(key=lambda p: (p[1], p[0]))
        have = board.seeds.get(crop, 0)
        for i, tile in enumerate(tiles):
            if i >= have:
                break
            tasks.append(PlantTask(tile, crop))
    return tasks
