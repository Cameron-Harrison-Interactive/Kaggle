"""Fixed public Tran H Hoang policy from episode 89674601, seat 0."""
import copy
...
_V27_ANCHOR_AGENT = agent

_V27_CASHFLOW_MODE = 'collision_front'
_V27_TERMINAL_RELAY = False
_V27_TERMINAL_START = 716
...
_V27_PROFILE_ACTIVE = False


def _v27_get(obj, key, default=None):
...
def _v27_prices(obs):
    market = _v27_get(obs, "market", {}) or {}
    return _v27_get(market, "prices", {}) or {}


def _v27_market_inventory(obs):
    market = _v27_get(obs, "market", {}) or {}
    return _v27_get(market, "inventory", {}) or {}


def _v27_private(obs):
    return _v27_get(obs, "private", {}) or {}


def _v27_farm(obs):
    farms = _v27_get(obs, "farms", []) or []
    player = int(_v27_get(obs, "player", 0) or 0)
    return farms[player] if 0 <= player < len(farms) else {}


def _v27_profile_counts(obs):
    farms = _v27_get(obs, "farms", []) or []
    player = int(_v27_get(obs, "player", 0) or 0)
    opponent = 1 - player
    counts = {}
    ...
    for row in (_v27_get(farms[opponent], "tiles", []) or []):
        for tile in row or []:
            if not isinstance(tile, dict):
                continue
            key = tile.get("animal") or tile.get("crop")
...
def _v27_update_profile(obs, step):
    global _V27_PROFILE_ACTIVE
    ...
    counts = _v27_profile_counts(obs)
    _V27_PROFILE_ACTIVE = bool(
        counts.get("WHEAT", 0) >= 5
        and counts.get("STRAWBERRY", 0) >= 26
        and counts.get("MELON", 0) == 6
        and counts.get("COW", 0) >= 8
...
def _v27_exposure(obs):
    ...
    return {
        ...
        "CARROT": counts.get("CARROT", 0),
        "TOMATO": counts.get("TOMATO", 0),
        "STRAWBERRY": counts.get("STRAWBERRY", 0),
        "MELON": counts.get("MELON", 0),
        "EGG": counts.get("GOOSE", 0),
        "MILK": counts.get("COW", 0),
        "WOOL": counts.get("SHEEP", 0),
        "FERTILIZER": animal_total,
    }


def _v27_sell_score(order, obs):
    if not (
    ...
    prices = _v27_prices(obs)
    price = float(prices.get(item, _V27_BASE_PRICE.get(item, 0)) or 0)
    exposure = float(_v27_exposure(obs).get(item, 0))
    glut = float(_V27_GLUT.get(item, 0.5))

    capped_notional = price * min(quantity, 12)
    collision_risk = price * min(quantity, 8) * exposure * glut
    urgency = price * quantity * glut

    return (
        capped_notional
        + 0.075 * collision_risk
        + 0.10 * urgency
    )


def _v27_cashflow(action, obs, step):
    if (
        _V27_CASHFLOW_MODE == "anchor"
        or not _V27_PROFILE_ACTIVE
        or not (300 <= step < 715)
    ):
        return action
    ...
    sells.sort(
        key=lambda order: _v27_sell_score(order, obs),
        reverse=True,
    )

    if _V27_CASHFLOW_MODE == "collision_slots":
        for index, order in zip(sell_indices, sells):
            market[index] = order

    elif _V27_CASHFLOW_MODE == "collision_front":
        non_sells = [
            order
            for order in market
            if not (
        ...
        market = sells + non_sells

    elif _V27_CASHFLOW_MODE == "static_slots":
        priority = {
            "WOOL": 0,
            "MELON": 1,
            "MILK": 2,
        ...
        sells.sort(
            key=lambda order: priority.get(
                order[1] if len(order) > 1 else "",
                99,
            )
        )
        for index, order in zip(sell_indices, sells):
...
def _v27_positions(farm):
    positions = [
        list(_v27_get(farm, "farmer", [0, 0]) or [0, 0])
    ]
    positions.extend(
        list(position)
        for position in (_v27_get(farm, "hands", []) or [])
    )
    return positions


def _v27_inventories(obs):
    private = _v27_private(obs)
    return _v27_get(private, "inventories", []) or []


def _v27_shed(obs):
    private = _v27_private(obs)
    return _v27_get(private, "shed", {}) or {}


def _v27_access_tiles(farm):
    tiles = _v27_get(farm, "tiles", []) or []
...
def _v27_is_adjacent(position, access_tiles):
    return tuple(position) in set(access_tiles)


def _v27_inventory_products(inventory):
    return {
        item: max(0, int(quantity or 0))
        for item, quantity in (inventory or {}).items()
        if item in _V27_PRODUCTS and int(quantity or 0) > 0
    }


def _v27_best_item(inventory, prices):
    products = _v27_inventory_products(inventory)
    choices = [
        (
            int(prices.get(item, _V27_BASE_PRICE[item]) or 0)
            * quantity
            * (1.0 + _V27_GLUT.get(item, 0.5)),
            int(prices.get(item, _V27_BASE_PRICE[item]) or 0),
            quantity,
            item,
        )
...
def _v27_move_to_access(farm, position, access_tiles):
    if not access_tiles:
        return None
    ...
    for op, dx, dy in moves:
        ...
        distance = min(
            abs(nx - ax) + abs(ny - ay)
            for ax, ay in access_tiles
        )
        choices.append((distance, op))
...
def _v27_terminal_relay(action, obs, step):
    if (
        not _V27_TERMINAL_RELAY
        or step < _V27_TERMINAL_START
        or step > 718
    ):
        return action

    farm = _v27_farm(obs)
    positions = _v27_positions(farm)
    inventories = _v27_inventories(obs)
    shed = _v27_shed(obs)
    prices = _v27_prices(obs)
    access_tiles = _v27_access_tiles(farm)

    actor_actions = [
        list(action.get("farmer", ["PASS"]) or ["PASS"])
    ]
    actor_actions.extend(
        list(hand_action or ["PASS"])
        for hand_action in (action.get("hands", []) or [])
    )

    while len(actor_actions) < len(positions):
        actor_actions.append(["PASS"])
    actor_actions = actor_actions[: len(positions)]

    shed_total = sum(
        max(0, int(quantity or 0))
        for quantity in shed.values()
    )
    room = max(0, 100 - shed_total)
    projected = {}
    ...
    for actor, position in enumerate(positions):
        ...
        if _v27_is_adjacent(position, access_tiles):
            ...
            if (
                product_total == inventory_total
                and product_total <= room
            ):
                actor_actions[actor] = ["DROP"]
                for item, quantity in products.items():
                    projected[item] = (
                        projected.get(item, 0) + quantity
                    )
                room -= product_total
            else:
                choice = _v27_best_item(inventory, prices)
                if choice is not None and room > 0:
            ...
            requested = actor_actions[actor]
            guaranteed_noop = (
                not requested
                or requested[0] in ("PASS", "PLACE", "DROP")
            )

            if (
                remaining_moves > 0
                and distance <= remaining_moves
                and guaranteed_noop
            ):
                move = _v27_move_to_access(
                    farm,
                    position,
                    access_tiles,
                )
                if move is not None:
                    actor_actions[actor] = move

    action["farmer"] = actor_actions[0]
    action["hands"] = actor_actions[1:]

    totals = {}
    for item in _V27_PRODUCTS:
        quantity = (
            max(0, int(shed.get(item, 0) or 0))
            + projected.get(item, 0)
        )
        if quantity > 0:
            totals[item] = quantity

    exposure = _v27_exposure(obs)
    ordered = sorted(
        totals,
        key=lambda item: (
            float(prices.get(item, _V27_BASE_PRICE[item]) or 0)
            * totals[item]
            * (
                1.0
                + 0.05
                * exposure.get(item, 0)
                * _V27_GLUT.get(item, 0.5)
            ),
            float(prices.get(item, _V27_BASE_PRICE[item]) or 0),
            totals[item],
        ),
...
def agent(obs, config=None):
    step = max(
        0,
        min(
            int(_v27_get(obs, "step", 0) or 0),
            len(TRACE_ACTIONS) - 1,
        ),
    )

    _v27_update_profile(obs, step)

    if _V27_CASHFLOW_MODE == "anchor":
        action = _V27_ANCHOR_AGENT(obs, config)
    else:
        action = copy.deepcopy(
            _TRAN_CASHFLOW_BASE(obs, config)
        )
        action = _v27_cashflow(action, obs, step)

    action = _v27_terminal_relay(
