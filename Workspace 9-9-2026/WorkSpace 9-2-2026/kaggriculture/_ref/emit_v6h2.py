"""Emit topbots/v6h2.py: v4_fr1 skeleton + fitted ramp + fitted tape (pure replay)."""
import ast
...
os.environ["TAPE_OUT"] = ""
mod = importlib.util.spec_from_file_location("auth_emit2", "_ref/tape_author3.py")
...
io.open("topbots/v6h2.py", "w", encoding="utf-8").write(bot)

v6h_src = io.open("topbots/v6h2.py", encoding="utf-8").read()
vt = json.loads(v6h_src.split(MARK)[1].split(Q)[0])
assert vt == tape, "round-trip mismatch"
ast.parse(v6h_src)
print("v6h2.py written:", os.path.getsize("topbots/v6h2.py"), "bytes, parses OK")
