...
def agent(obs, config=None):
    ...
    target_cows = 6 if milk_px >= 90 else 4
    target_geese = 0
    wool_px = price_of("WOOL", minv["WOOL"]) if "WOOL" in minv else 200
    wool_flood = minv.get("WOOL", 10000) - 10000
    target_sheep = 0 if wool_flood > 10 else (5 if wool_px >= 90 else 3)
    ...
    if hour <= 1:
        ...
        if day <= 22 and total_animals < 18:
            ...
            if 2 <= day <= 10 and animals.get("SHEEP", 0) + shed.get("SHEEP", 0) < target_sheep:
                plan.append(("SHEEP", min(2, target_sheep - animals.get("SHEEP", 0) - shed.get("SHEEP", 0))))
            for aname, n in plan:
                cost = ANIMALS[aname]["cost"] * n
                cushion = 600 if day < 10 else 250
                if money - cost >= cushion + n_animals * 20 and (aname != "COW" or animals.get("COW", 0) + shed.get("COW", 0) >= 2 or day >= 3):
                    o_animal.append(["BUY_ANIMAL", aname, n]); money -= cost
        # LAND: tiles are the engine
        if len(farm["unlocked_quadrants"]) == 1 and day >= 5 and money >= 1400:
            o_land.append(["BUY_LAND"]); money -= 1000
        elif len(farm["unlocked_quadrants"]) == 2 and day >= 10 and money >= 2800:
        ...
        if hour == 0:
            W = {0: [("WHEAT", 8), ("MELON", 12)],
                 1: [("WHEAT", 6)], 2: [("WHEAT", 4)], 3: [("WHEAT", 4)],
                 4: [("WHEAT", 4), ("STRAWBERRY", 4)],
                 5: [("WHEAT", 3), ("STRAWBERRY", 8)],
                 6: [("WHEAT", 3), ("STRAWBERRY", 8)],
                 7: [("WHEAT", 3), ("STRAWBERRY", 4)],
                 8: [("WHEAT", 3), ("STRAWBERRY", 4)],
                 11: [("MELON", 8)]}
            for d in range(9, 24):          # strb treadmill: 3/day continuous replant
                W.setdefault(d, []).append(("STRAWBERRY", 3))
            for d in (16, 17, 18, 19, 20, 21, 22):   # tomato late + small
                W.setdefault(d, []).append(("TOMATO", 3))
            for d in (22, 23, 24, 25, 26):           # carrot late waves (deficit play)
                W.setdefault(d, []).append(("CARROT", 6))
    ...
    if day <= 25:
        want_crops = []
        m_target = 12 if day < 4 else (8 if 10 <= day <= 13 else 0)
        if crop_count("MELON") < m_target and seeds.get("MELON", 0) > 0: want_crops.append("MELON")
        if seeds.get("CARROT", 0) > 0 and day >= 21 and productive("CARROT") < 8: want_crops.append("CARROT")
        w_target = 10
        ...
        s_target = 20 if (day >= 6 and spx >= 110 and strb_flood <= 20) else 0
