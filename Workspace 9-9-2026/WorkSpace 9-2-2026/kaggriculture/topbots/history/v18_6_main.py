...
VERSION = "HI_AgriBot_v18.6_ClonePreempt"
import base64
import copy
import json
import math
...
# ============================================================================
# CLONE PREEMPTION (ported from rayk/kaito THUNDER #1 trajectory)
# Market-only, route-safe: sell premium 1-3 turns BEFORE the tape's own
# planned sale when a clone opponent is detected, then repay later.
# Legal per ROUTING_BIBLE: "buy-sell timing" adaptation.
# ============================================================================
...
_ADAPT_MIN_EVENTS = 2
_RACE_STATE = {0: {}, 1: {}}
_SHIFT_STATE = {0: {"last_step": -1, "debts": {}}, 1: {"last_step": -1, "debts": {}}}


def _own_actions(obs):
    return _SEAT1_ACTIONS if _seat(obs) == 1 else _SEAT0_ACTIONS


def _planned_premium(obs, step, item):
    actions = _own_actions(obs)
...
def _observe_opponent_market(obs, step):
    ...
    if previous and prev_step == step - 1 and _clone_distance(obs) <= _PREEMPT_MAX_CLONE_DISTANCE:
        ...
        for item in _PREMIUM:
            ...
            inferred -= _planned_premium(obs, prev_step, item)
...
def _projected_shed(obs, action):
    ...
    access = set(_shed_access_tiles(len(tiles) or 10))
...
def _repay_shift(obs, action, step):
    ...
    due = {item: max(0, int(quantity)) for item, quantity in dict(debts.pop(step, {}) or {}).items()}
    if not due:
        return action
    market = []
...
def _future_sells(obs, step, horizon):
    actions = _own_actions(obs)
...
def _preempt_shift(obs, action, step):
    ...
    future = _future_sells(obs, step, horizon)
    if not future:
        return action
    market = list(action.get("market") or [])
    if len(market) >= 10:
