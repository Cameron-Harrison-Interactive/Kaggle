...
#   crash if oversold; FERTILIZER has NO town drain -> use it on crops, don't sell.
...
COOP_SPOTS = ((3, 4), (3, 5), (6, 4), (6, 5), (4, 3), (5, 3))
...
def agent(obs, config=None):
    ...
    BASE = {"WHEAT": 25, "CARROT": 35, "EGG": 50, "MILK": 160, "WOOL": 200,
    ...
    if hour >= 4:
        ...
        if shed_load > 88:
            ...
            for item in sorted(("EGG", "MILK", "WOOL", "CARROT", "WHEAT"),
    ...
    for i in range(n_units):
        ...
        # ---------- fert runners: last 2 units gather & spread free fert ----------
        is_fert_runner = (n_units >= 10 and i >= n_units - 2
                          and n_animals >= 6 and not dying_jobs)
        if is_fert_runner and act is None:
            if inv.get("FERTILIZER", 0) > 0 and fert_window:
                t = take_nearest(px, py, fert_window)
                act = (["FERTILIZE"] if (px, py) == t
                       else _gb_move(px, py, t[0], t[1]))
            else:
        ...
        if act is None:
            ...
            if weed_cand:
            ...
            elif (shed.get("FERTILIZER", 0) > 0 and fert_window
                    and my_carry == 0):
                if (px, py) in SHED_TILES:
                    act = ["PICKUP", "FERTILIZER", 4]
                else:
                    ts = min(SHED_TILES,
                             key=lambda s: abs(s[0] - px) + abs(s[1] - py))
                    act = _gb_move(px, py, ts[0], ts[1])
                ...
                if (len(plants_all) < p_max and planted_today < day_cap
                    ...
                    if cand:
                        t = take_nearest(px, py, cand)
                        claimed.add(t)
                        if (seeds.get("STRAWBERRY", 0) > 0 and 13 <= day <= 19
