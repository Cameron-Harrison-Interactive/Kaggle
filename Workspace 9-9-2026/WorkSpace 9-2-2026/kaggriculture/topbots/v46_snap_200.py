...
def plan_market(S, day):
    ...
    if day <= 1:
        targets["MELON"] = 12 if pxm >= 150 else 0   # wave 1 only
    else:
        targets["MELON"] = 0
    if 3 <= day <= 19:
        base_s = (26 if pxs >= 150 else 20) if day >= 10 else (20 if pxs >= 150 else 16)
    ...
    pipe_cap = {"STRAWBERRY": 28, "TOMATO": 10}
    for crop in order:
    ...
    if day >= 10:
        qsell("MELON", min(15, shed.get("MELON", 0)))
    ...
    tgt = {"GOOSE": (3 if day >= 4 else 0) if px.get("EGG", 50) >= 45 else 0,
           "COW": 9 if day >= 12 else (8 if day >= 10 else (6 if day >= 8 else (4 if day >= 6 else 2))),
           "SHEEP": sheep_t}
    have = {}
    for a in animals:
