...
# ---------------------------------------------------------------------------
# v24 market brain (TT router + price-aware liquidation).
# The router replays tapes blindly: its sells fire at recorded hours regardless
# of price, and its twin army (30+ votes) plays identical lines -> the 5 TIE
# games. This overlay adds price-triggered sells on the SAME labor (farmer/
# hands verbatim, tape market orders kept, never removed/delayed):
#   - crashable tail (STRB/MILK/WOOL/MELON) sold into strength BEFORE the
#     mutual-dump floor (model: strb floors at 63u, milk 77u, wool 60u, melon 159u)
#   - eggs cleared early and often (uncrashable: >1500u to half price)
#   - carrot into strength (231u cliff - deepest of the crashables)
#   - wheat/fert untouched (the tape's own trading engine) except endgame clear
#   - shed-cap guard (no-drop law: >85 load sells down to 60)
#   - endgame (step>=700): liquidate everything (unsold = $0)
# Twin math: same production as the twin, same labor, but our stock sells
# earlier at better prices -> ties become wins.
# ---------------------------------------------------------------------------

_TT_AGENT = agent
_TRIG = {"STRAWBERRY": 150, "MILK": 120, "WOOL": 150, "MELON": 180, "CARROT": 30, "TOMATO": 55}
_TRIG_LATE = {"STRAWBERRY": (18, 30)}          # (day, price): past d18, strb goes at >=$30
_EGG_CLEAR = 5
_CAP_GUARD, _CAP_TARGET = 85, 60


def _mk_agent(obs, configuration=None):
    action = _TT_AGENT(obs, configuration)
    try:
        ...
        shed = (priv.get("shed") if hasattr(priv, "get") else None) or {}
        if not shed:
            return action
        m = list(action.get("market") or [])
        if len(m) >= 10:
            return action
        added = False
        ...
        def _sell(item, qty):
            nonlocal m, added
            if qty > 0 and len(m) < 10:
                m.append(["SELL", item, int(qty)])
                added = True

        # 1) eggs: clear early and often
        if int(shed.get("EGG", 0) or 0) >= _EGG_CLEAR:
            _sell("EGG", shed.get("EGG", 0))
        # 2) crashables into strength
        for item, trig in _TRIG.items():
            have = int(shed.get(item, 0) or 0)
            if have >= 4 and (prices.get(item) or 0) >= trig:
                _sell(item, have)
        # 3) late-game strb panic floor
        for item, (dmin, ptrig) in _TRIG_LATE.items():
            have = int(shed.get(item, 0) or 0)
            if day >= dmin and have >= 4 and (prices.get(item) or 0) >= ptrig:
                _sell(item, have)
        # 4) shed-cap guard (no-drop law)
        load = sum(v for v in shed.values() if isinstance(v, (int, float)))
        if load > _CAP_GUARD:
            for item, _ in sorted(shed.items(), key=lambda kv: -(prices.get(kv[0], 0) or 0) * (kv[1] or 0)):
                if load <= _CAP_TARGET or len(m) >= 10:
                    break
                have = int(shed.get(item, 0) or 0)
                if have > 0:
                    _sell(item, have)
                    load -= have
        # 5) endgame full liquidation
        if step >= 700:
            for item, _ in sorted(shed.items(), key=lambda kv: -(prices.get(kv[0], 0) or 0) * (kv[1] or 0)):
                have = int(shed.get(item, 0) or 0)
                if have > 0:
