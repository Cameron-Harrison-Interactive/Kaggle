...
def agent(obs, config=None):
    ...
    def best_task(pool, u, i, allowed=None):
        best, bs_ = None, None
        for t in pool:
            if t[5]: continue
    ...
    # v35 TASK-ARC TERRITORIES: computed ONCE per day (day-stable), cached in st.
    # Wedges are angular slices around the day's task-set centroid; each unit owns
    # the wedge nearest its day-start position. Task->wedge by angle each step.
    st.setdefault("arc_state", None)
    arcst = st["arc_state"]
    open_field = [t for t in field if not t[5]]
    if arcst is None or arcst.get("day") != day or arcst.get("n_units") != len(units) or arcst.get("ntasks") // 4 != len(open_field) // 4:
        if len(units) > 1 and len(open_field) >= 2:
            cx = sum(t[1][0] for t in open_field) / len(open_field)
            cy = sum(t[1][1] for t in open_field) / len(open_field)
            k = len(units)
            # unit -> wedge by proximity of unit to wedge's task-arc center (day start)
            angs = sorted(math.atan2(t[1][1] - cy, t[1][0] - cx) for t in open_field)
            wedges = []
            for j in range(k):
                lo = angs[int(j * len(angs) / k)]; hi = angs[int((j + 1) * len(angs) / k) - 1] if k > 1 else angs[-1]
                mid = angs[int((j + 0.5) * len(angs) / k)] if len(angs) > 0 else 0.0
                r = max(1.0, (sum((t[1][0]-cx)**2 + (t[1][1]-cy)**2 for t in open_field)/len(open_field)) ** 0.5)
                wedges.append((lo, hi, (cx + r * math.cos(mid), cy + r * math.sin(mid))))
            pairs = sorted(((_d(u, wedges[j][2]), i, j) for i, u in enumerate(units) for j in range(k)))
            taken_u, taken_j, umap = set(), set(), {}
            for dd, i, j in pairs:
                if i in taken_u or j in taken_j: continue
                taken_u.add(i); taken_j.add(j); umap[i] = j
            arcst = {"day": day, "n_units": len(units), "cx": cx, "cy": cy, "wedges": wedges, "umap": umap, "ntasks": len(open_field)}
        else:
            arcst = {"day": day, "n_units": len(units), "wedges": None, "umap": {}, "ntasks": len(open_field)}
        st["arc_state"] = arcst

    def wedge_of(pos):
        w = arcst["wedges"]
        if not w: return None
        a = math.atan2(pos[1] - arcst["cy"], pos[0] - arcst["cx"])
        for j, (lo, hi, c) in enumerate(w):
            if lo <= a <= hi or (lo > hi and (a >= lo or a <= hi)):
                return j
        # wrap-around: use nearest center
        return min(range(len(w)), key=lambda j: _d(pos, w[j][2]))
    ...
    # v35 TASK-ARC TERRITORIES (offline-optimal pattern, online):
    # partition today's open field tasks into |units| contiguous angular arcs
    # around the task centroid; each unit works the arc nearest to it.
    open_field = [t for t in field if not t[5]]
    arcs = None
    if len(units) > 1 and len(open_field) >= len(units):
        cx = sum(t[1][0] for t in open_field) / len(open_field)
        cy = sum(t[1][1] for t in open_field) / len(open_field)
        order = sorted(open_field, key=lambda t: math.atan2(t[1][1] - cy, t[1][0] - cx))
        k = len(units)
        arcs = [set(id(t) for t in order[j::k]) for j in range(k)]
        # arc centers for unit->arc matching
        centers = []
        for j in range(k):
            ax = sum(t[1][0] for t in order[j::k]) / max(1, len(order[j::k]))
            ay = sum(t[1][1] for t in order[j::k]) / max(1, len(order[j::k]))
            centers.append((ax, ay))
        # match units to arcs by proximity (greedy on distance, each arc once)
        pairs = sorted((( _d(u, centers[j]), i, j) for i, u in enumerate(units) for j in range(k)))
        taken_u, taken_j, umap = set(), set(), {}
        for dd, i, j in pairs:
            if i in taken_u or j in taken_j: continue
            taken_u.add(i); taken_j.add(j); umap[i] = j
        for i in range(len(units)):
            if i not in umap: umap[i] = None
        arcs_by_unit = umap
    else:
        arcs_by_unit = {i: None for i in range(len(units))}
    ...
    for i, u in enumerate(units):
        ...
        if chosen is None:
            if i == 0:
                ...
                if wed:
                    j = arcst["umap"].get(i)
                    my = [t for t in field if not t[5] and wedge_of(t[1]) == j]
                    chosen = best_task(my, u, i)
                    if chosen is None:
                        for jj in sorted(range(len(wed)), key=lambda c: _d(u, wed[c][2])):
                            if jj == j: continue
                            other = [t for t in field if not t[5] and wedge_of(t[1]) == jj]
                            chosen = best_task(other, u, i)
                            if chosen is not None: break
                if chosen is None:
                    chosen = best_task(field, u, i)
        if chosen is None:
