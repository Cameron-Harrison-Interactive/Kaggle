...
def agent(obs, config=None):
    ...
    if st["v43"] is None or st["v43"].get("day") != day:
        ...
        if len(units) == 1:
            ...
            # pen crew: first 2-3 hands (they service animals near center,
            # picking up wheat on the way out)
            k = max(2, min(3, len(ani) // 5)) if len(hands) > 3 else max(1, len(hands) - 1)
            ...
            for ii, i in enumerate(pen):
                ...
                routes[i] = [(t[2], tuple(t[1])) for t in my]
