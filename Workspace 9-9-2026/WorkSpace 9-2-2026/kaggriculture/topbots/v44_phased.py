...
def agent(obs, config=None):
    ...
    # ============ v43 FULL-DAY PLAN (h0, complete information) ============
    # Daily reset guarantees: everyone at center, flags cleared, weeds only
    # spawn at EOD -> the h0 task set is COMPLETE for the day. Plan once:
    # stable index->arc territories, NN chains, feeders carry wheat from the
    # start, NO intraday deposits (EOD auto-drops), urgent interrupts only.
    ...
    st.setdefault("v43", None)
    # rebuild when the day changes OR the crew size changes (hires land after
    # h0 unit actions — at h0 itself the farm has ZERO hands after the reset)
    if st["v43"] is None or st["v43"].get("day") != day or st["v43"].get("nu") != len(units):
        routes = {}
        ...
        if len(units) == 1:
            ...
            # v44 PHASED DAY: the pen sits at the spawn point, so the whole
            # crew clears it first at ~0.3 walk/task, THEN fans out to field
            # arcs. (Dedicated-crew split made 8 units walk past unclaimed pen
            # work — measured -$15k.)
            hands = list(range(1, len(units)))
            n_all = len(units)
            feeders = [t for t in ani if t[2] == "FEED"]
            ani_sorted = sorted(ani, key=lambda t: (t[1][0] + t[1][1]))
            per = max(1, (len(ani_sorted) + n_all - 1) // n_all)
            # field arcs: angular wedges by unit index (fan-out from center)
            farcs = {}
            if fld:
                ...
                # CONTIGUOUS WEDGES (slices), not pinwheels: each unit sweeps
                # one wedge outward — no crossings through center
                n_f = len(order)
                arcs = [order[j * n_f // n_all:(j + 1) * n_f // n_all] for j in range(n_all)]
            else:
                arcs = [[] for _ in range(n_all)]
            for i in range(n_all):
                seq = []
                if feeders and shed.get("WHEAT", 0) > 0 and i > 0:
                    seq.append(("PICKUPW", (4, 4)))
                my = ani_sorted[i * per:(i + 1) * per]
                seq += [(t[2], tuple(t[1])) for t in my]
                rem = sorted(arcs[i], key=lambda t: _d(units[i], t[1]))
                if any(t[2] == "FERTILIZE" for t in rem) and shed.get("FERTILIZER", 0) > 0 and i > 0:
                    seq.append(("PICKUPF", (4, 4)))
                seq += [(t[2], tuple(t[1])) for t in rem]
                routes[i] = seq
        st["v43"] = {"day": day, "nu": len(units), "routes": routes, "ptr": {}}
    v43 = st["v43"]
    v43routes, v43ptr = v43["routes"], v43["ptr"]
        ...
        while p < len(rt):
            ...
            for t in (field + logi):
                if not t[5] and tuple(t[1]) == tuple(pos) and t[2] == op and valid(t):
                    need = t[4]
                    if need and carrying[i].get(need, 0) <= 0 and op != "PICKUP":
                        # carrying gap: fetch the item first, then come back
                        for pt in logi + field:
                            if not pt[5] and pt[2] == "PICKUP" and pt[3] == need:
                                return pt
                        p += 1
                        v43ptr[i] = p
                        continue
            ...
            if op == "PICKUPW":
                for t in logi + field:
                    if not t[5] and t[2] == "PICKUP" and t[3] == "WHEAT" and tuple(u) in ((4,4),(5,4),(4,5),(5,5)):
                        return t
                return None
            p += 1
            v43ptr[i] = p
