...
def agent(observation, configuration=None):
    ...
    day = t // 24
    # ---- v41 SELL BRAIN: taped labor + taped buys; sells go adaptive ----
    # Rationale (measured): router self-play crashes MILK/WOOL d16 and STRB to $12
    # because taped sells fire into any price. Hold through crashes, sell at
    # take-profit / d26+ dump / cash floor. Buys stay 100% taped -> farm state
    # evolves exactly as recorded -> no labor cascade.
    buys = [list(a) for a in source['market'] if a[0] != 'SELL']
    if day < 12:
        return {'farmer':farmer,'hands':hands,'market':[list(a) for a in source['market']]}
    try:
        mkt = observation['market']
        px = mkt['prices']
        ...
        for row in farm['tiles']:
            for tl in row:
                if isinstance(tl, dict) and 'animal' in tl:
                    n_an += 1
        orders = []
        TAKE = {'MILK': 280, 'STRAWBERRY': 210, 'WOOL': 210, 'MELON': 105,
                'EGG': 38, 'CARROT': 40, 'TOMATO': 70}
        if day >= 26:
            for it in ('MILK','STRAWBERRY','WOOL','MELON','EGG','CARROT','TOMATO','FERTILIZER'):
                q = shed.get(it, 0)
                if q > 0:
                    orders.append(['SELL', it, q])
            wq = shed.get('WHEAT', 0)
            keep = n_an if day < 29 else 0
            if wq > keep:
                orders.append(['SELL', 'WHEAT', wq - keep])
        else:
            # fert: crashes all game (100->29); move it daily
            if px.get('FERTILIZER', 0) >= 20 and shed.get('FERTILIZER', 0) > 0:
                orders.append(['SELL', 'FERTILIZER', shed['FERTILIZER']])
            for it, tp in TAKE.items():
                q = shed.get(it, 0)
                if q > 0 and px.get(it, 0) >= tp:
                    orders.append(['SELL', it, q])
            wq = shed.get('WHEAT', 0)
            if wq > n_an and px.get('WHEAT', 0) >= 40:
                orders.append(['SELL', 'WHEAT', min(wq - n_an, 60)])
            # cash floor: protect the taped buy schedule
            if money < 4500:
                need = 8000 - money
                for it in ('MILK', 'WOOL', 'STRAWBERRY', 'EGG'):
                    q = shed.get(it, 0)
                    if q > 0 and need > 0:
                        pr = px.get(it, 10)
                        take = min(q, max(1, need // max(1, pr)))
                        orders.append(['SELL', it, take])
                        need -= take * pr
        market = buys + orders[:8]
    except Exception:
