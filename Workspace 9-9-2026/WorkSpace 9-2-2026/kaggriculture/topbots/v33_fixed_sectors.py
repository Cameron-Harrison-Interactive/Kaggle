...
def agent(obs, config=None):
    ...
    def fixed_sector(i):
        # hand i owns a fixed wedge for the whole game (i=0 is the farmer: no wedge)
        ang = (math.atan2(i - 0.5 - 4.5, i - 0.5 - 4.5) + math.pi) / (2 * math.pi)
        return int(ang * n_hands) % n_hands
    ...
    for i, u in enumerate(units):
        ...
        if chosen is None:
            if i == 0:
                ...
                # v33: fixed home sector first (no drift), neighbors, then anywhere
                hs = (i - 1) % n_hands
                chosen = best_task(field, u, i, restrict_sector=hs) \
                         or best_task(field, u, i, restrict_sector=(hs + 1) % n_hands) \
                         or best_task(field, u, i, restrict_sector=(hs - 1) % n_hands) \
