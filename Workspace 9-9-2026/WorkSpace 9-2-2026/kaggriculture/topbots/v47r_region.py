...
def plan_market(S, day):
    ...
    want = min(13, max(crew_target(day), min(13, 8 + (n_ops - 60) // 20)))
    ...
    # ---- FULL FARM: fill the board; every crop bounded by its last
    # profitable sow day (prods must land before d29-30) ----
    if day >= 10 and len(S["quadrants"]) >= 3:
        if pxs >= 120 and day <= 19:
            targets["STRAWBERRY"] = max(targets.get("STRAWBERRY", 0), 32)
        if pxt >= 60 and day <= 21:
            targets["TOMATO"] = max(targets.get("TOMATO", 0), 12)
        if pxw >= 30 and day <= 26:
            targets["WHEAT"] = max(targets.get("WHEAT", 0), 30)
    ...
    pipe_cap = {"STRAWBERRY": 33, "TOMATO": 16}
    for crop in order:
...
def plan_day(S, day):
    ...
    if field:
        # ---- REGION ROUTER: partition tasks by quadrant; units own compact
        # 5x5 regions (walks 1-3 steps) instead of angular cross-farm wedges
        ...
        buckets = {"NW": [], "NE": [], "SW": [], "SE": []}
        for t in field:
            buckets[_qof(t[1])].append(t)
        regions = [q for q in ("NW", "NE", "SW", "SE") if buckets[q]]
        K = len(avail)
        alloc = {}
        remaining_units = K
        open_regions = list(regions)
        for q in regions:
            n = max(1, round(K * len(buckets[q]) / max(1, len(field))))
            n = min(n, remaining_units - (len(open_regions) - 1))
            alloc[q] = n
            remaining_units -= n
            open_regions.remove(q)
        ui = 0
        for q in regions:
            n = alloc[q]
            seg_all = sorted(buckets[q], key=lambda t: (t[1][0] + t[1][1], t[1][0], t[1][1]))
            for k in range(n):
                if ui >= K:
                    break
                u = avail[ui]; ui += 1
                seg = seg_all[k * len(seg_all) // n:(k + 1) * len(seg_all) // n]
                if not seg:
                    continue
                if any(t[0] == "FERTILIZE" for t in seg):
                    routes[u].append(("PICKUP", _shed_near(spawn[u] or (4, 4)), "FERTILIZER",
                                      min(6, sum(1 for t in seg if t[0] == "FERTILIZE"))))
                pos = spawn[u] or (4, 4)
                rem = list(seg)
                while rem:
                    ni = min(range(len(rem)), key=lambda kk: _d(pos, rem[kk][1]))
                    t = rem.pop(ni)
                    routes[u].append((t[0], t[1], t[2]))
                    pos = t[1]
