...
def agent(obs, config=None):
    ...
    _early_milk = day < 6 and n_cows < 4
    if (0 <= day <= 20 and n_cows < (4 if _early_milk else tgt_cows)
            and money > (420 if day < 6 else (750 if race else 900))
    ...
    if (day <= 27 and n_animals > 0 and money > 300
            and shed.get("WHEAT", 0) + 2 * len(wheat_ripe) < n_animals + 4
            and _feed_px < 90 and hour >= 2):
        _q = min(n_animals + 4 - shed.get("WHEAT", 0),
