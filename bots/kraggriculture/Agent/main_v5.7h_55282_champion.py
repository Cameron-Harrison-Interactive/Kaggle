"""
Kaggriculture v5.7h — water-safe mandatory route crews test.
...
def worker_sector(idx, board_size):
    """Assign exactly 3 hands per ACTIVE unlocked quadrant.

    Previous mapping cycled NW/NE/SW/SE even when SW/SE were still locked, so
    hands 3/4 were effectively assigned to unavailable land and then wandered.

    Correct staging:
      NW only        -> hands 1,2,3 all NW
      NW+NE          -> hands 1-3 NW, hands 4-6 NE
      NW+NE+SW       -> hands 1-3 NW, 4-6 NE, 7-9 SW
      all 4          -> hands 1-3 NW, 4-6 NE, 7-9 SW, 10-12 SE
    """
    if idx <= 0:
        return None
    quads = _ACTIVE_QUADS or ["NW"]
    q_index = ((idx - 1) // 3) % len(quads)
...
def route_progress_for_lane(lane, ax, ay, h):
    """Three route designs inside each quadrant, flipped by quadrant via local coords.

    lane 0: outer/perimeter route, best for long outer crops.
    lane 1: middle serpentine route.
    lane 2: inner/near-shed loop route, best for high-maintenance fast crops.
    """
    # ring from shed-side entrance: 0 near shed, h-1 outer corner/edges
    ring = max(ax, ay)
    if lane == 0:
        # Outer perimeter path: prefer far ring and border cells, sweep around the outside.
        border_bonus = 0 if (ax == h - 1 or ay == h - 1) else 4
        # approximate clockwise perimeter order from entrance-side top/side out around edge
        if ay == h - 1:
            order = ax
        elif ax == h - 1:
            order = (h - 1) + (h - 1 - ay)
        else:
            order = 2 * h + abs((h - 1) - ring)
        return abs((h - 1) - ring) * 8 + border_bonus + order
    if lane == 1:
        # Middle serpentine: own the middle rings/strips.
        ideal = max(1, h // 2)
        strip_penalty = abs(ax - ideal) * 5
        progress = ay if ax % 2 == 0 else (h - 1 - ay)
        ring_penalty = abs(ring - ideal) * 2
        return strip_penalty + ring_penalty + progress
    # Inner loop: keep near shed/center rows tight for wheat/carrot maintenance.
    inner_penalty = ring * 6
    progress = ax + ay
    return inner_penalty + progress


def lane_patrol_score(idx, pos, target, board_size, kind="empty"):
    """3-workers-per-quadrant route-lane scoring.

    Hands are conceptually assigned:
      hands 1-3   -> first active quadrant lanes 0/1/2
      hands 4-6   -> second active quadrant lanes 0/1/2
      hands 7-9   -> third active quadrant lanes 0/1/2
      hands 10-12 -> fourth active quadrant lanes 0/1/2

    This is soft; if the assigned lane has no work, the worker can still help elsewhere.
    """
    if idx <= 0:
        return 0
    q, ax, ay, h = quadrant_entry_coords(target, board_size)
    sector = worker_sector(idx, board_size)
    in_my_sector = sector is None or in_sector(target, sector)
    # Three workers per quadrant: three hands per active quadrant.
    lane = (idx - 1) % 3
    sector_penalty = 0 if in_my_sector else 30
    route = route_progress_for_lane(lane, ax, ay, h)
    dist = manhattan(pos, target)
    if kind in ("water", "harvest"):
        # Keep urgent work practical; lane is only a weak tiebreaker.
        return sector_penalty // 3 + route // 3 + dist
    return sector_penalty + route + dist // 2


def route_lane_match(idx, target, board_size):
    """Whether target belongs to this hand's assigned route lane.

    Lane 0 = outer/perimeter route
    Lane 1 = middle route
    Lane 2 = inner/shed-side route
    """
    if idx <= 0:
        return True
    q, ax, ay, h = quadrant_entry_coords(target, board_size)
    ...
    ring = max(ax, ay)
    if lane == 0:
        return ring >= h - 2 or ax == h - 1 or ay == h - 1
    if lane == 1:
        return 1 < ring < h - 1
    return ring <= 2
...
def route_owned_candidates(idx, candidates, board_size):
    """Strict route hierarchy.

    1. Use targets in this worker's quadrant AND assigned lane.
    2. If none, use targets in this worker's quadrant.
    3. If none, help globally.

    This prevents workers from abandoning their path/lane too early.
    """
    ...
    if sector is None:
        return candidates
    own_sector = [p for p in candidates if in_sector(p, sector)]
    own_lane = [p for p in own_sector if route_lane_match(idx, p, board_size)]
    if own_lane:
        return own_lane
    if own_sector:
        return own_sector
...
def plan_unit(idx, pos, inv, tiles, info, targets, seeds, prices, day, step, reserved, reserved_next, stage_empty=False, stage_slots=None, opp_info=None):
    ...
    if not is_dump:
        # Water-safe priority:
        # - direct current-tile PLANT/WATER still happens above
        # - for movement, prevent crop death first, then continue planting lanes
        # The seed-first movement test caused unwatered crops to turn into weeds.
        ...
        # Harvest after water/plant movement. Current-tile harvest still happens above.
        priority_bands.append(([(a[0], a[1]) for a in info.get("harvestable", [])], True, "harvest"))

        # Weeds: clear only if they are blocking usable work or if no better task exists.
        weed_list = list(info.get("weeds", []))
        if weed_list and (len(info.get("empty", [])) <= 4 or not empty_stage_ok) and day <= 24:
    ...
    for candidates, urgent, kind in priority_bands:
        if candidates:
            # Strictly try assigned quadrant/lane before letting a hand wander elsewhere.
            candidates = route_owned_candidates(idx, candidates, board_size)
            if kind == "empty":
                ...
                def empty_target_key(p):
                    ...
                    if crop_here is None:
                        crop_here = pcrop
                    return lane_task_key(idx, (x, y), p, board_size, "empty", crop_here, urgent=False)
                target = min(candidates, key=empty_target_key)
                if stage_slots is not None:
                    stage_slots[0] -= 1
            else:
                target = min(candidates, key=lambda p: lane_task_key(idx, (x, y), p, board_size, kind, None, urgent=urgent))
            reserve_coord(info, target)
            return route_step((x, y), target, idx, board_size, reserved_next, tiles)
    # Main farmer is a flexible sweeper: never sit at the barn if useful work exists.
    if idx == 0 and not is_dump:
        fallback = []
        fallback += [(u[0], u[1]) for u in info.get("unwatered", [])]
        if sum(seeds.values()) > 0:
            fallback += list(info.get("empty", []))
        fallback += [(a[0], a[1]) for a in info.get("harvestable", [])]
        fallback += list(info.get("weeds", []))[:4]
        if fallback:
            target = min(fallback, key=lambda p: manhattan((x, y), p))
            reserve_coord(info, target)
            return route_step((x, y), target, idx, board_size, reserved_next, tiles)
    return ["PASS"]
...
def agent(obs):
    try:
        ...
        # Rule: 3 hands per active/pending quadrant, plus 1 burst hand while that
        # quadrant is still being filled. This prevents day-4 no-worker weedouts.
        if not is_dump and hour <= 20 and len(market) < 10:
            ...
            # Burst worker: each current/new block gets 4 workers while there is still
            # obvious empty dirt to fill, then settles back to 3 per quadrant.
            fill_burst = 1 if (len(my_info.get("empty", [])) > 3 and day <= 24) else 0
            desired_hands_pre = min(int(DNA.get("target_hires", 13)), 3 * managed_quadrants + fill_burst)
            # Leave at least two market slots for seed/sell orders. Extra hires can happen later.
            while len(hands) + sum(1 for o in market if o == ["HIRE"]) < desired_hands_pre and len(market) < 8 and cash > 70:
        ...
        if not is_dump:
            ...
            target_hires = int(DNA.get("target_hires", 13))
