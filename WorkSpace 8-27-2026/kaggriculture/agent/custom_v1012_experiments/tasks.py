...
class CollectFertTask(Task):
    ...
    priority = 55      # S62 tested 68: +$2k on seeds 1-3 but -$3.7k on the
                       # 5-seed bench (3-seed benches are noise). Reverted.
    required_items = {}
...
class HarvestAnimalTask(Task):
    kind = "HARVEST_ANIMAL"
    # S62 tested 72: -$3.5k solo (beats out water/fert synergy). Kept 65.
    priority = 65
    required_items = {}
    def terminal_action(self, u, b): return ["HARVEST"]
    ...
    for (x, y) in board.plants():
        ...
        if not t["watered_today"]:
            ...
            if urgent:
                ...
                pri = 80  # S62 tested 64 for ongoing: -$5.4k (fert +2 needs
                          # watered-that-day; deferral breaks the synergy)
            else:
                pri = 40
        ...
        if t["yield_units"] > 0:
            if not cd["ongoing"]:
                ...
                # S62: 68 -> 74 — straw sellable volume was 57 vs opp 303 on
                # seed 1 (36/day shop drain); harvest lost the assigner race.
                if age >= cd["first"] and t["yield_units"] > 0:
                    tasks.append(HarvestTask((x, y), priority=74))
...
def generate_planting_tasks(board):
    ...
    for crop, tiles in per_crop.items():
        ...
        if crop == "STRAWBERRY":
            ...
            pri = 40   # WHEAT (S62 tested 55: -$38k, seed2 collapsed to $148)
