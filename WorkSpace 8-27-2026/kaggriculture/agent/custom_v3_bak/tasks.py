...
class FertilizeTask(Task):
    kind = "FERTILIZE"
    priority = 88      # very high — must happen BEFORE that day's end-of-day
...
def generate_tasks(board):
    ...
    for (x, y) in board.plants():
        ...
        # Only do this if we have fertilizer in shed (task will detour).
        if crop in ("STRAWBERRY", "TOMATO") and board.shed.get("FERTILIZER", 0) > 0:
...
def generate_planting_tasks(board):
    ...
    for crop, tiles in per_crop.items():
        ...
        # Strawberry gets urgent priority (needs full 16-day production window).
        pri = 78 if crop == "STRAWBERRY" else 40
