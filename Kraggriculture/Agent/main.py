"""
Kaggriculture v5.8z5f — strawberry layer + late fast fill.
...
LAST_PLANT_DAY = {"MELON": 16, "STRAWBERRY": 16, "TOMATO": 20, "WHEAT": 25, "CARROT": 26}
...
def crop_value(crop, prices, day, opp_info=None):
    ...
    if crop == "STRAWBERRY" and day > 12: val *= 0.95
    if crop == "MELON":
...
def desired_mix(info, opp_info, prices, day, step, farm):
    ...
    if day <= 1:
        ...
        # Cash-burn expansion push. Add a real strawberry value layer if its market is not crashed.
        if "MELON" in eligible: targets["MELON"] += int(desired_total * 0.38)
        if "STRAWBERRY" in eligible and prices.get("STRAWBERRY", 120) >= 145:
            targets["STRAWBERRY"] += int(desired_total * 0.18)
        if "TOMATO" in eligible: targets["TOMATO"] += int(desired_total * 0.20)
...
def is_trimmed_side_column(pos, board_size):
    """Ignore only the far/fence-side vertical column of each quadrant.

    The previous trimmed test removed both side columns, which also removed the
    shed-side/closest vertical row. That was too much. Keep the closest row and
    trim only the far outside column so 2 workers cover 20 useful tiles/quad.
    """
    if pos is None:
        return False
    q, ax, ay, h = quadrant_entry_coords(pos, board_size)
    return ax == h - 1
...
def best_crop_for_empty_slot(pos, prices, day, opp_info, board_size):
    ...
    if is_outer_plot(pos, board_size):
        # If strawberries are scarce/pricey, use the neglected market. Otherwise melon remains default.
        if 12 <= day <= LAST_PLANT_DAY["STRAWBERRY"] and prices.get("STRAWBERRY", 120) >= max(155, prices.get("MELON", 250) * 0.70) and crop_allowed_on_plot("STRAWBERRY", pos, board_size):
            return "STRAWBERRY"
        if 12 <= day <= LAST_PLANT_DAY["MELON"] and crop_allowed_on_plot("MELON", pos, board_size):
            return "MELON"
    best = None
    best_score = -10**9
    for crop in CROPS:
...
def lane_local_path(lane, h):
    """Two strict adjacent routes over the 4 usable columns.

    Far/fence-side column ax=h-1 is ignored. The shed-side column ax=0 is restored.
    lane 0 covers ax 0-1. lane 1 covers ax 2-3. Consecutive points are adjacent.
    """
    if h <= 2:
        return [(0, 0)]
    max_work_col = h - 2
    if lane == 0:
        # Shed-side route: column 0 down, column 1 up.
        pts = []
        for ay in range(0, h):
            pts.append((0, ay))
        if max_work_col >= 1:
            for ay in range(h - 1, -1, -1):
                pts.append((1, ay))
        return pts
    # Outer-middle route: pass through row 0 to col 2, then sweep col 2 down and col 3 up.
    c1 = min(2, max_work_col)
    c2 = max_work_col
    pts = []
    for ax in range(0, c1 + 1):
        pts.append((ax, 0))
    for ay in range(1, h):
        pts.append((c1, ay))
    if c2 > c1:
        pts.append((c2, h - 1))
        for ay in range(h - 2, -1, -1):
            pts.append((c2, ay))
...
def route_work_match(idx, target, board_size):
    """Whether this tile is this hand's assigned WORK segment.

    lane 0 works shed-side columns ax 0-1. lane 1 works outer-middle columns ax 2-3.
    Far/fence column is ignored. Pass-through route cells are movement only.
    """
    if idx <= 0:
        return not is_trimmed_side_column(target, board_size)
    if is_trimmed_side_column(target, board_size):
...
def strict_patrol_action(idx, pos, tiles, info, seeds, prices, day, board_size, reserved_next, opp_info=None):
    """Force a hired hand to move exactly one step along its route.

    No searching ahead for better targets. No skipping. No veering. Direct work on
    the current route tile is handled before this function. If no direct action
    exists, the only legal movement is the next tile in the assigned route.
    """
    if idx <= 0:
        return None
    return next_route_action(pos, idx, board_size, reserved_next, tiles)
        ...
        if not is_dump:
            ...
            for crop in CROPS:
                ...
                if crop in ("WHEAT", "CARROT"):
                    # Late fast-fill tweak: these can still mature/clear near the end.
                    stop_buy_day = LAST_PLANT_DAY[crop] + 1
                elif crop == "MELON" and all_land_open and cash >= 1200 and day <= LAST_PLANT_DAY["MELON"]:
                    stop_buy_day = LAST_PLANT_DAY[crop] + 1
                elif crop == "STRAWBERRY" and cash >= 1000 and day <= LAST_PLANT_DAY["STRAWBERRY"]:
                    stop_buy_day = LAST_PLANT_DAY[crop] + 1
                else:
            ...
            if all_land_open and cash >= 1200 and 12 <= day <= LAST_PLANT_DAY["MELON"]:
                ...
                # If strawberry prices are strong, reserve part of the outer ring for strawberries.
                if day <= LAST_PLANT_DAY["STRAWBERRY"] and prices.get("STRAWBERRY", 120) >= 145:
                    strawberry_need = max(0, min(outer_empty_count // 3, 10) - seeds.get("STRAWBERRY", 0))
                    deficits["STRAWBERRY"] = max(deficits.get("STRAWBERRY", 0), strawberry_need)
                    deficits["MELON"] = max(0, deficits["MELON"] - strawberry_need)
                # During outer value surge, do not waste cash on additional wheat bank.
                deficits["WHEAT"] = min(deficits.get("WHEAT", 0), 2)
                deficits["CARROT"] = min(deficits.get("CARROT", 0), 4)

            # Seed-bank discipline. Keep rolling banks small early/mid, but allow
            # late fast-fill on empty dirt if there is still time to harvest.
            if seeds.get("WHEAT", 0) >= int(DNA.get("max_wheat_seed_bank", 6)) and day < 23:
                deficits["WHEAT"] = 0
            if day > LAST_PLANT_DAY["WHEAT"]:
                deficits["WHEAT"] = 0
            if seeds.get("CARROT", 0) >= 10 and day < 24:
                deficits["CARROT"] = 0
            if day > LAST_PLANT_DAY["CARROT"]:
                deficits["CARROT"] = 0
            ...
            if day <= 1:
                ...
                # Late fast-fill: use empty dirt with crops that can still pay out.
                order = ["CARROT", "WHEAT", "TOMATO", "MELON", "STRAWBERRY"]
            else:
            ...
            for crop in order:
                ...
                if crop == "MELON" and all_land_open and cash >= 1200 and 12 <= day <= LAST_PLANT_DAY["MELON"]:
                    ...
                    cap = 8 if day <= LAST_PLANT_DAY["STRAWBERRY"] else 0
                elif crop == "CARROT":
