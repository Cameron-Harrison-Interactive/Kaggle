...
# water schedule
# --------------------------------------------------------------------------
def compute_water_days(plants, obs_history, daily=True, switch_day=None):
...
def compile_seat(seed, seat, mod, jitter=None, variant=None):
    ...
    jitter = jitter or {}
    variant = variant or {}
    tape, obs_history, ref_reward = record_reference(seed, seat, mod, variant)
    ...
    if cadence == "daily":
        schedule = compute_water_days(plants, obs_history, daily=True)
    elif cadence == "eod":
        schedule = compute_water_days(plants, obs_history, daily=False)
    elif cadence == "switch":
        schedule = compute_water_days(plants, obs_history, daily=False,
    ...
    for day in range(30):
        ...
        for tile, days in schedule.items():
            if day in days and alive_on_day(tile, day, obs_history, seat):
...
def apply_sell_mutations(tape, variant):
    ...
    for s, o in sells:
        ...
        if split and qty > 10:
            ...
            batches = [(max(0, min(718, s + shift)), item, qty)]
        else:
            batches = [(s, item, qty)]
