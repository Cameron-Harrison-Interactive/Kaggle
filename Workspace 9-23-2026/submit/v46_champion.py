...
def agent(obs, configuration=None):
    ...
    # Majkel sells fertilizer. That cash lands NE at d6 h6.
    if day <= 8:
        fert_targets = set()
        fert_keep = 0
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        if quad == "NW":
            ...
            if (not final_day and vseeds.get("MELON", 0) > 0
                    and counts["MELON"] < 12 and day <= 16):
                return "MELON"
        ...
        if (quad in ("SW", "SE") and not final_day
                ...
                and day <= 16):
            return "MELON"
        # Replay: SW is wheat the day it opens. After the fill,
        # argmax resumes (all-season wheat on SW killed the factory).
        if (quad == "SW" and not final_day
                and vseeds.get("WHEAT", 0) > 0 and day <= 11):
            return "WHEAT"
        if (not final_day and vseeds.get("STRAWBERRY", 0) > 0
                and counts["STRAWBERRY"] < STRB_TARGET):
            return "STRAWBERRY"
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        for q in quads:
            ...
            # Replay: NE packs ~20 (berries + herd). Cap 12 left
            # d8 dirt on y=2-4.
    ...
    if (state.get("plan_day") != day
        ...
        if hour > 0 and _fill_lock:
            ...
            for _i in range(len(positions)):
                ...
                _k = _oldt.get(_i, 0)
                if (_k < len(_old) and _old[_k][0] in ("PLANT", "WATER")
                        and quadrant(_old[_k][1]) in ("NE", "SW")):
                    plans[_i] = _old[_k:]
                    ptrs[_i] = 0
    ...
    if (hour > 0 and day <= 11 and "SW" in owned
        ...
        for _i in range(len(positions)):
            ...
            if _k < len(_pl):
                continue
            _tgt = None
            for _p in sorted(_sw_empty,
                             key=lambda p, _i=_i: distance(positions[_i], p)):
                if _p not in _claimed:
                    _tgt = _p
                    break
            if _tgt is None:
                break
    ...
    if (not final_day and state.get("land_day") == day
        ...
        if want > 0:
            unit = CROPS[opener][0]
            qty = min(1, want, int(max(0, virtual_cash - 150) // unit))
            if qty > 0:
                buy_order(["BUY_SEED", opener, qty], unit * qty, 150)
