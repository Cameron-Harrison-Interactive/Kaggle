...
# Horizon-Line-V1. THEIR opening, OUR mid-late swap.
# Open: 2 cows + 3 sheep, 6 melon, wheat, 4 hands. No d0 berries,
# no mass hire. From d4: berries on y=2 into NE (horizon).
# Do not submit.
...
def agent(obs, configuration=None):
        ...
        for q in quads:
            ...
            if q == "NW":
                ...
                for si, wi in enumerate(wr_q[:nband] if nband else wr_q[:1]):
                    ...
                    # ((0,3)(1,3)(0,4)). A-worker already walks these as
                    # row_plants after the packed 6. Spray tick days so
                    # they don't wait on a crop-worker pickup.
                    _row_fert = []
                    if (not final_day
                    ...
                    if _row_fert:
                        ...
                        _rf_take = min(3, len(_row_fert))
                        if _have >= 1:
                            _row_fert = _row_fert[:min(_rf_take, _have)]
                        elif shed["FERTILIZER"] > 0:
                            st = shed_near(w["pos"])
                            if (w["busy"] + distance(w["pos"], st) + 1
                                    <= budget):
                                w["plan"].append(
                                    ("PICKUP_FERT", st, _rf_take))
                                w["busy"] += distance(w["pos"], st) + 1
                                w["pos"] = st
                                _row_fert = _row_fert[:_rf_take]
                            else:
                                _row_fert = []
                        else:
                            _row_fert = []
    ...
    if (not final_day and state.get("land_day") == day
        ...
        q_tiles = sum(
            ...
            and not animal_line((x, y)) and (x, y) not in SHED)
        opener = "WHEAT"
        # mix-first through d10: do not strawberry-flood a new quad
        if (quoted("STRAWBERRY") >= 160 and not final_day
                and day >= 10 and day + 10 <= total_days - 1):
            opener = "STRAWBERRY"
    ...
    targets = {
        ...
        "COW": min(6, day + 2) + (
            1 if n_milk_shop and quoted("MILK") >= 160 else 0),
        "SHEEP": min(8 if (n_yarn and quoted("WOOL") >= 160) else 6,
                    3 if day <= 6 else max(3, day - 4)) + (
