        ...
        for q in quads:
            ...
            if not wr_q:
                ...
                if not wr_q:
                    ...
                    continue
            # CLONE C52 (C51: day-gate changed nothing - s5 damage is
            # d<=10 too. Both buy late; s5 recovers (A-depth), s303
            # doesn't (solo feeder). Borrow iff solo feeder.
            _need = (len(mine_q) + 2) // 3
            _hun = sum(1 for _, _t in mine_q
                       if _t.get("consecutive_unfed", 0) >= 1
                       and not _t.get("fed_today"))
            _sev = (_hun >= 3
                    and len(wr_q) * 7 < len(mine_q) * 2
                    and len(wr_q) == 1)
            while mine_q and _sev and len(wr_q) < _need:
                _ql = [i for i, r in enumerate(roles)
                       if r == ("Q", q) and i not in wr_q]
                if not _ql:
                    break
                wr_q.append(_ql[0])
