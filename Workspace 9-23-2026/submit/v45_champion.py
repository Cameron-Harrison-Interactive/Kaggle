...
def agent(obs, configuration=None):
    ...
    if opp_strb >= 30 and quoted("STRAWBERRY") < 160:
        ...
        if n_strb_shop >= 2 and quoted("STRAWBERRY") >= 160 and day <= 22:
            ...
            strb_target = 33 if opp_strb >= 30 else 24
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        if quad == "NW" and not war:
            # Horizon line: y=2 is berries (into NE). Never melon.
            # Wheat cover only before d3 so the row is empty for the
            # berry wave. y=0-1 melon (cap 10 so it stays off y=2).
            # 9-10 by d6: y=2 (5) + leftover holes (3) + y=1 east (2).
            # East of NW, next to NE — not the west strip.
            if pos in ((3, 1), (4, 1)) or (pos is not None and pos[1] == 2):
                if (day >= 3 and not final_day
                        and vseeds.get("STRAWBERRY", 0) > 0
                        and (counts["STRAWBERRY"] < STRB_TARGET
                             or (pos[1] == 2 and day >= 16
                                 and quoted("STRAWBERRY") >= 160))):
                    return "STRAWBERRY"
                # Keep empty through d3 so the berry wave can land.
                # After berries die (~d20) hold y=2 for a berry replant
                # when straw still pays; wheat only if it does not.
                if day < 4:
                    return None
                if (pos[1] == 2 and 16 <= day <= 22 and not final_day
                        and quoted("STRAWBERRY") >= 160):
                    return None
            ...
            if (day <= 12 and vseeds.get("MELON", 0) > 0
                    and counts["MELON"] < (6 if day <= 3 else 10)):
                return "MELON"
            if vseeds.get("WHEAT", 0) > 0 and day <= 26:
                return "WHEAT"
            return None
    ...
    _pending_feed = any(
        ...
        for _i, _pl in _plans_cur.items()
        for j in _pl[_ptrs_cur.get(_i, 0):])
    _feed_rebuild = (day <= 10 and hour == 1)
    ...
    if (
        ...
        if len(owned) == 1 and day >= 6:
            ...
            # s202 SW at $2334 left $334 and the crew died. KEEP's
            # package bar is ~$3300; do not buy SW broke.
            _need = max(_need, 3000)
        if (density >= _dens
                and money >= _need
                and herd_ready
    ...
    if (not final_day and state.get("land_day") == day
        ...
        q_tiles = sum(
            ...
            and not animal_line((x, y)) and (x, y) not in SHED)
        opener = "WHEAT"
        # Majkel: berries into NE the day it opens
        if (not final_day and day >= 6 and day + 10 <= total_days - 1
                and len(owned) >= 2):
            opener = "STRAWBERRY"
    ...
    if day >= 2 and not final_day and hour > 0:
    ...
    if day <= 18 and not final_day and not (day == 0 and hour == 0):
        ...
        for kind in ("GOOSE", "COW", "SHEEP"):
            ...
            if day + first >= total_days - 2:
                continue
            if kind == "SHEEP" and quoted("WOOL") < 80 and (
                    herd["SHEEP"] + held_animals["SHEEP"] >= 3):
                continue
    ...
    if 2 <= day <= 8 and not final_day:
        ...
        if _bgap > 0:
            _bq = min(_bgap, int(max(0, virtual_cash - 150) // 100))
            if _bq > 0:
    ...
    if day == 0:
        ...
        desired_hands = min(4, max(3, desired_hands))
    elif day <= 8:
        desired_hands = max(desired_hands, 5)
    ...
    if (day <= 22
        ...
        if strb_gap > 0:
            _res = 1150 if ("NE" not in owned or day < 10) else 150
            if "NE" not in owned or day < 10:
                _pace = 6
            elif strb_gap >= 8:
                _pace = 8
            else:
            ...
            if qty > 0:
                buy_order(["BUY_SEED", "STRAWBERRY", qty], 100 * qty, 150)
