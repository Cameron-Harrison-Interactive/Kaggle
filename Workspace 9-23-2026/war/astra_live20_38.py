...
def agent(obs, configuration=None):
    ...
    if not final_day and quoted("STRAWBERRY") > quoted("FERTILIZER"):
        ...
        for _pos, _t in plants:
            ...
            if _tick or _tmrw:
                _nk += 1
        fert_keep = max(fert_keep, min(2, _nk))
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        for q in quads:
            ...
            # R66: NE strawberries, half then the rest the next day.
            # Tick interval is 2, so consecutive plant days split labor
            # onto opposite tick days. 15 crop tiles → 8 then the rest.
            ne_s_have = sum(
                1 for p, t in plants
                if quadrant(p) == "NE" and t.get("crop") == "STRAWBERRY")
            ne_s_planned = [0]

            def allow_ne_s():
                have = ne_s_have + ne_s_planned[0]
                w0 = state.get("ne_strb_wave0")
                if w0 is None or day == w0:
                    return have < 8
                if day == w0 + 1:
                    return True
                cap = int(state.get("ne_strb_wave_cap", 15))
                return have < cap

            def note_ne_s():
                ne_s_planned[0] += 1
                if state.get("ne_strb_wave0") is None:
                    state["ne_strb_wave0"] = day
                if day == state.get("ne_strb_wave0") + 1:
                    state["ne_strb_wave_cap"] = ne_s_have + ne_s_planned[0]
            ...
            for bi, wi in enumerate(wr):
                ...
                for _, idx, pos_b, kind_b in stops:
                    ...
                    if kind_b == "PLANT":
                        ...
                        if q == "NE" and crop == "STRAWBERRY":
                            if not allow_ne_s():
                                crop = ("WHEAT" if vseeds.get("WHEAT", 0) > 0
                                        else None)
                                if crop is None:
                                    continue
                            else:
    ...
    if (10 <= day <= 15
        ...
        if strb_gap > 0:
            # R66: 8+8 on the two stagger days (plant next morning),
            # then 4/day so SW still fills. Consecutive plant days put
            # the two NE cohorts on opposite tick parities.
            _rate = 8 if day <= 11 else 4
            qty = min(_rate, strb_gap, int(max(0, virtual_cash - 150) // 100))
            if qty > 0 and buy_order(
