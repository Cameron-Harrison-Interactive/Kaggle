...
# R74: 5 extra sheep sit on the first crop-walk tiles next to the
# shed (SW worker-1's first 3, SE's two shed-adjacent). Crop workers
# CARE/FEED as they walk out. Not a dedicated A-worker.
SHEEP_PADS = ((3, 5), (4, 6), (3, 6), (6, 5), (5, 6))
...
def agent(obs, configuration=None):
    ...
    # R74: shed-ring sheep pads stay out of the crop list so a strawberry
    # never blocks the install (engine refuses animals on planted tiles).
    for _p in SHEEP_PADS:
    ...
    def tend_pad(w, pos_a):
        # One animal stop on a crop walk: feed, care, harvest, collect.
        # Worker is already walking out of the shed over this tile.
        tile = tile_map.get(pos_a)
        if not (isinstance(tile, dict) and tile.get("animal")):
            return
        d = distance(w["pos"], pos_a)
        cap = w.get("wbudget", budget)
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        if filler:
            ...
            # R42 (user): carrots OUT - wheat is the fill crop (fewer
            # ops per unit, home-grown feed, gentle-log price that
            # holds under volume). Carrot stays only as the mirror arm
            # (matching a carrot-committed opponent is doctrine).
            _fill_order = (("CARROT", "WHEAT") if _endgame_carrot
                            else ("WHEAT",))
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        if not final_day:
            ...
            # R74: 5 shed-ring pads (south of the barn). Sheep only.
            for _p in SHEEP_PADS:
                ...
                for _ in range(held_animals[kind]):
                    ...
                    for pos_s in sites:
                        ...
                        if q == "NW" and pos_s not in (
                            ...
                            continue
                        if q in ("SW", "SE") and (
                                pos_s not in SHEEP_PADS or kind != "SHEEP"):
                            continue
                        if qcount[q] >= (6 if q == "NW"
                                         else (3 if q == "SW"
                                               else (2 if q == "SE" else 10))):
                            continue
                        t_s = tile_map[pos_s]
        ...
        for q in quads:
            ...
            if not wr_q:
                # R74: south pad sheep are tended by the crop walk that
                # already steps on them. Do NOT convert a Q worker into
                # a full A-sweep (R43: SW fallback stole strawberries).
                if q in ("SW", "SE"):
                    continue
        ...
        for q in quads:
                    ...
                    if my_fert:
                        ...
                        if _have >= 1:
                            ...
                            if (w["busy"] + distance(w["pos"], st) + 1
                                ...
                                my_fert = []
                    # wheat for pad sheep this worker will step on
                    _pads = [p for p in band if p in SHEEP_PADS]
                    if _pads and shed.get("WHEAT", 0) > 0 and not final_day:
                        _need = sum(
                            1 for p in _pads
                            if isinstance(tile_map.get(p), dict)
                            and tile_map[p].get("animal")
                            and not tile_map[p].get("fed_today"))
                        if _need:
                            st = shed_near(w["pos"])
                            take = min(int(shed.get("WHEAT", 0)), _need)
                            if w["busy"] + distance(w["pos"], st) + 1 <= budget:
                                w["plan"].append(("PICKUP_WHEAT", st, take))
                                w["busy"] += distance(w["pos"], st) + 1
                                w["pos"] = st
                    for pos_b in band:
                        if pos_b in SHEEP_PADS:
                            tend_pad(w, pos_b)
                            continue
            ...
            for bi, wi in enumerate(wr):
                ...
                band_set = set(band)
                # R74: first SE crop worker tends the two shed-adjacent
                # sheep pads on the way out of the barn.
                if q == "SE" and bi == 0:
                    _pads = [p for p in SHEEP_PADS if quadrant(p) == "SE"]
                    if _pads and shed.get("WHEAT", 0) > 0 and not final_day:
                        _need = sum(
                            1 for p in _pads
                            if isinstance(tile_map.get(p), dict)
    ...
    if (not final_day and state.get("land_day") == day
        ...
        q_tiles = sum(
            ...
            if quadrant((x, y)) == _qf
            and not animal_line((x, y)) and (x, y) not in SHED)
        opener = "WHEAT"  # R42 (user): carrots out, wheat in - plants
        # immediately on any ground (no idle pocket stock), feeds the
        # herd at $0 bought-wheat, and drops the NE money bar to $1,300
    ...
    if day >= 2 and not final_day:
        ...
        cands = [c for c in CROPS
                 ...
                 and not (c == "STRAWBERRY" and day < 10)
                 # R42 (user): carrot buys OFF (mirror arm excepted)
                 and not (c == "CARROT" and not carrot_target
                           and not _endgame_carrot)
                 and seeds.get(c, 0) < 4]
    ...
    # R74: remaining 5 animals as sheep on the shed-ring pads.
    # North is full at 15 (6 NW + 9 NE). These buys do not count
    # against the north cap. 1/day, wool still paying.
    if (12 <= day <= 22 and not final_day
            and len(orders) < max_orders
            and quoted("WOOL") > 60):
        _pads_owned = [p for p in SHEEP_PADS if quadrant(p) in owned]
        _on_pads = sum(1 for pos, _t in animals if pos in SHEEP_PADS)
        _held_s = int(held_animals.get("SHEEP", 0))
        _n_north = sum(1 for pos, _t in animals
                       if quadrant(pos) in ("NW", "NE"))
        # pocket empty so we never stack unhousable sheep; north already
        # at the 15-head physical fill (6 NW + 9 NE).
        if (_n_north >= 15 and _held_s == 0
                and _on_pads < len(_pads_owned)):
            buy_order(["BUY_ANIMAL", "SHEEP", 1], ANIMALS["SHEEP"][0], 400)
