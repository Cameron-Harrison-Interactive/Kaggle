...
def agent(obs, configuration=None):
    ...
    # ---- R57: PAIRED WATER+FERT on strawberries, success ratchet (user) ----
    # Engine L799: fertilized = was_watered and fertilized_until_day >= day
    # -- fertilizer is worth ZERO unless the tile is watered that same day.
    # Strawberry's water days are ages 9/11/13/15 (needs_water: ongoing ->
    # age >= first-1 and (age-(first-1)) % interval == 0), which line up
    # exactly with its four yield ticks at 10/12/14/16. So the NEW
    # strawberry routing only fires in the instant after we water that tile.
    # The proven baseline 8/day (mostly melon) is left COMPLETELY untouched
    # -- gating it too measured -10,910.
    # Ratchet: the per-quad carry starts at 2 and gains +1 for every 5
    # confirmed WATER+FERT pairs.
    fert_paired = set([
        p for _, p in fert_candidates
        if (tile_map.get(p) or {}).get("crop") == "STRAWBERRY"
    ][:min(40, fert_total)])
    fert_targets = fert_targets | fert_paired
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        for q in quads:
            ...
            for bi, wi in enumerate(wr):
                ...
                if my_fert:
                    st = shed_near(w["pos"])
                    _carry = min(2 + int(state.get("fert_bonus", 0)),
                                 len(my_fert))
                    if (w["busy"] + distance(w["pos"], st) + 1 <= budget
                    ...
                    if kind_b == "PLANT":
                        ...
                        if harvestable(tile):
                            ...
                            if w["busy"] + d + 1 <= w["wbudget"]:
                                ...
                                dropped += 1
                        if (pos_b in my_fert
                                and (pos_b not in fert_paired or _did_water)
                                and w["busy"] + 1 <= w["wbudget"]):
                            w["plan"].append(("FERTILIZE", pos_b, None))
                            w["busy"] += 1
                            my_fert.remove(pos_b)
                            if pos_b in fert_paired:
                                # ratchet: +1 carry per 5 confirmed pairs
                                _ok = int(state.get("fert_ok", 0)) + 1
                                state["fert_ok"] = _ok
                                if _ok % 5 == 0:
                                    state["fert_bonus"] = int(
                                        state.get("fert_bonus", 0)) + 1
