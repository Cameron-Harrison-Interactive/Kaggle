...
def agent(obs, configuration=None):
    ...
    # R70: carrot denial latches on the FIRST opponent carrot tile.
    # Their seed pouch is private - a planted tile is the soonest
    # signal. Waiting for 20 standing is too late.
    if opp_crops.get("CARROT", 0) >= 1:
        state["se_carrot_deny"] = True
    ...
    def harvestable(tile):
        ...
        if crop == "STRAWBERRY":
            return True
        if crop == "CARROT" and state.get("se_carrot_deny"):
            return True
        if crop == "TOMATO":
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        # R70: SE plants carrots the morning we see theirs. Otherwise
        # the usual mix (forced wheat-only on SE lost 3k vs pass).
        if (quad == "SE" and state.get("se_carrot_deny")
                and vseeds.get("CARROT", 0) > 0 and not final_day
                and day <= 26):
            return "CARROT"
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        for q in quads:
            ...
            if q == "SW":
                ...
                continue
            if q == "SE":
                # R70: packed crop walk from the SE shed (5,5).
                # 24 crop tiles, 3 workers x 8, shed-adjacent first.
                SE3 = (
                    ((6, 5), (6, 6), (5, 6), (5, 7),
                     (6, 7), (6, 8), (5, 8), (5, 9)),
                    ((6, 9), (7, 9), (7, 8), (7, 7),
                ...
                if len(wr) >= 3:
                    ...
                    _use = [SE3[0] + SE3[1][:4], SE3[1][4:] + SE3[2]]
                else:
                    _use = [SE3[0] + SE3[1] + SE3[2]]
                for si, wi in enumerate(wr[:len(_use)]):
                    w = workers[wi]
                    band = _use[si]
                    for pos_b in band:
    ...
    if (state.get("se_carrot_deny") and not final_day and day <= 26
            and len(orders) < max_orders):
        _cgap = max(0, 8 - seeds["CARROT"])
        if _cgap > 0:
            _cq = min(4, _cgap, int(max(0, virtual_cash - 150) // 20))
