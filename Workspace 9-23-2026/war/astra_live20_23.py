...
def agent(obs, configuration=None):
    ...
    # ---- R56b: let STOCK be the limit, not an arbitrary 8/day ----
    # MEASURED (fert debug trace, seed 42, live20_18): we collect 264
    # fertilizer a season and spray 17 (6.4%); the shed holds 3-19 units
    # every single day from d4 on while 30-47 tiles/day are profitable
    # candidates, and only 8 get flagged. The FERTILIZE op only fires when
    # a worker is already standing on the tile with budget left, so the way
    # to raise execution is to flag MORE tiles - the worker will then hit a
    # flagged tile on the route it was already walking.
    # Value check: engine L800 pays +2 instead of +1 per tick when a tile
    # is fertilized AND watered, and fertilizer lasts 3 days (L481), so one
    # application is worth +1 to +2 units. At strawberry $200-316 that is
    # $200-632 in the field versus ~$50 sold on the market.
    fert_targets = {
        pos for _, pos in fert_candidates[:min(40, fert_total)]
    }
