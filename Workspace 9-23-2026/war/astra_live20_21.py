...
def agent(obs, configuration=None):
    ...
    if (state.get("plan_day") != day
        ...
        # R55: 13 -> 14 hands (crew 15). The 15th hand is the one lever
        # the playbook has never actually tested (line 233: "hand #15
        # costs ~$377-610/day - crew 14 is the cap that matters";
        # line 235 left the south-animal build disabled "for a future
        # paying-15th-hand test"). Both open defects trace to the same
        # root cause - the crew is oversubscribed, so survival waters
        # and care ops are the jobs that get dropped:
        #   * ~4 strawberries/game die BEFORE first yield (age 3-9),
        #     a total loss (war/weed_deaths2.py, 6 seeds);
        #   * care coverage sits at 40%, leaving the cow care prize
        #     (11 -> 31 units, x2.8) unclaimed at milk ~$300.
    ...
    desired_hands = max(3, min(14, int(state.get("crew", 1)) - 1))  # R55: 13 -> 14
    if day <= 1:
