...
# R105b NE cash first. 79 had $1203 at d7h0 and still opened NE d9
# — feed wheat bought first and land missed the slot. Land before
# feed when NE is payable. Urgent = cuw>=2. 150k is NE d7 morning.
...
def agent(obs, configuration=None):
    ...
    _ne_now = (hour <= 3 and len(owned) == 1 and 6 <= day <= 21
               and money >= 1150)
    if not final_day:
    ...
    if (
        ...
        urgent = sum(
            ...
            and int(t.get("consecutive_unwatered", 0)) >= 2
