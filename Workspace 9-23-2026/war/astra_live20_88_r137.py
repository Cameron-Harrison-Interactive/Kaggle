...
# R137. Pizza/milk town is a berry town. Stefano ran 33 berries
# on pizza with 0 berry shops; we sat at 12. If milk shops >= 1
# and straw >= $140 through d16, factory 24 (not 33, not
# opp>=8). NE opener berries at straw $140 (was $160) so the
# new quad follows the start. Land $1150 stays. Do not submit.
...
def agent(obs, configuration=None):
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        if pos is not None and quadrant(pos) == "NW" and animal_line(pos):
            ...
            if (4 < day <= 8 and vseeds.get("WHEAT", 0) > 0
                ...
                return "WHEAT"
            # R122: leftover animal-row tiles the packed 6 never use.
        ...
        if quad == "NW" and not war:
            ...
            if (day <= 12 and vseeds.get("MELON", 0) > 0
                    and counts["MELON"] < 12):
                return "MELON"
            # R122: leftover NW crop after 12 melon is berries, not wheat.
    ...
    # R137: 4 berry seeds on d0-d1 at the $1150 land bar. Tops have
    # 6-8 berries in the melon field by d6; we had 0 because the
    # quota waited for NE. This does not lower land reserve (R123
    # 150/400 and R127 400 died). Plan plants the seeds in NW holes.
    if day <= 1 and not final_day:
        _bgap = 4 - counts["STRAWBERRY"] - seeds["STRAWBERRY"]
        if _bgap > 0:
            _bq = min(_bgap, int(max(0, virtual_cash - 1150) // 100))
            if _bq > 0 and buy_order(
                ["BUY_SEED", "STRAWBERRY", _bq], 100 * _bq, 1150
            ):
