...
def agent(obs, configuration=None):
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        # ---- R58: NO STRAWBERRY IN SW, EVER (user) ----
        # Measured (war/weed_deaths2.py, 6 seeds): every single one of the
        # ~4/game strawberries that die BEFORE first yield is in SW, and SW
        # accounts for 103 of the 187 strawberry deaths. Strawberry is the
        # thirstiest crop we grow - water every 2 days on ages 9/11/13/15
        # plus a harvest between every tick - and the SW band is the one
        # that cannot land them, so those tiles return nothing at all.
        # SW goes to crops that pay on 2 workers: late-game carrots (4 ops,
        # crash the market) and wheat (5 ops, $40+ at the endgame).
        if quad == "SW":
            # R58: SW runs the CHEAP crops. Strawberry needs water on ages
            # 9/11/13/15 plus a harvest between every tick (9 ops); wheat is
            # 5 ops and carrot 4, and both hold up on 2 workers. Carrots
            # whenever the carrot target is live (that gate is opponent-
            # driven - opp >= 20 standing - so it is 0 in solo, which is
            # exactly the "bot needs to measure it" gap you flagged),
            # otherwise wheat, which closes at $40+ every game.
            if (carrot_target and vseeds.get("CARROT", 0) > 0
                    and not final_day):
                return "CARROT"
            if vseeds.get("WHEAT", 0) > 0:
                return "WHEAT"
            return None
        elif (not final_day and vseeds.get("STRAWBERRY", 0) > 0
