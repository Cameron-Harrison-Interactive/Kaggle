...
def agent(obs, configuration=None):
    ...
    # ---- R57: PAIRED WATER+FERT, gated on a success ratchet (user) ----
    ...
    # So we only ever spray a tile in the instant after we water it, and we
    # start tiny (2/day) and let the ratchet open up: every 5 confirmed
    # water+fert pairs earns +1 to the daily budget. Strawberries are the
    # target (4 production ticks, +1 unit each when fertilized, $200-316).
    _fert_budget = int(state.get("fert_budget", 2))
    _strb_fert = [p for _, p in fert_candidates
                  if (tile_map.get(p) or {}).get("crop") == "STRAWBERRY"]
    fert_targets = fert_targets | set(
        _strb_fert[:min(_fert_budget, fert_total)])
                    ...
                    if kind_b == "PLANT":
                        ...
                        if harvestable(tile):
                            ...
                            if w["busy"] + d + 1 <= w["wbudget"]:
                                ...
                                dropped += 1
                        if (pos_b in my_fert and _did_water
                                and w["busy"] + 1 <= w["wbudget"]):
                            w["plan"].append(("FERTILIZE", pos_b, None))
                            w["busy"] += 1
                            my_fert.remove(pos_b)
                            # R57 ratchet: every 5 confirmed WATER+FERT
                            # pairs earns +1 to the daily budget.
                            _ok = int(state.get("fert_ok", 0)) + 1
                            state["fert_ok"] = _ok
                            if _ok % 5 == 0:
                                state["fert_budget"] = int(
                                    state.get("fert_budget", 2)) + 1
