...
def agent(obs, configuration=None):
    ...
    # ---- R56d: strawberry fert, but ONLY where the tile gets watered ----
    # Engine L799: fertilized = was_watered and fertilized_until_day >= day
    # -- fertilizer pays NOTHING on a tile that is not watered. We measured
    # (weed_deaths2) that every one of the ~4/game total-loss strawberries
    # is in SW, killed by missed water, so spraying the south buys zero and
    # the hour spent spraying steals from watering. Restrict the extra
    # strawberry fert to the quads that actually land their water (NW/NE).
    # If this reads positive it confirms the diagnosis and the ordering:
    # fix the SW water coverage FIRST, then fertilizer starts paying.
    _strb_fert = [p for _, p in fert_candidates
                  if (tile_map.get(p) or {}).get("crop") == "STRAWBERRY"
                  and quadrant(p) in ("NW", "NE")]
