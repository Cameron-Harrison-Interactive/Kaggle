...
def agent(obs, configuration=None):
    ...
    if not final_day and quoted("STRAWBERRY") > quoted("FERTILIZER"):
        ...
        for _pos, _t in plants:
            ...
            if _tick or _tmrw:
                _nk += 1
        fert_keep = max(fert_keep, min(8, _nk))
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        for q in quads:
            ...
            # R67: every paying NE tick strawberry, but only as many
            # as leftover hours after must-work (harvest + survival
            # water). Coverage never loses to a spray.
            _ne_pair = []
            ...
            for bi, wi in enumerate(wr):
                ...
                if q == "NE":
                    _ticks = [p for p in _ne_pair if p in band_set]
                    _pos_e = w["pos"]
                    _must = 0
                    for _p in band:
                        _t = tile_map.get(_p)
                        ...
                        if isinstance(_t, dict) and _t.get("kind") == "PLANT":
                            if harvestable(_t):
                                _need = True
                            elif (not _t.get("watered_today") and not final_day
                                  and needs_water(_t)):
                                _need = True
                        if _need:
                            _must += distance(_pos_e, _p) + 1
                            _pos_e = _p
                    _room = max(0, w["wbudget"] - _must)
                    _ns = min(len(_ticks), _room // 2)
                    my_fert = _ticks[:_ns]
                if my_fert:
                    _carry = len(my_fert)
                    _have = 0
