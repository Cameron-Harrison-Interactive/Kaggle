...
# R93 KEEP. 150k opening on R91. vs-pass AVG 135273 (+6197 vs 63).
# H2H vs 41 12-0 median +4281. 0 geese, 2 cows d0 -> 6 by d7,
# NE d6 @ dens 0.80, 12 melon + strb from d0 (2/day until NE).
# Sheep after NE, yarn to 10. Never SE. R32 cow-swap-alone was
# -7.4k because it kept d10 berries; the whole package pays.
...
def agent(obs, configuration=None):
    ...
    if (
        ...
        if len(owned) == 1 and day >= 6:
            _need = land_cost + 150
            _dens = .80  # 150k has NE d7h0; 0.90 blocked us at 12 melon
        if (density >= _dens
                and money >= _need
                and herd_ready
    ...
    targets = {
        ...
        "COW": min(6, day + 2),
        "SHEEP": (
            min(10, max(2, day - 3)) if n_yarn
            else (0 if "NE" not in owned else 2)),
    }
