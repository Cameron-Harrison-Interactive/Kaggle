...
# R88 REJECT s42 105k vs 58 126k, weeds 23 vs 22. FIB tail \$233+377+610/day.
# R88 SE-float crew on R86. When SE is locked, hire the SE crop team
# anyway and put them on NW/NE/SW (weeds/water). Hire toward 16
# until FIB cost is too steep. When SE opens they go home. Do not
# overwrite 41/51. v34 is 58.
...
def agent(obs, configuration=None):
    ...
    if war:
        ...
        strb_target = 8
        # Berry-shop routine. 4 of 8 shops eat strawberries. Ladder
        # 140k bots (ominteam/Taeyang/SH11235) stand 33 with 0 weeds
        # and straw stays $200+ because the town eats them. v32's
        # always-33 is 676 rating; v33's always-8 is 610. Long-horizon:
        # 0 berry shops -> 8; 1 shop and quote holding -> 16; 2+ and
        # quote >= $160 -> 33. Window still d10-d15 plant.
        if n_strb_shop >= 2 and quoted("STRAWBERRY") >= 160 and day <= 15:
            strb_target = 33
        elif n_strb_shop >= 1 and quoted("STRAWBERRY") >= 140 and day <= 15:
            strb_target = 16
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        while len(roles) < n_workers:
            # floaters cover SW then NE (weedy far edges), not NW
            _pool = [q for q in ("SW", "NE") if q in quads] or list(quads)
            roles.append(("Q", _pool[qi % len(_pool)]))
            qi += 1
        while len(roles) > n_workers:
            roles.pop()
    ...
    if (state.get("plan_day") != day
        ...
        # SE crop team (3 Q) floats on owned quads until SE unlocks.
        # SE crop team floats only when the berry factory is on
        # (33 standing is what overruns 13 bodies). Cheap 12-hand
        # crew stays for the 8-berry town.
        if ("SW" in owned and "SE" not in owned and STRB_TARGET >= 16):
            _spec_n += 3
        ...
        _work_floor = min(16, max(4, (25 * len(owned) + len(animals)) // 5))
    ...
    targets = {
        ...
        "COW": min(6, max(0, day - 3)) + (1 if n_milk_shop and "SW" in owned else 0),
        "SHEEP": min(6, max(0, day - 4)) + (1 if n_yarn and "SW" in owned else 0),
    }
    ...
    desired_hands = max(3, min(15, int(state.get("crew", 1)) - 1))
    if day <= 1:
    ...
    if hour <= 3:
        ...
        while hires_today < desired_hands and hires_today < len(FIB):
            ...
            # First 12 hands are cheap (FIB sum $376). SE-float tail
            # (hands 13-15, $233/$377/$610) hires if cash is there.
            _res = 20 if hires_today < 12 else (250 if day < 2 else 80)
            if hires_today >= 12 and cost > 400 and day < 12:
                break
