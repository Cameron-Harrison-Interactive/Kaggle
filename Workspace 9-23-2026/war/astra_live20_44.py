...
def agent(obs, configuration=None):
    ...
    _se_reserved = set(SE_ROW) if state.get("se_row") else set()
    _sw_reserved = set()
    if state.get("sw_row"):
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        if not final_day:
            ...
            if state.get("se_row"):
                for _sp in SE_ROW:
                    _st = tile_map.get(_sp)
                    if _sp in SHED:
                        continue
                    if _st is None or (isinstance(_st, dict) and (
                            _st.get("kind") == "WEED"
                            or (_st.get("kind") in ("COOP", "PASTURE")
                                and not _st.get("animal")))):
                        sites.append(_sp)
        ...
        for q in quads:
            ...
            if q == "NW":
                ...
                continue
            if q == "SE" and state.get("se_row"):
                w = workers[wr_q[0]] if wr_q else None
                if w is None:
                    dropped += sum(
                        1 for _, tl in mine_q
                        if not tl.get("fed_today") and not final_day)
                else:
                    mine = [(pp, tl) for pp, tl in mine_q if pp in SE_ROW]
                    mine.sort(key=lambda a: SE_ROW.index(a[0])
                              if a[0] in SE_ROW else 99)
                    to_feed = ([a for a in mine
                                if not a[1].get("fed_today")]
                               if not final_day else [])
                    if to_feed and shed_wheat > 0:
    ...
    if (15 <= day <= 20 and not final_day and "SW" in owned
        ...
        pass
    if ("SE" in owned and not final_day and not state.get("se_row")
            and sum(opp_animals.values()) >= 3 and day >= 12):
        _kind = min(
            ("COW", "SHEEP", "GOOSE"),
            key=lambda k: (opp_animals[k],
                           {"COW": 0, "SHEEP": 1, "GOOSE": 2}[k]))
        if quoted(ANIMALS[_kind][2]) > 20:
            state["se_row"] = _kind
            state["se_target"] = 4
    if (state.get("se_row") and not final_day and day <= 26
            and len(orders) < max_orders):
        _sk = state["se_row"]
        _se_have = sum(1 for _p, _t in animals
                       if quadrant(_p) == "SE" and _t.get("animal") == _sk)
        if _se_have < state.get("se_target", 4):
            buy_order(["BUY_ANIMAL", _sk, 1], ANIMALS[_sk][0], 400)
