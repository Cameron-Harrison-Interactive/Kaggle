                ...
                for idx, pos_b in enumerate(band):
                    ...
                    if tile is None:
                        ...
                        if (tile.get("kind") == "WEED"
                            ...
                            # R60: a tile with RIPE UNITS is money sitting
                            # in the field - it must be in the priority-0
                            # queue with the dying plants and tick-day
                            # waters, not lumped in with bare dirt at
                            # priority 1. This is the routing half of the
                            # fertilizer problem: engine L800 caps standing
                            # units at max_yield (4), so if the harvest does
                            # not come back before the next tick, the +2
                            # from fertilizer is silently thrown away.
                            ripe = harvestable(tile)
                            stops.append(
                                (0 if (dying or tick or ripe) else 1,
                                 idx, pos_b, "PLANT"))
