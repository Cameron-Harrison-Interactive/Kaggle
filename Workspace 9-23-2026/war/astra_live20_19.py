...
def agent(obs, configuration=None):
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        def maybe_drop(w):
            ...
            # R53 FIX 3b: on the final day also dump opportunistically when
            # a worker is ALREADY near a shed tile (<=3 moves). Cheap by
            # construction - no detour, no work displaced. The all-days
            # threshold-2 variant measured CATASTROPHIC (-$40-65k) and the
            # 6-hour work-budget reserve measured -5.4k/-8.1k: cutting final
            # -day harvests costs far more than the stranded cargo is worth.
            if w["cargo"] < 4:
                if not (final_day and w["cargo"] >= 2
                        and distance(w["pos"], shed_near(w["pos"])) <= 3):
                    return
            st = shed_near(w["pos"])
