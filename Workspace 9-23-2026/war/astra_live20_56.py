...
# R84 KEEP (ladder, not H2H): north herd + north A-hands UNCHANGED.
# Three SW crop-walk pads (2,5)(3,5)(4,6) when SW opens. Tomatoes on
# SW/SE only when PIZZA_SHOP or FARMERS_MARKET is already unlocked
# and first yield still fits (cap 6/8). s5 planted zero (no shop in
# time). Solo AVG 108578 vs 51 101894 (+6684). Do not overwrite 41/51.
SCATTER_PADS = (
    (3, 5), (2, 5), (4, 6),
    (6, 5), (7, 6), (8, 5),
)
...
def agent(obs, configuration=None):
    ...
    tomato_room = 0
    _tom_shops = sum(
        1 for s in (obs.get("town", {}) or {}).get("unlocked_shops", [])
        if s in ("PIZZA_SHOP", "FARMERS_MARKET"))
    ...
    if (not final_day and day >= 2 and day + 8 <= total_days - 2
        ...
        tomato_ok = True
        tomato_cap = 6 if _tom_shops == 1 else 8
        se_fill_tomato = True
