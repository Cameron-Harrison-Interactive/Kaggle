                ...
                for si, wi in enumerate(wr_q[:nband] if nband else wr_q[:1]):
                    ...
                    for pos_r, tile_r in (row_plants_q[si::max(1, len(wr_q))]
                        ...
                        if (not tile_r.get("watered_today") and not final_day
                                and int(tile_r.get("consecutive_unwatered", 0)) >= 1
                                and w["busy"] + d2 + 1 <= budget):
                            w["plan"].append(("WATER", pos_r, None))
                            w["busy"] += d2 + 1
                            w["pos"] = pos_r
                        elif harvestable(tile_r):
            ...
            if q == "NE":
                ...
                for si, wi in enumerate(wr[:len(_use)]):
                    ...
                    for pos_b in band:
                        ...
                        _live = (not tile.get("watered_today") and not final_day
                                 and int(tile.get("consecutive_unwatered", 0)) >= 1)
                        if _live and not _sprayed:
                            d = distance(w["pos"], pos_b)
                            if w["busy"] + d + 1 <= w["wbudget"]:
                                w["plan"].append(("WATER", pos_b, None))
                                w["busy"] += d + 1
                                w["pos"] = pos_b
                                _sprayed = True
                        if (harvestable(tile) or _ne_rotate) and not _live:
                            d = distance(w["pos"], pos_b)
                            if w["busy"] + d + 1 <= w["wbudget"]:
                    ...
                    for pos_b in band:
                        ...
                        if _live and not _sprayed:
                            ...
                            if w["busy"] + d + 1 <= w["wbudget"]:
                                ...
                                w["pos"] = pos_b
                                _sprayed = True
                        if harvestable(tile) and not _live:
                            d = distance(w["pos"], pos_b)
                            if w["busy"] + d + 1 <= w["wbudget"]:
                                w["plan"].append(("HARVEST", pos_b, None))
                                w["busy"] += d + 1
