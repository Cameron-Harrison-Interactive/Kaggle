...
def agent(obs, configuration=None):
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        # R71: SE holds all four cash crops so we always have a first
        # sale in each market. 6 tiles each. No tomatoes.
        if quad == "SE" and not final_day:
            have = Counter()
            for _p, _t in plants:
                if quadrant(_p) == "SE":
                    have[_t.get("crop")] += 1
            have.update(se_mix_plan)
            cands = []
            for c, floor in (("STRAWBERRY", 6), ("MELON", 6),
                             ("CARROT", 6), ("WHEAT", 6)):
                if vseeds.get(c, 0) <= 0:
                    continue
                if c == "STRAWBERRY" and (
                        day < 10 or day + 10 > total_days - 1):
                    continue
                if c == "MELON" and (
                        counts["MELON"] >= melon_cap
                        or day + 10 > total_days - 2):
                    continue
                if c == "CARROT" and day > 26:
                    continue
                if c == "WHEAT" and day > 27:
                    continue
                cands.append((have[c] - floor, have[c], c))
            if cands:
                pick = min(cands)[2]
                se_mix_plan[pick] += 1
                return pick
    ...
    # R71: SE mix ammo - carrots are otherwise never bought.
    if ("SE" in owned and not final_day and day <= 26
            and len(orders) < max_orders):
        _cgap = max(0, 4 - seeds["CARROT"])
        if _cgap > 0:
            _cq = min(_cgap, int(max(0, virtual_cash - 150) // 20))
