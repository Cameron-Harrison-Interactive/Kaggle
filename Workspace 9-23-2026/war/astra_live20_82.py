...
# R106 KEEP. 150k open + new land is berries. vs-pass 136632.
# H2H vs 41 12-0 s42 105k/117k. vs 78 mix-first 12-0 median +4380.
# vs 65 8-4. NEXT SUBMIT. 81/83/84 REJECT (broke NE, NW-berries
# -15k, hire-cap -14k, NE-staff -1k, fert8 wash).
...
def agent(obs, configuration=None):
    ...
    if (not final_day and state.get("land_day") == day
        ...
        q_tiles = sum(
            ...
            and not animal_line((x, y)) and (x, y) not in SHED)
        opener = "WHEAT"
        if (quoted("STRAWBERRY") >= 140 and not final_day
