...
def agent(obs, configuration=None):
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        def maybe_drop(w):
            ...
            #
            # ---- R61: stop paying for what the engine does for free ----
            # _end_of_day() calls _drop_inventories_to_shed() and then
            # clears farm["hands"], so every scrap of cargo reaches the shed
            # at h23 anyway. A mid-day DROP is a round trip that buys
            # nothing - UNLESS the shed would overflow, because that same
            # function DISCARDS anything past capacity (100). Measured
            # peak: shed 76 non-wheat, cargo 69 - so the guard is real.
            # Drop mid-day only when loaded (10+) or when the shed is
            # heading for the cap. Final day always drops (score is money
            # at step 719 and the EOD dump lands too late to be sold).
            if w["cargo"] < 4:
                return
            if not final_day:
                if (sum(shed.values()) + w["cargo"] <= 70
                        and w["cargo"] < 10):
                    return
            st = shed_near(w["pos"])
