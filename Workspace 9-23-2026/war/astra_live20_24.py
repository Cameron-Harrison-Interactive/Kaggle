...
def agent(obs, configuration=None):
    ...
    # ---- R56c: STRAWBERRY fert priority (user) ----
    # Flagging 40 tiles of ANY crop measured -8,106: the `value > 35` gate
    # admits wheat (~$100) and carrot (~$70), and each FERTILIZE hour is an
    # hour not spent reaching the next tile. So widen the flag set for
    # STRAWBERRY ONLY and leave every other crop on the proven 8/day.
    # Why strawberry: engine L800 pays +2 instead of +1 per tick when a tile
    # is fertilized AND watered, fertilizer lasts 3 days (L481) and
    # strawberry ticks every 2, so one application is +1 to +2 units - at
    # $200-316 that is $200-632 for a single 1-hour op on a tile the worker
    # is already standing on, versus ~$50 if we just sell the fertilizer.
    _strb_fert = [p for _, p in fert_candidates
                  if (tile_map.get(p) or {}).get("crop") == "STRAWBERRY"]
    fert_targets = fert_targets | set(_strb_fert[:min(40, fert_total)])
