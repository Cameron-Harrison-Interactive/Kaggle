...
# R83: animals on the shed (spawn tiles, 0-walk) and on the first
# crop-walk square of each quad. Each crop worker tends 1-2 then crops.
HYBRID_PADS = (
    (4, 4), (5, 4), (4, 5), (5, 5),
    (3, 3), (2, 4),
    (6, 2), (7, 4),
...
def agent(obs, configuration=None):
    ...
    # R83: pads include shed tiles (Fizie put a cow on (4,4)).
    for _p in HYBRID_PADS:
        if quadrant(_p) in owned:
            _sw_reserved.add(_p)
    crop_empty = [p for p in crop_empty if p not in _sw_reserved]
    crop_weeds = [p for p in crop_weeds if p not in _sw_reserved]
    available_sites = []
    for pos in HYBRID_PADS:
        if quadrant(pos) not in owned:
            continue
    ...
    _tomato_shops = sum(1 for s in _shops if s in (
        "PIZZA_SHOP", "FARMERS_MARKET"))
    # R83: plant tomatoes when a tomato shop is open or quote pays seed.
    if (not final_day and day >= 4 and day + 8 <= total_days - 2
            and (quoted("TOMATO") >= 70 or _tomato_shops >= 1)):
        se_fill_tomato = True
    if ("SE" in owned and not final_day
            and 6 <= day and day + 8 <= total_days - 2):
        se_fill_n = sum(1 for p in crop_empty if quadrant(p) == "SE")
        if se_fill_n >= 3 and not se_fill_tomato:
            proj_m = Counter(counts)
            proj_m["MELON"] += se_fill_n
            s_wheat = max(1e-9, crop_score("WHEAT"))
            if crop_score("MELON", proj_m) > s_wheat:
                melon_cap += se_fill_n
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        if (quad in ("SW", "SE") and se_fill_tomato
                and vseeds.get("TOMATO", 0) > 0
                and counts["TOMATO"] < 11
        ...
        live = [
            ...
            and not (c == "TOMATO" and quad not in ("SW", "SE"))
        ]
        if filler:
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        def prefix_pads(w, q, slot, nslot):
            pads = [pos for pos in HYBRID_PADS if quadrant(pos) == q]
        ...
        for q in quads:
            ...
            if not wr_q:
                # R83: crop workers tend pads on their walk. Never steal
                # a Q into a full A-sweep (R43).
                continue
    ...
    if day <= 18 and not final_day:
        ...
        _north_cap = sum(1 for pos in HYBRID_PADS if quadrant(pos) in owned)
    ...
    if (se_fill_tomato and 4 <= day <= 21 and not final_day):
        _tgap = 11 - counts["TOMATO"] - seeds["TOMATO"]
        if _tgap > 0:
            _tq = min(4, _tgap, int(max(0, virtual_cash - 150) // 50))
            if _tq > 0 and buy_order(
                ["BUY_SEED", "TOMATO", _tq], 50 * _tq, 150
            ):
                seeds["TOMATO"] += _tq
