...
# R103 KEEP on 77. Strb d8 only if they already have 8 berries.
# Unconditional d8 REJECT -10.7k. vs-pass +0. H2H vs 41 12-0.
# NEXT SUBMIT = 78: Stefano 1-shop + 8/day catch-up + tomato hold
# + carrot-after-factory + goose-2 if they 25 + d8 if they 8.
...
def agent(obs, configuration=None):
    ...
    if day >= 2 and not final_day:
        ...
        cands = [c for c in CROPS
                 ...
                 and not (c == "STRAWBERRY" and day < (
                     8 if opp_strb >= 8 else 10))
    ...
    targets = {
        ...
        "GOOSE": (
            2 if (n_yarn or n_milk_shop or opp_strb >= 25)
    ...
    _strb_open = 8 if opp_strb >= 8 else 10
    if (_strb_open <= day <= 22
            and not final_day):
        strb_gap = STRB_TARGET - counts["STRAWBERRY"] - seeds["STRAWBERRY"]
        if strb_gap > 0:
            _pace = 8 if strb_gap >= 8 else 4
            qty = min(_pace, strb_gap, int(max(0, virtual_cash - 150) // 100))
            if qty > 0 and buy_order(
