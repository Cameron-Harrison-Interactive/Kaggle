...
def agent(obs, configuration=None):
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        # ---- R59: SE = wheat + carrots, no strawberries (user) ----
        # SE is the far corner: every visit is a long walk, and strawberry
        # is the thirstiest crop we grow (water on ages 9/11/13/15 plus a
        # harvest between every tick, 9 ops). Wheat is 5 ops and carrot 4,
        # and both are the late-game crops - wheat closes at $40+ every
        # game. Carrots whenever the carrot target is live (NOTE: that gate
        # is opponent-driven - opp >= 20 standing - so it is 0 in solo,
        # which is the measurement gap you flagged).
        if quad == "SE":
