...
def agent(obs, configuration=None):
    ...
    if war:
        ...
        # R78: just enough berries to sell before the seed stops
        # paying (+11 sold => price < $100). 8 plants x ~5-6 units
        # is that window. Extra tiles are melon/carrot/wheat so the
        # labor that used to babysit 33 berries waters, ferts, and
        # feeds instead. War/match vs a 30+ berry opponent stays.
        strb_target = 8
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        if pos is not None and quadrant(pos) == "NW" and animal_line(pos):
            ...
            if (day <= 4 and vseeds.get("WHEAT", 0) > 0
                ...
                return "WHEAT"
            # R80: no second wheat cycle. Those 3-4 tiles at d7 were
            # blocking the last cow/sheep installs. After the d0-4
            # cash-bridge wheat, rows stay empty for the herd. One
            # wheat already lives in the shed all game.
            ...
            if q == "NW":
                ...
                for si, wi in enumerate(wr_q[:nband] if nband else wr_q[:1]):
                    ...
                    my_inst = [(ps, k) for ps, k in list(nw_late_installs)
                               if ps in bt]
                    n_inst = (2 if not mine else (1 if len(mine) < len(bt) else 0))
    ...
    # R80: 3 extra pasture animals on the old wheat pads.
    # Opponent's weaker herd first; else town shops (yarn vs milk).
    _opp_c = opp_animals.get("COW", 0)
    _opp_s = opp_animals.get("SHEEP", 0)
    _shops = list(obs.get("town", {}).get("unlocked_shops", []) or [])
    _milk_n = sum(1 for s in _shops if s in (
        "PIZZA_SHOP", "ICE_CREAM_SHOP", "SMOOTHIE_SHOP"))
    _yarn_n = sum(1 for s in _shops if s == "YARN_STORE")
    if _opp_c < _opp_s:
        _extra = "COW"
    elif _opp_s < _opp_c:
        _extra = "SHEEP"
    elif _yarn_n > _milk_n:
        _extra = "SHEEP"
    else:
        _extra = "COW"
    if day >= 5:
        targets[_extra] = min(9, targets[_extra] + 3)
    ...
    if day <= 18 and not final_day:
        ...
        _north_cap = sum(9 if q == "NW" else 10 for q in _north)
