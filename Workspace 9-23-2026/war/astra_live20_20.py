...
def agent(obs, configuration=None):
    ...
    # ---- R54a: PRICE-CONDITIONAL COW CEILING (the herd-mix pivot) ----
    # MEASURED (price probe, live20_18): MILK climbs 169 -> 293 (seed 42)
    # and 169 -> 314 (seed 5) over the season while EGG sits flat at $42.
    # The north's 18 sites are filled on a FIXED 6 cow / 6 sheep / 6 goose
    # plan with no price response, so we cap out on the best product on the
    # board. war/horizon.py --war already ranks MILK highest for own-gain
    # (+$7,571 per unit/day) because town drain (22/day) keeps milk in
    # permanent structural deficit - selling more does not crash it.
    #
    # WHY THIS IS NOT R32. R32's cow-for-geese swap measured -7.4k because
    # it pulled cows EARLIER (cow #2 forward ~4 days) straight into the
    # d4-7 cash window where the melon wave pre-buys its seeds - "any
    # mid-game cash add cascades" (R51 RANCH canary, seed 101 -12.2k).
    # This raises the cow CEILING only, so cows 7-8 are first targetable at
    # d11 (min(8, day-3)), well after the melon seed pre-buy, and it adds
    # no headcount: the north cap still binds at 18, so it can only change
    # the MIX of slots that were going to be filled anyway.
    _milk_rich = day >= 9 and quoted("MILK") >= 250
    ...
    targets = {
        ...
        # R54a: 6 normally, 8 in milk-rich games (see _milk_rich above).
        "COW": min(6 + (2 if _milk_rich else 0), max(0, day - 3)),
