...
def agent(obs, configuration=None):
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        # R75: leftover hours only. Crop workers leave the shed over the
        # four animal tiles that touch it. If the crop band finished with
        # slack, prepend CARE/FEED/collect on THAT worker's ring tile —
        # paid with leftover, so the strawberries still get done. Skip
        # any op an A-worker already booked (no double walk).
        _claimed = set()
        _RING = {"NW": ((4, 3), (3, 4)), "NE": ((5, 3), (6, 4))}
        if hour == 0:
            for _w in workers:
                for _op, _pos, _ in _w["plan"]:
                    if _op in ("FEED", "CARE", "HARVEST", "COLLECT_FERTILIZER"):
                        _claimed.add((_op, _pos))
        if hour != 0:
            _RING = {}
        for wi, w in enumerate(workers):
            if wi >= len(roles) or roles[wi][0] != "Q":
                continue
            q = roles[wi][1]
            tiles = _RING.get(q)
            if not tiles:
                continue
            q_qs = [j for j, r in enumerate(roles) if r == ("Q", q)]
            slot = q_qs.index(wi) if wi in q_qs else 0
            if slot >= len(tiles):
                continue
            pos_a = tiles[slot]
            ...
            if slack < 1:
                continue
            cur = w.get("home", w["pos"])
            prefix = []
            add = 0
            # wheat for a leftover feed (only if A did not already book it)
            if (not tile.get("fed_today") and not final_day
                    and ("FEED", pos_a) not in _claimed
                    and shed.get("WHEAT", 0) > 0):
                st = shed_near(cur)
                d = distance(cur, st)
                if add + d + 1 <= slack:
                    prefix.append(("PICKUP_WHEAT", st, 1))
                    add += d + 1
                    cur = st
            d = distance(cur, pos_a)
            if (not tile.get("fed_today") and not final_day
                    and ("FEED", pos_a) not in _claimed
                    and add + d + 1 <= slack):
                prefix.append(("FEED", pos_a, None))
                add += d + 1
                cur = pos_a
                d = 0
            if (not final_day and not tile.get("cared_today")
                    and quoted(ANIMALS[tile["animal"]][2]) > 10
                    and ("CARE", pos_a) not in _claimed
                    and add + d + 1 <= slack):
                prefix.append(("CARE", pos_a, None))
                add += d + 1
                cur = pos_a
                d = 0
            if (int(tile.get("yield_units", 0)) >= 1
                    and ("HARVEST", pos_a) not in _claimed
                    and add + d + 1 <= slack):
                prefix.append(("HARVEST", pos_a, None))
                add += d + 1
                cur = pos_a
                d = 0
                w["cargo"] += 1
            if (tile.get("fertilizer_available")
                    and ("COLLECT_FERTILIZER", pos_a) not in _claimed
                    and add + d + 1 <= slack):
                prefix.append(("COLLECT_FERTILIZER", pos_a, None))
                add += d + 1
                cur = pos_a
            if prefix:
                w["plan"] = prefix + w["plan"]
                w["busy"] += add
