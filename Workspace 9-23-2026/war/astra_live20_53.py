...
def agent(obs, configuration=None):
    ...
    if (
        hour <= 3 and 1 <= len(owned) < 4
        and day <= (25 if len(owned) == 3 else 21)
        ...
        if len(owned) == 1:
            ...
            _animal_budget = 0
        # R81: 4th quad (SE) never opened in v33 self-play — H2H cash
        # never cleared fill+density .95 before d21. SE is $4000 land;
        # drop the wheat-fill pre-budget and ease density so we actually
        # buy it. NE/SW keep the old bar.
