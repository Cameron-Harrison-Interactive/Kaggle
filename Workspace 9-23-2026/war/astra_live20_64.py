...
# R92 REJECT. 150k-mix (d0 berries + cut melon + NE d6) s42 84980
# vs 63 127973 (-43k). First cut (melon 8, 4 strb/day) 91443 then
# 116311. Starves NE cash/density. Do not restack opening. Live=63.
...
def agent(obs, configuration=None):
    ...
    if opp_strb >= 30 and quoted("STRAWBERRY") < 160:
        ...
        if n_strb_shop >= 2 and quoted("STRAWBERRY") >= 160 and day <= 15:
            ...
            strb_target = 16
    # 150k bots stand 12 berries by d7 with or without shops.
    if day <= 12:
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        if quad == "NW" and not war:
            # R92: 150k mix. 8 melon (still the NE cash) then
            # strawberries on the rest of NW. They stand 12 strb + 12
            # melon by d7; our old 15-melon lockout left 0 berries
            # until d10 and $200 at d12.
    ...
    targets = {
        ...
        "GOOSE": (
            2 if (n_yarn or n_milk_shop)
            else min(6, 2 + max(0, day - 3))),
    ...
    if day <= 1:
        _m_gap_n, _m_res = 12, 150  # R92: 12 melon + berries, not 8
    ...
    if (day <= 15
        ...
        if strb_gap > 0:
            # Pay for NE first. 4/day after it opens; 2/day before.
            _res = 1150 if "NE" not in owned else 400
            _rate = 2 if "NE" not in owned else 4
            qty = min(_rate, strb_gap, int(max(0, virtual_cash - _res) // 100))
            if qty > 0 and buy_order(
