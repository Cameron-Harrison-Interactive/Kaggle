...
def agent(obs, configuration=None):
    ...
    # ---- R56a: reserve fertilizer against the OPPORTUNITY, not the stock ----
    # MEASURED (fert_probe, seed 42): we COLLECT 264 fertilizer a season and
    # spray only 17 (6.4%); the rest is sold at ~$50. Meanwhile strawberry
    # closes the season at $200-316, and engine L800 pays +2 units per tick
    # instead of +1 when a tile is BOTH fertilized and watered - on an
    # ongoing crop with 4 production ticks that is 8 units instead of 4, so
    # one fertilizer is worth $200-632 in the field versus ~$50 sold.
    #
    # The old line sized the reserve from len(fert_targets), but targets
    # were themselves capped by fert_total - and the sell loop empties the
    # shed every hour because fertilizer trades above its $40 floor. That is
    # a self-reinforcing starvation loop: sell it all -> no stock -> no
    # targets -> no reserve -> sell it all. Size the reserve by how many
    # profitable targets the FIELD has instead, so stock can accumulate.
    _fert_want = min(8, len(fert_candidates))
    fert_keep = max(0, _fert_want - all_carried["FERTILIZER"])
    try:
        with open("/tmp/fert_dbg.txt", "a") as _ff:
            _ff.write("%s %s %s %s\n" % (day, len(fert_candidates),
                                          fert_total, _fert_want))
    except Exception:
        pass
