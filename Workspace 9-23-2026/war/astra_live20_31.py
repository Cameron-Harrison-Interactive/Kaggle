...
def agent(obs, configuration=None):
    ...
    def plan_crop_choice(vseeds, filler=False, quad=None, pos=None):
        ...
        # ---- R59b: SE -> wheat/carrot for the LATE GAME only (user) ----
        # Full-season SE wheat measured -8,245, so early SE strawberries
        # (which complete their 17-day cycle) stay. From _SE_SWITCH on, a
        # strawberry sown in SE first-yields at +10 days and gets at most
        # one tick before the horn, so SE switches to the cheap crops.
        if quad == "SE" and day >= 20:
