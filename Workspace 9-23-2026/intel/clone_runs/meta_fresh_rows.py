    ...
    if (state.get("plan_day") != day
        ...
        if hour > 0 and _fill_lock:
            ...
            for _i in range(len(positions)):
                ...
                # New morning hires change band ownership. Preserve a
                # seed/water chain, not an entire old row that duplicates
                # the new hand's row and erases its assigned work.
                if day >= 10 and hour <= 3:
                    _keep = _chain
                if _keep:
                    plans[_i] = _old[_k:]
