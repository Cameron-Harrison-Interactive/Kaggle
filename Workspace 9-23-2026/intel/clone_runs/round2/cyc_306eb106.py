...
def agent(obs, configuration=None):
    ...
    def harvestable(tile):
        ...
        if crop == "WHEAT" and not near_decay and day >= 10:
            # Replay 109798422 d11+: wheat is cut at 2-3 units
            # (age 2-3), then the tile replants the same pass.
            # Drives the d11 wave (6 cuts + 9 plants). Gated post-dump:
            # the opening desk must mature untouched.
            return units >= 2 and age >= 2
                        ...
                        if harvestable(tile) or _ne_rotate or _nw_mrot:
                            ...
                            if w["busy"] + d + 1 <= w["wbudget"]:
                                ...
                                if tile.get("crop") == "MELON":
                                    ...
                                    maybe_drop(w)
                                # Replay: a cut tile is refilled one-for-one.
                                # This replaces, not expands, serviced ground,
                                # so the zone cap must not block it. The DROP
                                # above runs first: dump-day delivery timing
                                # is untouched.
