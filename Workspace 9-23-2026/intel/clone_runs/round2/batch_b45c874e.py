    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        for q in quads:
            ...
            for bi, wi in enumerate(wr):
                ...
                # In-band melon cuts remaining (helpers excluded: they run
                # their own water/cut/deliver job). Feeds batch-2 logic.
                _melon_batch_left = sum(
                    1 for _, _, _p, _k in stops
                        ...
                        if harvestable(tile) or _ne_rotate or _nw_mrot:
                            ...
                            if w["busy"] + d + 1 <= w["wbudget"]:
                                ...
                                if tile.get("crop") == "MELON":
                                    ...
                                    # round trips the refills need.
                                    _melon_batch_left -= 1
                                    if (w["cargo"] >= 12
                                            or _melon_batch_left <= 0):
                                        maybe_drop(w)
                                elif not my_fert:
                                    maybe_drop(w)
