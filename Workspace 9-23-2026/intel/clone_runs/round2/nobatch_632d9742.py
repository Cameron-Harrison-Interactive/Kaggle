...
def agent(obs, configuration=None):
    ...
    def build_plan(n_workers, budget, pinned=None):
        ...
        def maybe_drop(w):
            ...
            # Replay d11+ batching to 12 REVERTED (round 3): it saved
            # ~10 moves/day but the 12-unit chunks crashed sale prices
            # (-17.5k s202, -9.6k s5; price impact dominates travel on
            # berry-heavy seeds). His 16/drop works with his pricing;
            # ours must stay in small chunks. Keep 4.
            if w["cargo"] < 4:
                return
            st = shed_near(w["pos"])
        ...
        # Replay: his buyer covers rotation same-day (d22-24: 12-13
        # wheat/day). Our plans snapshot leftover before the buy block
        # runs, so rotation starves on cohort mornings. Assume today's
        # wheat buys land (market executes h0, before hands move). A
        # short only costs that hour's tail plants (per-turn atomic).
        if 18 <= day <= 26:
            _rot_need = sum(
                1 for _, t in plants
                if isinstance(t, dict)
                and t.get("crop") in ("STRAWBERRY", "TOMATO")
                and int(t.get("max_lifespan_step", -1)) >= 0
                and int(t.get("yield_units", 0)) == 0)
            vseeds["WHEAT"] = max(vseeds["WHEAT"],
                                  min(14, _rot_need))
        ...
        for q in quads:
            ...
            if (q in ("NE", "NW") and not final_day and day >= 11
                ...
                for _p in band_order:
                    ...
                    if int(_t.get("fertilized_until_day", -1)) >= day:
                        continue
                    if int(_t.get("max_lifespan_step", -1)) >= 0:
                        continue
                    _age = day - int(_t.get("planted_day", day))
                    ...
                    if tile is None:
                        ...
                        if (tile.get("kind") == "WEED"
                            ...
                            _window_wheat = (not tile.get("watered_today")
                                             ...
                                             and needs_water(tile))
                            # Replay d19-24: spent berries (final tick done,
                            # expiry scheduled) rotate to wheat instead of
                            # drinking and dying into weeds. Ongoing crops
                            # start with no expiry; the engine sets it only
                            # at the final tick, so this cannot misfire.
                            _spent = (18 <= day <= 26
                                      and tile.get("crop") in ("STRAWBERRY", "TOMATO")
                                      and int(tile.get("max_lifespan_step", -1)) >= 0)
                            stops.append((-1 if (_m_dump and day == 10) else 0 if (dying or tick or _m_dump or _window_wheat or _spent) else 1,
                                          idx, pos_b, "PLANT"))
                    ...
                    if kind_b == "PLANT":
                        ...
                        if harvestable(tile) or _ne_rotate or _nw_mrot:
                            ...
                            if w["busy"] + d + 1 <= w["wbudget"]:
                                ...
                                dropped += 1
                        # Late rotation: a spent berry/tomato is dug and
                        # replanted to wheat in the same stop (his d22-24:
                        # ST 24->10, WH 25->36). Never dig without a wheat
                        # seed in hand; never water or hire for the dead.
                        if (_spent_now and not final_day
                                and vseeds.get("WHEAT", 0) > 0):
                            _rd = distance(w["pos"], pos_b)
                            if w["busy"] + _rd + 3 <= w["wbudget"]:
                                w["plan"].append(("DIG", pos_b, None))
                                w["plan"].append(("PLANT", pos_b, "WHEAT"))
                                w["plan"].append(("WATER", pos_b, None))
                                w["busy"] += _rd + 3
                                w["pos"] = pos_b
                                vseeds["WHEAT"] -= 1
        ...
        if order[0] == "BUY_SEED" and len(order) >= 3:
            ...
            _pmax = 8 if day <= 1 else (
                ...
                10 if _c == "STRAWBERRY" and day <= 8 else (
                    ...
                    14 if (_c == "WHEAT" and 12 <= day <= 26
                          and "SW" in owned
                          ) else (
                    4 if (_c == "WHEAT" and day <= 22
                          and "SW" in owned
                          ) else (
                        min(8, max(2, _dirt)) if (
    ...
    if day >= 2 and not final_day and hour > 0 and state.get("land_day") != day:
        ...
        room = max(0, field_capacity - len(plants)
                   ...
                   + _rot_spent)
        need = max(0, min((14 if 18 <= day <= 26 else 12), slots, room)
                   - standing_seeds)
        ...
        cands = [c for c in CROPS
                 ...
                 and seeds.get(c, 0) < (14 if (c == "WHEAT" and day >= 18) else 2)]
        for _ in range(need) if cands else []:
            ...
            if not buy_order(["BUY_SEED", crop, 1], cost, 600):
                break
            projected[crop] += 1
            _pc = 14 if (crop == "WHEAT" and day >= 18) else 2
