...
def plan_market_orders(board, cfg):
    ...
    if buy_land and hour == 0:
        ...
        LAND_RESERVE = {"NE": 50, "SW": 100, "SE": 400}  # tighter — SW earlier
    ...
    if len(orders) < 10 and hour <= 3:  # only buy early in the day
        ...
        # (Early-game D0-D2 aggressive wheat+animal buy tested: solo drops
        # $80k -> $26k because cash-to-zero window hurts many actions.
        # H2H marginal. Reverted.)
