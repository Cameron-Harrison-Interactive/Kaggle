...
def plan_market_orders(board, cfg):
    ...
    # ---------- HIRING: front-load at hour 1 (10-order cap = 10 hires/day
    # max realistically; more requires multi-hour spread, which we tried and
    # over-hired — reverted).
    ...
    # MELON seeds — $80 each, plant window closes ~D16 (need 10 days to first
    # yield). Only buy if we can plant before D18.
    ...
    elif current_herd > 0 and wheat_in_shed < current_herd * 2 \
            and money > 200 and len(orders) < 10:
        # Buy enough to cover 3 more feed days
        n_buy = min(current_herd * 3, 10 - len(orders))
        if n_buy > 0:
            orders.append(["BUY_PRODUCT", "WHEAT", n_buy])
            money -= 30 * n_buy   # rough cost estimate
