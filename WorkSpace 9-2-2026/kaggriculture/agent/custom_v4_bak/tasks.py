...
def generate_planting_tasks(board):
    ...
    per_crop = {}
    for (x, y), role in layout.items():
        if role not in ("CROP", "STRAW", "MELON"):
            continue
        if board.is_locked(x, y):
            continue
    ...
    for crop, tiles in per_crop.items():
        ...
        # Priority by crop:
        #   STRAWBERRY: urgent — needs full 16-day production window
        #   MELON:      high — 12-day cycle, only worthwhile planted by ~D16
        #   WHEAT:      base — quick 4-day cycle, can catch up any time
        if crop == "STRAWBERRY":
            pri = 78
        elif crop == "MELON":
            pri = 72
