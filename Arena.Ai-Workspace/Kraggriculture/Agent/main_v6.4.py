...
def choose_target(pos, farm, private, obs, carrying, is_farmer):
    ...
    if feed and carrying != "WHEAT":
        if any_unfed(farm):
            t, _ = nearest(pos, SHED_TILES)
            return t
    # When all animals are fed/cared for, claim crop jobs per-turn so hands
    # don't pile onto the same plant ("alternating watering"). If animals still
    # need work, use simple nearest so any hand can opportunistically help.
    if not any_unfed(farm):
        for bucket in (feed, ancare, water, harv, weeds, plant):
            if bucket:
                t = _claim_target(pos, bucket)
                if t:
                    return t
    else:
        for bucket in (feed, ancare, water, harv, weeds, plant):
