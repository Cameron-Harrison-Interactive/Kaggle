...
DEFAULT_DNA = {
    "version": "v6.2",
    # Aggressive cow-heavy build (Seb meta): 13 cows + 7 sheep across 4 quads.
    "target_cows": 13,
    "target_sheep": 7,
    "buy_ne_day": 4,
    "buy_sw_day": 6,
...
# Compact pasture corridor target positions (built in this order). Mirrors Seb:
# a dense animal cluster around the shed spreading through all 4 quadrants,
# eventually holding 13 cows + 7 sheep = 20 animals.
PASTURE_LAYOUT = [
    # NW day 0: 4 around shed
    (4, 3), (5, 3), (3, 4), (4, 4),
    # NE / NW expansion days 1-5 (cows)
    (3, 3), (2, 3), (5, 2), (6, 3), (6, 4),
    # SW / SE after lands day 6-10
    (3, 5), (4, 5), (5, 5), (2, 5), (6, 5),
    (2, 4), (7, 4), (7, 3), (4, 6), (3, 6),
    (5, 6), (2, 6), (7, 5), (6, 6),
]
...
def plan_market(obs, farm, private):
    ...
    if day == 0 and hour == 1:
        # Seb-style aggressive day-0 opening: 7 hires, 2 cows + 2 sheep,
        # 12+2 wheat seed, 3 melon seed. Wheat bought through the day for feed.
        for _ in range(int(DNA["hires_day0"])):
            market.append(["HIRE"])
        market.append(["BUY_ANIMAL", "COW", 2])
        market.append(["BUY_SEED", "WHEAT", 12])
        market.append(["BUY_SEED", "MELON", 3])
        return market

    if day == 0 and hour == 2:
        # Second wave (after cows process): sheep + extra wheat seed.
    ...
    # ---- Land: buy late in the day (hour >= 18) after product sales have
    # generated cash. Seb buys NE/SW/SE at hours 22/6/17. ----
    if hour >= 18:
        if (day >= int(DNA["buy_ne_day"]) and "NE" not in unlocked(farm)
                and cash >= 1000):
            market.append(["BUY_LAND"])
            cash -= 1000
        if (day >= int(DNA["buy_sw_day"]) and "SW" not in unlocked(farm)
                and cash >= 2000):
            market.append(["BUY_LAND"])
            cash -= 2000
        if (day >= int(DNA["buy_se_day"]) and "SE" not in unlocked(farm)
                and cash >= 4000):
            market.append(["BUY_LAND"])
            cash -= 4000
    ...
    # Seb animal ramp: +1c d1, +1c d3, +1c d4, +2c+1s d9, +1s later d9,
    # +2c+2s d10, +2c+1s d11, +2c d13 -> 13 cows / 7 sheep.
    schedule_buy("COW", 1, 1)
    schedule_buy("COW", 1, 3)
    schedule_buy("COW", 1, 4)
    schedule_buy("COW", 2, 9)
    schedule_buy("SHEEP", 2, 9)
    schedule_buy("COW", 2, 10)
    schedule_buy("SHEEP", 2, 10)
    schedule_buy("COW", 2, 11)
    schedule_buy("SHEEP", 1, 11)
    schedule_buy("COW", 2, 13)

    # ---- Seed restocks (Seb buys strawberries/melons in waves after land opens) ----
    if day == 5 and seeds.get("STRAWBERRY", 0) < 11:
        n = 11 - seeds.get("STRAWBERRY", 0)
        market.append(["BUY_SEED", "STRAWBERRY", n])
    if day == 7 and seeds.get("STRAWBERRY", 0) < 9:
        market.append(["BUY_SEED", "STRAWBERRY", 9 - seeds.get("STRAWBERRY", 0)])
    if day == 8 and seeds.get("STRAWBERRY", 0) < 4:
        market.append(["BUY_SEED", "STRAWBERRY", 4 - seeds.get("STRAWBERRY", 0)])
    if day == 9 and seeds.get("STRAWBERRY", 0) < 7:
        market.append(["BUY_SEED", "STRAWBERRY", 7 - seeds.get("STRAWBERRY", 0)])
    if day == 11 and seeds.get("STRAWBERRY", 0) < 9:
        market.append(["BUY_SEED", "STRAWBERRY", 9 - seeds.get("STRAWBERRY", 0)])
    if day in (4, 8, 12, 16, 20, 24, 28):
        if seeds.get("WHEAT", 0) < 3:
            market.append(["BUY_SEED", "WHEAT", 3 - seeds.get("WHEAT", 0)])

    # ---- Buy wheat for feed (Seb buys ~1/day early, more after expansion) ----
    animals_now = count_animals(farm)
    wheat = shed.get("WHEAT", 0)
    need = max(0, animals_now + 4 - wheat)
    if need > 0 and cash >= 1:
        qty = 6 if day <= 2 else (18 if animals_now >= 10 else 10)
        market.append(["BUY_PRODUCT", "WHEAT", min(need, qty)])

    # ---- Surplus wheat sell ----
    reserve = int(DNA["feed_reserve"]) + animals_now
    if shed.get("WHEAT", 0) > reserve:
...
def choose_target(pos, farm, private, obs, carrying, is_farmer):
        ...
        if isinstance(t, dict):
            if t.get("animal"):
                ...
                if not t.get("watered_today"):
                    ...
                    harv.append((x, y))
        elif t is None and day < 25 and any(seeds.get(c, 0) > 0 for c in ("WHEAT", "MELON", "STRAWBERRY")):
            plant.append((x, y))
    ...
    if feed and carrying != "WHEAT":
        if any_unfed(farm):
            t, _ = nearest(pos, SHED_TILES)
            return t
    for bucket in (feed, ancare, water, harv, plant):
        if bucket:
...
def choose_crop(x, y, day, seeds, farm):
    ...
    if day <= 12 and seeds.get("MELON", 0) > 0:
        return "MELON"
    if day >= 7 and seeds.get("STRAWBERRY", 0) > 0 and d <= 4:
        return "STRAWBERRY"
