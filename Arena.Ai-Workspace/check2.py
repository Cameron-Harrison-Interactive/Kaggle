...
def check_positions(obs, configuration=None):
    ...
    if step in [716, 717, 718]:
        ...
        print(f"  Farmer: {farmer} shed={is_shed_access(farmer)}", flush=True)
        for i, h in enumerate(hands):
            print(f"  Hand{i}: {h} shed={is_shed_access(h)}", flush=True)
        ...
        print(f"  FERT price: ${prices.get('FERTILIZER', 0)}", flush=True)
