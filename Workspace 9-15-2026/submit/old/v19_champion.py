...
def agent(obs, configuration=None):
    ...
    def build_plan(n_workers, budget):
            ...
            for i, w in enumerate(workers):
                ...
                for pos_a, tile in mine:
                    ...
                    if (not tile.get("fed_today") and w["busy"] + d + 1 <= budget):
                        ...
                        w["pos"] = pos_a
                        d = 0
                    if tile.get("fertilizer_available") and w["busy"] + d + 1 <= budget:
                        ...
                        d = 0
                        maybe_drop(w)
                    if int(tile.get("yield_units", 0)) >= 1 and w["busy"] + d + 1 <= budget:
        ...
        for q in sorted(owned):
            for idx, t in enumerate(serpentine(q)):
                order_index[t] = idx
        crop_jobs.sort(key=lambda j: order_index.get(j[1], 999))
        ...
        # ---- 6. planting pass: fill empty crop tiles, plant + water ----
        for pos_e in sorted(crop_empty, key=lambda p: distance(p, shed_near(p))):
            crop = plan_crop_choice(vseeds)
            if crop is None:
                break
            best = None
            for w in workers:
                ...
                if w["busy"] + d + 2 <= budget and (best is None or d < best[0]):
                    best = (d, w)
            if best is None:
                dropped += 1
                continue
    ...
    if (state.get("plan_day") != day
        ...
        for n in range(max(1, len(positions)), 15):
            p_try, dropped = build_plan(n, budget)
    ...
    for i in range(len(positions)):
        ...
        while act is None and ptr < len(plan_i):
            ...
            if op == "WATER":
                ...
                ok = tile is None and free_seeds.get(arg, 0) > 0
            elif op == "INSTALL":
