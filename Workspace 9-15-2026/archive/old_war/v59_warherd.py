...
def plan_market(S, day):
    ...
    herd = n_an + sum(shed.get(k, 0) for k in ("COW", "SHEEP", "GOOSE"))
    targets["WHEAT"] = max(targets.get("WHEAT", 0), min(44, 12 + 2 * herd))
    if 18 <= day <= 26:
    ...
    # ---- 4) WAR HERD 6/6/8 (user directive): geese = uncrashable egg anchor;
    # cows/sheep = fert engine + milk/wool denial vs animal metas ----
    goose_t = min(8, max(0, (day) // 1)) if day <= 8 else 8
    tgt = {"GOOSE": (goose_t if day >= 2 else 0),
           "COW": (min(6, max(1, day - 1)) if 2 <= day <= 7 else 6),
           "SHEEP": (min(6, max(0, day - 3)) if 3 <= day <= 9 else 6)}
    have = {}
    for a in animals:
