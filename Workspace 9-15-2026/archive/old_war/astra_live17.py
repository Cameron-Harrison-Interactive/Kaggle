...
def agent(obs, configuration=None):
    ...
    for pos, tile in plants:
        ...
        if ready:
            ...
            # VOLUME FIX (measured): ongoing crops (TOMATO/STRAWBERRY) keep
            # their EOD +1 production regardless of watering (engine L799-800:
            # only the fertilizer +2 requires a watered day) and survive on
            # every-other-day water (skip at cuw=0, refresh -> 1, water
            # tomorrow at cuw=1; death needs cuw>=2). Tetsu runs 1115
            # waters/season for 57 standing; our daily watering consumed the
            # worker slack PLANT (priority 340, slack-only) needs. Fresh
            # plantings still water same-day: _new_plant starts cuw=1 ->
            # dry=1 -> 1400 job, plus the mandatory PLANT->WATER follow-up.
            # Finite crops keep daily watering (yield gains land on watered
            # days inside the gain window). Note: no `continue` here — the
            # FERTILIZE append below must still run.
            if not (tile["crop"] in ("TOMATO", "STRAWBERRY") and dry < 1):
                # Due-today water (dry >= 1) is a hard deadline: unserved by
                # EOD the tile dies at refresh (measured: 2-6 strb deaths/
                # night at flat 1400 — far tiles lose to nearer jobs all day
                # until cost > remaining drops them). Escalate with the hour
                # so deadline service dominates as the day runs out.
                prio = (1400 + 40 * hour) if dry >= 1 else 740
                jobs.append(("WATER", pos, None, prio))
    ...
    if (
        ...
        # Crisis count must match the watering schedule: ongoing crops sit
        # at cuw=1 every other morning BY DESIGN (due today, served at 1400)
        # — counting them blocked every BUY_LAND d4-20 in the first live17
        # run (owned=1 through d20, $34k seed 101). Only finite crops with
        # cuw>=1 are genuine neglect.
        urgent = sum(
    ...
    try:
        with open("/tmp/astra_econ17.txt", "a") as _lf:
            _lf.write(
