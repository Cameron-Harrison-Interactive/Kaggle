...
def build_tasks(S, day):
    ...
    for a in S["animals"]:
        ...
        if not a["collected"] and (day % 2 == 0 or (S["shed"].get("FERTILIZER", 0) < 20)):
            tasks.append(("COLLECT", a["pos"], None))   # fert every other day is plenty
...
def plan_day(S, day):
    ...
    if ani:
        ...
        n_servers = max(2, min(6, (len(ani) + 2) // 3))
        ...
        for si, u in enumerate(servers):
            ...
            routes[u].append(("PICKUP", _shed_near(spawn[u] or (4, 4)), "WHEAT",
                              min(6, len(mine))))
            # HARVEST FIRST: product at max_held is active production loss;
            # feed/care/collect are maintenance and can trail.
            for a in mine:
                if a["yield"] >= (2 if a["kind"] == "GOOSE" else 3):
            ...
            for a in mine:
                ...
                if day % 2 == 0 or (S["shed"].get("FERTILIZER", 0) < 20):
    ...
    def unit_step(i, h):
        ...
        while ptr[i] < len(rt):
            ...
            if kind == "HARVESTC":
                ...
                if u == pos:
                    pl = sim["plant_at"][pos]
                    if sim.get("harv_units", 0) + pl["yield"] > 96:
                        ptr[i] += 1; continue
        ...
        for t in backlog:
            ...
            if kind == "PLANT":
                ...
                if avail <= 0:
                    ...
                    continue
                if h >= 23:
                    continue              # h23 plant still gets EOD water credit? no —
                # h22 plants get watered same day by the WATER task we insert
                t[3] = i
                if u == pos:
