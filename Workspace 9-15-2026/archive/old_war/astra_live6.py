...
def agent(obs, configuration=None):
    ...
    def crop_score(crop, projected_counts=None):
        ...
        # ONGOING crops (strb/tomato) sell gradually over their life; charging
        # the full 0.65x lifetime supply up-front over-penalizes the exact
        # high-base volume plays the top metas profit from (TRACE-04: this
        # chassis never planted strb/tomato once in 30 days). Reduced to 0.30x
        # for ongoing crops only; one-time crops unchanged.
        own_factor = .30 if crop in ("TOMATO", "STRAWBERRY") else .65
        own_supply = n[crop] * units * own_factor
    ...
    try:
        with open("/tmp/astra_econ6.txt", "a") as _lf:
            _lf.write(
