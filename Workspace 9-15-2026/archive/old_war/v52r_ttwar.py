...
def _tt_agent(observation, configuration=None):
...
# ==== v52p war overlay: tape orders verbatim + evening/endgame war sells ====
WAR_ITEMS = ("MELON", "STRAWBERRY", "MILK", "WOOL", "EGG", "CARROT", "TOMATO")

def agent(obs, config=None):
    base = _tt_agent(obs, config)
    p = int(obs["player"])
    ...
    n_an = 0
    fm = obs.get("farms", [])[p] if len(obs.get("farms", [])) > p else None
    ...
    # ALL tape orders verbatim (never displace hires/buys/sells)
    orders = [list(o) for o in base.get("market", [])][:10]
    ...
    if day >= 29:
        for it in WAR_ITEMS:
            if room() <= 0:
                break
            orders.append(["SELL", it, 10 ** 6])
        if shed.get("FERTILIZER", 0) > 20 and room() > 0:
            orders.append(["SELL", "FERTILIZER", shed["FERTILIZER"] - 20])
        n = shed.get("WHEAT", 0) - keep_w
        if n > 0 and room() > 0:
            orders.append(["SELL", "WHEAT", n])
    elif hour >= 1:
        # all-hours standing sells: convert yesterday's shed (and intraday
        # drops) ~20h earlier than the tape's own h20-23 window — lockstep
        # 1:1 means earlier volume takes the better price. Fert stays
        # protected (the tape's units PICKUP it to fertilize fields).
        for it in WAR_ITEMS:
            if room() <= 0:
                break
            orders.append(["SELL", it, 10 ** 6])
