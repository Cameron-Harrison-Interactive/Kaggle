...
def plan_market(S, day):
    ...
    if day >= 28:
        ...
        keep_w = max(4, n_an)  # 1-day reserve only
    else:
        keep_w = max(8, n_an * 2)
    ...
    # ---- 2.5) WAR SELLS (v49: volume + timing beat solo pacing; playbook §8b) ----
    # Shared market, shed-direct execution, lockstep 1:1 per unit: sell max at
    # the earliest indices. The dump floods items the enemy relies on; our own
    # first waves still sell near the top of the price curve.
    ...
    # fert beyond today's use: crashes to $1 fast; early units are real money
    qsell("FERTILIZER", max(0, shed.get("FERTILIZER", 0) - fert_use))
    # full dump of every product, every morning (first-seller + glut pressure)
    for it in ("MELON", "STRAWBERRY", "MILK", "WOOL", "EGG", "CARROT", "TOMATO"):
        qsell(it, shed.get(it, 0))
    # wheat flood: all above the feed reserve. Log cliff floors wheat ~$17-20,
    # so our volume still pays while capping the enemy's late $35-46 premium.
    qsell("WHEAT", max(0, shed.get("WHEAT", 0) - keep_w))
    # shed-capacity safety (overflow at EOD is discarded = pure loss)
    if shed.get("WHEAT", 0) > 90:
