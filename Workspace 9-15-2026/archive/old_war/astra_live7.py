...
def agent(obs, configuration=None):
    ...
    # ---------- Crew target (moved above market: the sell loop must reserve
    # morning order slots for hires — TRACE-04 measured crew=3 at game end
    # because hourly SELLs consumed all 10 order slots in the h0-3 window) ----
    ...
    # reserve up to 3 order slots for HIREs during the morning hire window
    sell_slot_cap = max_orders
    if hour <= 3 and not final_day:
        need_hires = max(0, desired_hands
                         - int(farm.get("hires_today", len(positions) - 1)))
        sell_slot_cap = max_orders - min(3, need_hires)
    for item in sell_order:
        if len(orders) >= sell_slot_cap:
            break
        qty = sellable[item]
    ...
    # (desired_hands computed above the market section; the sell loop now
    # reserves morning slots so these HIRE orders actually fit.)
    ...
    if day >= 2 and not final_day:
        ...
        # STRB TREADMILL QUOTA (TRACE-04: the score-max pipeline buys only
        # the top scorer — melon — every slot, so the highest-base ongoing
        # crop is never planted; the $200k metas run strb as their #2 income).
        # Reserve part of each day's budget d6-12 toward a 34-tile strb book;
        # planted d6-12 (horizon gate open), produces every 2 days through d29.
        strb_standing = counts["STRAWBERRY"] + seeds["STRAWBERRY"]
        if 6 <= day <= 12 and strb_standing < 34:
            strb_room = min(12 - standing_seeds,
                            room + standing_seeds,
                            34 - strb_standing)
            if strb_room > 0 and virtual_cash >= strb_room * 100 + 600:
                if len(orders) < max_orders:
                    orders.append(["BUY_SEED", "STRAWBERRY", strb_room])
                    virtual_cash -= strb_room * 100
                    seeds["STRAWBERRY"] += strb_room
                    standing_seeds += strb_room
    ...
    # ---------- Land: expansion gate (TRACE-03 fix) ----------
    # Measured on the traced run: the density>=.80 AND day<=17 interlock passed
    # only 2 of 120 qualifying decisions — farm capped at 2 quads while cash
    # idled at $21-45k. Keep the original density path; allow a cash-rich
    # path (money >= cost + 8000) and extend the day cap to 21 so melon-sale
    # cash (d13-17) can fund quads 3-4.
    if (
        ...
        if ((density >= .80 or money >= land_cost + 8000)
            ...
            land_ok = True
            buy_order(["BUY_LAND"], land_cost, 1600)

    try:
        with open("/tmp/astra_econ7.txt", "a") as _lf:
            _lf.write(
