...
def agent(obs, configuration=None):
    ...
    # Workers already at their previous target, or holding supplies, go first.
    def worker_order(i):
        old = old_goals.get(i)
        at_goal = bool(old and positions[i] == old[1])
        inv = inventories[i]
        supplied = inv.get("WHEAT", 0) + inv.get("FERTILIZER", 0)
        return (not at_goal, -supplied, i)
    ...
    # ---------- Land: purchase only when an existing crop block is dense ----------
    if (
        hour <= 3 and 1 <= len(owned) < 4 and day <= 17
        ...
        if density >= .80 and urgent <= max(2, len(positions)):
