...
def agent(obs, configuration=None):
    ...
    if day >= 2 and not final_day:
        ...
        # live13 lesson: a bigger standing stock helps the post-dump fill
        # (standing 38-51 vs collapse) but its early argmax buys compete
        # with the strb quota budget. Protect the factory build window;
        # unthrottle once the factory is stocked.
        seed_cap = (
            12 if (1 <= day <= 9
                   and counts["STRAWBERRY"] + seeds["STRAWBERRY"] < STRB_TARGET)
            else 2 * (desired_hands + 1)
        )
        need = max(0, min(seed_cap, slots, room) - standing_seeds)
    ...
    try:
        with open("/tmp/astra_econ16.txt", "a") as _lf:
            _lf.write(
