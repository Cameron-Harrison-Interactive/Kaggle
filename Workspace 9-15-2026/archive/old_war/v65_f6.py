...
def plan_market(S, day):
    ...
    # CREW: work-driven, fib-cheap zone (8 = $54/d, 10 = $143/d, 12 = $376/d,
    # 14 = $986/d — hires are priced DAILY). The user's 3/quad is the CAP;
    # the day's counted work sets the target. Never hire hands without tasks.
    nq_ = len(S["quadrants"])
    quad_cap = 3 * nq_ + max(0, -(-(max(0, n_an - 5 * nq_)) // 5))
    try:
        _tk = build_tasks(S, day)
    except Exception:
        _tk = []
    work_est = len(_tk) + 10
    peak_crew = -(-int(work_est * 2.0) // 20)
    want = max(8, min(10, peak_crew, max(8, quad_cap)))
    if day <= 2:
        want = 8
    ...
    # USER OPENING: 3 geese day 1 (fert $100 x3/day from d2 + eggs = expansion
    # capital that cannot be crashed). Herd grows stage by stage AFTER cash
    # flows: cows d6+, sheep d10+. Never day-0 animals; never a $60 feed panic.
    goose_t = 3 if day >= 1 else 0
    if day >= 6:
        goose_t = min(6, 3 + (day - 6) // 3)
    cow_t = min(4, day - 5) if day >= 6 else 0
    sheep_t = min(4, day - 9) if day >= 10 else 0
