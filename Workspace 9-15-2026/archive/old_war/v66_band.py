...
def plan_market(S, day):
    ...
    # MATH-DRIVEN CREW: 2.87 ops/animal/day measured -> ~5 animals per hire;
    # field tasks ~1.5h each; never hire what isn't tasked.
    n_servers = max(2, -(-n_an // 5)) if n_an else 0
    field_est = sum(1 for pl in S["plants"] if not pl["watered"]) + len(S["weeds"]) + 6
    field_crew = -(-int(field_est * 1.5) // 20)
    want = max(8, min(14, 1 + n_servers + field_crew))
    if day <= 1:
        want = 8
...
def plan_day(S, day):
    ...
    # ---- v66 BAND ROUTER (user + astra design): mirrored shed-anchored
    # bands, complete tile-visit bundles, TIME-BUDGETED routes with RELIEF.
    # A unit fills its route to budget (~19h) taking nearest bundles — own
    # quad first, then any quad (relief: finished C-band units pull A/B
    # overflow; nobody PASSes while claimable work exists). Leftover
    # CRITICAL bundles (feed/deadline water) are appended to the least-loaded
    # units — established stock is never sacrificed. Leftover optional work
    # (fert/dig) is dropped. Care/feed stay DAILY (the care bank). ----
    routes = {i: [] for i in range(n_units)}

    def _qof(p):
        return ("N" if p[1] < 5 else "S") + ("W" if p[0] < 5 else "E")

    # 1) bundle tasks into complete tile visits
    bund = {}
    for (kind, pos, arg) in tasks:
        bund.setdefault(pos, []).append((kind, arg))
    ORDER = {"FEED": 0, "HARVESTA": 1, "CARE": 2, "COLLECT": 3,
             "HARVESTC": 4, "WATER": 5, "FERTILIZE": 6, "DIGP": 7}
    cuw_at = {pl["pos"]: pl["cuw"] for pl in S["plants"]}
    unfed_at = {a["pos"]: a["unfed"] for a in S["animals"]}
    crit = {p: (any(k == "FEED" for k, _ in v) and unfed_at.get(p, 0) >= 1)
                 or cuw_at.get(p, 0) >= 1
            for p, v in bund.items()}
    for p in bund:
        bund[p].sort(key=lambda e: ORDER.get(e[0], 8))

    # 2) unit -> quad (proportional to load; spawn = shed corner)
    owned_q = sorted(S["quadrants"]) or ["NW"]
    qload = {}
    for p in bund:
        qload[_qof(p)] = qload.get(_qof(p), 0) + 1
    assign = {}
    counts = {q: 0 for q in owned_q}
    for i in range(n_units):
        q = min(owned_q, key=lambda qq: (counts[qq] / (1 + qload.get(qq, 0)),
                                         owned_q.index(qq)))
        assign[i] = q
        counts[q] += 1

    # 3) time-budgeted packing with relief
    BUDGET = 19
    ucost = {i: 0 for i in range(n_units)}
    upos = {i: (spawn[i] or (4, 4)) for i in range(n_units)}
    unclaimed = set(bund)
    for i in range(n_units):
        while True:
            own = [p for p in unclaimed if _qof(p) == assign[i]]
            cands = own if own else list(unclaimed)
            if not cands:
                break
            best = min(cands, key=lambda p: _d(upos[i], p))
            step = _d(upos[i], best)
            act = len(bund[best])
            if routes[i] and ucost[i] + step + act > BUDGET:
                # RELIEF: take any feasible nearby bundle that still fits
                fits = [p for p in unclaimed
                        if ucost[i] + _d(upos[i], p) + len(bund[p]) <= BUDGET]
                if not fits:
                    break
                best = min(fits, key=lambda p: _d(upos[i], p))
                step = _d(upos[i], best)
                act = len(bund[best])
            for (k, a) in bund[best]:
                routes[i].append((k, best, a))
            ucost[i] += step + act
            upos[i] = best
            unclaimed.discard(best)

    # 4) leftover criticals -> least-loaded units (never sacrifice stock)
    for p in sorted(unclaimed, key=lambda p: (not crit.get(p, False), _d((4, 4), p))):
        if crit.get(p, False):
            i_min = min(range(n_units), key=lambda i: ucost[i])
            routes[i_min].extend((k, p, a) for (k, a) in bund[p])
            ucost[i_min] += len(bund[p])
            unclaimed.discard(p)
    # noncritical leftovers stay unclaimed (dropped this day)

    # 5) supply pickups at route start (wheat for FEED, fert for FERTILIZE)
    for i in range(n_units):
        n_feed = sum(1 for (k, _, _) in routes[i] if k == "FEED")
        n_fert = sum(1 for (k, _, _) in routes[i] if k == "FERTILIZE")
        head = []
        sh = _shed_near(spawn[i] or (4, 4))
        if n_feed:
            head.append(("PICKUP", sh, "WHEAT", min(6, n_feed)))
        if n_fert:
            head.append(("PICKUP", sh, "FERTILIZER", min(6, n_fert)))
        routes[i] = head + routes[i]
    ...
    for kind in bought_kinds:
        ...
        spots = [c for c in sorted(sim["empty_at"],
                 key=lambda c: (min(_d(c, s2) for s2 in (reserved or {(4, 4)})), _d(c, (4, 4))))
                 if c not in reserved and c not in SHED_TILES]
        if not spots:
            break
        c = spots[0]
        reserved.add(c)
