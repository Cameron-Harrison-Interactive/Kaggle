...
def agent(obs, configuration=None):
    ...
    STRB_TARGET = 33
    # TRACE-05: wheat backbone 24-42 standing all game (~$1.2k/day at rising
    # prices 27->50); argmax never selects wheat while melon/carrot/strb
    # dominate the score. Seeds cost $10, so this does not compete with the
    # strb budget (the live13 lesson).
    WHEAT_TARGET = 24
    ...
    def output_for(job, pos, inv):
        ...
        if op == "INSTALL":
            ...
            if (1 <= day <= 16 and counts["STRAWBERRY"] < STRB_TARGET
                    and free_seeds["STRAWBERRY"] > 0):
                crop = "STRAWBERRY"
            elif (day <= 25 and counts["WHEAT"] < WHEAT_TARGET
    ...
    # ---------- Wheat backbone quota funding ----------
    # Cheap seeds ($10), spend-to-floor, after the strb quota. Window ends
    # d25 (wheat horizon blocks d27+; d25 planting still yields by d27-28).
    if 2 <= day <= 25 and not final_day:
    ...
    try:
        with open("/tmp/astra_econ14.txt", "a") as _lf:
            _lf.write(
