"""
Kaggriculture v5.7m — outside-first strict route patrol test.
...
def lane_local_path(lane, h):
    """Adjacent local route paths for design #1.

    Every consecutive point is cardinal-adjacent. This avoids the back-and-forth
    bouncing caused by diagonal/jump gaps in previous inner-route lists.
    """
    if h <= 1:
        return [(0, 0)]
    if lane == 0:
    ...
    if lane == 1:
        ...
        y_far = h - 2
        pts = []
        for ay in range(0, y_far + 1):
            pts.append((c1, ay))
        for ax in range(c1 + 1, c2 + 1):
            pts.append((ax, y_far))
        for ay in range(y_far - 1, -1, -1):
            pts.append((c2, ay))
        for ax in range(c2 - 1, c1, -1):
            pts.append((ax, 0))
        return pts
    # Inner/shed-side route: small adjacent loop from entrance corner.
    m = min(2, h - 1)
    pts = []
    for ax in range(0, m + 1):
        pts.append((ax, 0))
    for ay in range(1, m + 1):
        pts.append((m, ay))
    for ax in range(m - 1, -1, -1):
        pts.append((ax, m))
    for ay in range(m - 1, 0, -1):
        pts.append((0, ay))
    return pts
...
def route_work_match(idx, target, board_size):
    """Whether this tile is part of the worker's assigned WORK segment.

    A route may pass through inner tiles to reach the outside. Those pass-through
    tiles should not be planted by the outer worker, otherwise it never reaches
    the outside-first path the user designed.
    """
    if idx <= 0:
        return True
...
def agent(obs):
    try:
        ...
        if not is_dump and hour <= 20 and len(market) < 10:
            ...
            # Exactly 3 hired hands per active/pending quadrant. Main farmer is the +1 floater.
            desired_hands_pre = min(int(DNA.get("target_hires", 12)), 3 * managed_quadrants)
            # Leave at least two market slots for seed/sell orders.
