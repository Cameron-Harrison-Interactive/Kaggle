"""
Kaggriculture v5.8z2f — shed-side restored + late fast crops.
...
LAST_PLANT_DAY = {"MELON": 16, "STRAWBERRY": 14, "TOMATO": 20, "WHEAT": 25, "CARROT": 26}
...
def is_trimmed_side_column(pos, board_size):
    ...
    Keep the shed-side/closest column because it is cheap to work. Trim only the
    far outside column so 2 workers cover 20 useful tiles per quadrant.
    """
    if pos is None:
        return False
...
def lane_local_path(lane, h):
    ...
    Far/fence-side column ax=h-1 is ignored. The shed-side column ax=0 is used.
    lane 0 covers ax 0-1. lane 1 covers ax 2-3.
    """
    if h <= 2:
        return [(0, 0)]
    max_work_col = h - 2
...
def agent(obs):
    try:
        ...
        if not is_dump:
            ...
            for crop in CROPS:
                # Slow crops stop early; fast crops can fill empty dirt until their true last day.
                if crop in ("WHEAT", "CARROT"):
                    ...
                    stop_buy_day = LAST_PLANT_DAY[crop] - 2
                if day >= stop_buy_day:
                    deficits[crop] = 0
                else:
            ...
            if day <= 1:
                ...
                # Endgame fill: use fast crops on any remaining empty route tile.
