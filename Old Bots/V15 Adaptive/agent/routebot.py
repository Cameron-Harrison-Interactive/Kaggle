...
VERSION = "HI_AgriBot_v11.0_Seasonscript"
...
def assign_jobs(obs, farm, private, jobs, ctx):
    """ROUTE ENGINE: stable beats instead of a churning job board.
    Survival overrides detach units; position-derived rejoin is automatic."""
    day = obs.get("day", 0)
    hour = obs.get("hour", 0)
    seeds = private.get("seeds", {}) or {}
    shed = private.get("shed", {}) or {}
    ...
    units = [{"idx": i, "pos": tuple(p), "inv": inv_list[i] if i < len(inv_list) else {}}
    ...
    # ---------------- 1. SURVIVAL OVERRIDES (hard priority) ----------------
    unfed_urgent, water_urgent = [], []
    for (x, y) in _owned_tiles(farm):
        t = _tile(farm, x, y)
        if isinstance(t, dict):
            if t.get("animal") and not t.get("fed_today") \
                    and (t.get("consecutive_unfed", 0) or 0) >= 1:
                unfed_urgent.append((x, y))
            elif t.get("kind") == "PLANT" and not t.get("watered_today") and day < 29:
                water_urgent.append((x, y))
    # water most-stressed first so nothing weeds out
    def _wc(p):
        t = _tile(farm, p[0], p[1])
        cu = (t.get("consecutive_unwatered", 0) or 0) if isinstance(t, dict) else 0
        return -cu
    water_urgent.sort(key=_wc)
    wheat_pool = shed.get("WHEAT", 0) + sum(u["inv"].get("WHEAT", 0) for u in units)
    for tile in unfed_urgent:
        free = free_units()
        if not free:
            break
        ...
        if carriers:
            ...
            u = min(free, key=lambda u: _shed_dist(u["pos"]))
            if _shed_dist(u["pos"]) == 0:
    ...
    # ---------------- 2. ANIMAL HOUSING (shed animals need homes) ----------
    waiting = sum(shed.get(k, 0) for k in ("COW", "SHEEP", "GOOSE"))
    if waiting > 0:
        empty_past = _empty_pastures(farm)
        ...
        housing_duty = 0
        for u in free_units():
            if waiting <= 0 or housing_duty >= 2:
                break
            ...
            if held:
                if empty_past:
                    ...
                    if u["pos"] == tgt:
                        ...
                        housing_duty += 1
            elif empty_past and _shed_dist(u["pos"]) == 0:
    ...
    tenders = set(u["idx"] for u in units[:min(3, n_units)])
    # tender beat: pasture tiles + shed, ordered in a stable loop
    pastures = [(x, y) for (x, y) in _owned_tiles(farm)
                if isinstance(_tile(farm, x, y), dict)
                and _tile(farm, x, y).get("kind") in ("PASTURE", "COOP")]
    pastures.sort(key=lambda p: (math.atan2(p[1] - 4.5, p[0] - 4.5), p))
    tender_route = pastures + list(SHED_TILES)
    # workers: quadrant serpents with phase offsets
    workers = [u for u in units if u["idx"] not in tenders]
    routes = {}
    for i, u in enumerate(workers):
        q = active_quads[i % len(active_quads)] if active_quads else "NW"
        routes[u["idx"]] = ("lane", q, (i * 5) % 25)
    for u in units[:min(3, n_units)]:
        routes[u["idx"]] = ("corridor", None, (u["idx"] * 3) % max(1, len(tender_route)))

    phase = MEM.setdefault("route_phase", {})
    for u in units:
        if u["idx"] in assigned:
            continue
        inv = u["inv"]
        load = sum(v for k, v in inv.items()
                   if v > 0 and k not in ("WHEAT", "COW", "SHEEP", "GOOSE"))
        # heavy haulers break beat to drop at the shed
        if load >= 4:
            if _shed_dist(u["pos"]) == 0:
                assigned[u["idx"]] = ["DROP"]
            else:
                ...
                assigned[u["idx"]] = _move_toward(u["pos"], t2)
            continue
        kind, q, off = routes.get(u["idx"], ("lane", active_quads[0] if active_quads else "NW", 0))
        # persist phase so beats don't reset (stable across turns)
        key = str(u["idx"])
        i_here = phase.get(key, off)
        if kind == "corridor":
            route = tender_route if tender_route else list(SHED_TILES)
        else:
            route = quad_route(q)
        L = len(route)
        i_here = i_here % L
        # rejoin: if far from the beat point, walk back to it first
        target = route[i_here]
        if _manhattan(u["pos"], target) > 2:
            assigned[u["idx"]] = _move_toward(u["pos"], target)
            continue
        if u["pos"] != target:
            assigned[u["idx"]] = _move_toward(u["pos"], target)
            continue
        # standing on the beat tile: do whatever it needs
        ...
        if isinstance(t, dict):
            if t.get("animal"):
                ...
                elif not t.get("cared_today") and day >= 6 and day <= 28:
                    act = ["CARE"]
                elif not t.get("fed_today") and shed.get("WHEAT", 0) > 0 and kind == "corridor":
                    if _shed_dist(u["pos"]) == 0:
                        act = ["PICKUP", "WHEAT", 6]
                    else:
                        t2, _ = _nearest(u["pos"], SHED_TILES)
                        act = _move_toward(u["pos"], t2)
            elif t.get("kind") == "PLANT":
                if (t.get("yield_units") or 0) > 0:
                    crop = t.get("crop")
                    ready = crop not in FIRST_YIELD_DAY or (day - t.get("planted_day", day)) >= FIRST_YIELD_DAY[crop]
                    if crop in PEAK_AGE:
                        age = day - t.get("planted_day", day)
                        if age >= PEAK_AGE[crop] or day >= 27:
                            act = ["HARVEST"]
                ...
                if act is None and not t.get("watered_today") and day < 29:
                    act = ["WATER"]
            elif t.get("kind") == "WEED" and day < 28:
                act = ["DIG"]
        elif t is None and day <= 26 and hour <= 18 and kind == "lane":
        ...
        if act:
            ...
            # advance the beat
            phase[key] = (i_here + 1) % L
            nxt = route[(i_here + 1) % L]
            assigned[u["idx"]] = _move_toward(u["pos"], nxt) if u["pos"] != nxt else ["PASS"]
...
def agent(obs):
    try:
        ...
        with open("/tmp/rb_err.log", "a") as _f:
            _f.write(f"day {obs.get('day')} hour {obs.get('hour')}\n" + _tb.format_exc() + "\n")
        return {"farmer": ["PASS"], "hands": [], "market": []}
