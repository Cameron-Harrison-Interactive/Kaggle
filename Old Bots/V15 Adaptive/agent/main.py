"""HI_AgriBot_v14.0_Portfolio — v26 seat-aware current-meta portfolio for Kaggriculture.
Forked from kaitofukami v26 (routes: Yubo WANG seat0, Gbining seat1) with attribution.
...
"""
VERSION = "HI_AgriBot_v15_AdaptivePortfolio"
import base64
import copy
import json
import math
...
def _feed_collect_optimizer(obs, action):
    try:
        farm=_farm(obs,_seat(obs))
        tiles=_get(farm,"tiles",[]) or []
        private=_get(obs,"private",{}) or {}
        ...
        for i,(pos,act) in enumerate(zip(positions, unit_actions)):
            if not pos or not act:
                continue
            if act[0] not in ("PASS",):
                continue  # only fix idle PASS, not steal from WATER/MOVE
            try:
                x,y=int(pos[0]),int(pos[1])
                tile=tiles[y][x] if 0<=y<len(tiles) and 0<=x<len(tiles[y]) else None
            except:
            ...
            if isinstance(tile,dict) and "animal" in tile:
                if tile.get("fertilizer_available"):
                    unit_actions[i]=["COLLECT_FERTILIZER"]
                    continue
                if not tile.get("fed_today") and tile.get("consecutive_unfed")==1:
                    inv=inventories[i] if i<len(inventories) and isinstance(inventories[i],dict) else {}
                    has_wheat=inv.get("WHEAT",0)>0
                    # also check shed has wheat? worker could pickup, but if no wheat in inv, can't feed
                    if has_wheat:
                        unit_actions[i]=["FEED","WHEAT"]
                        continue
        action["farmer"]=unit_actions[0] if unit_actions else ["PASS"]
        action["hands"]=unit_actions[1:]
        return _align_hands(action, obs)
...
def _harvest_pass_optimizer(obs, action):
    try:
        farm=_farm(obs,_seat(obs))
        tiles=_get(farm,"tiles",[]) or []
        ...
        if len(unit_actions)<len(positions):
            unit_actions+=[["PASS"]]*(len(positions)-len(unit_actions))
        for i,(pos,act) in enumerate(zip(positions, unit_actions)):
            if not pos or not act or act[0]!="PASS":
                continue
            try:
                x,y=int(pos[0]),int(pos[1])
                tile=tiles[y][x] if 0<=y<len(tiles) and 0<=x<len(tiles[y]) else None
...
def _locked_quad_guard(obs, action):
    try:
        farm=_farm(obs,_seat(obs))
        tiles=_get(farm,"tiles",[]) or []
        board_size=len(tiles) or 10
        shed_allowed=set(_shed_access_tiles(board_size))
        positions=[_get(farm,"farmer"), *list(_get(farm,"hands",[]) or [])]
        unit_actions=[action.get("farmer",["PASS"]), *list(action.get("hands") or [])]
        if len(unit_actions)<len(positions):
            unit_actions+=[["PASS"]]*(len(positions)-len(unit_actions))
        for i,(pos,act) in enumerate(zip(positions, unit_actions)):
            if not pos or not act or act[0] not in ("NORTH","SOUTH","EAST","WEST"):
                continue
            try:
                x,y=int(pos[0]),int(pos[1])
                nx,ny=x,y
                if act[0]=="NORTH": ny-=1
                elif act[0]=="SOUTH": ny+=1
                elif act[0]=="EAST": nx+=1
                elif act[0]=="WEST": nx-=1
                if 0<=ny<len(tiles) and 0<=nx<len(tiles[0]):
                    if tiles[ny][nx]=="LOCKED" and (nx,ny) not in shed_allowed:
                        unit_actions[i]=["PASS"]
            except:
                pass
        action["farmer"]=unit_actions[0] if unit_actions else ["PASS"]
...
def _tomato_hedge(obs, action):
    try:
        day=int(obs.get("day", obs.get("step",0)//24))
        if 7 <= day <= 13:
            market=obs.get("market",{}) or {}
            inv=market.get("inventory",{}) or {}
            prices=market.get("prices",{}) or {}
            straw_inv=inv.get("STRAWBERRY",10000)
            straw_price=prices.get("STRAWBERRY",120)
            # earlier trigger: d7-13, if straw already trending glut
            surge = straw_inv > 10050 or straw_price < 100
            # also check momentum: if d10 price <110 and inv>10020
            if surge:
                private=obs.get("private",{}) or {}
                seeds=private.get("seeds",{}) or {}
                farm=obs["farms"][obs["player"]]
                money=farm["money"]
                # buy tomato if needed
                if seeds.get("TOMATO",0)==0 and money>200 and len(action.get("market",[]))<10:
                    # don't exceed order cap, add buy
                    market_orders=list(action.get("market") or [])
                    # avoid duplicate buys
                    if not any(m and m[0]=="BUY_SEED" and m[1]=="TOMATO" for m in market_orders):
                        market_orders.append(["BUY_SEED","TOMATO",5])
                        action["market"]=market_orders[:10]
                # convert plants: need tomato seeds
                private=obs.get("private",{}) or {}
                seeds=private.get("seeds",{}) or {}
                if seeds.get("TOMATO",0)>0:
                    ...
                    for i,h in enumerate(hands):
                        if h and h[0]=="PLANT" and h[1]=="STRAWBERRY" and conv<2:
                            hands[i]=["PLANT","TOMATO"]
                            conv+=1
                    action["hands"]=hands
                    if action.get("farmer") and action["farmer"][0]=="PLANT" and action["farmer"][1]=="STRAWBERRY":
                        action["farmer"]=["PLANT","TOMATO"]
    except: pass
    return action


_SEAT_TOMATO = _SEAT0_ACTIONS  # alias for clarity, will be 14.6 base
_SEAT_STRAW = _SEAT1_ACTIONS  # use seat1 as straw heavy variant (actually 14.5 is similar, but we reuse)
_ADAPTIVE_CHOICE = {}
def _adaptive_select(obs):
    # classify opp at day 2-3
    try:
        seat=_seat(obs)
        opp=1-seat
        ...
        if opp >= len(farms):
            return 0
        opp_farm=farms[opp]
        # count opp strawberry
        opp_straw=sum(1 for row in opp_farm.get("tiles",[]) for t in row if isinstance(t,dict) and t.get("kind")=="PLANT" and t.get("crop")=="STRAWBERRY")
        opp_wheat=sum(1 for row in opp_farm.get("tiles",[]) for t in row if isinstance(t,dict) and t.get("kind")=="PLANT" and t.get("crop")=="WHEAT")
        # market surge
        market=obs.get("market",{}) or {}
        inv=market.get("inventory",{}) or {}
        straw_inv=inv.get("STRAWBERRY",10000)
        # if opp straw heavy (>15) or market straw high, go tomato (1)
        if opp_straw > 12 or straw_inv > 10080:
            return 1
        if opp_wheat > 25:
            # vs Seb wheat heavy, go strawberry
            return 0
        return 0
    except:
        return 0

def _adaptive_agent(obs, configuration=None):
    step=int(obs.get("step",0) or 0)
    seat=_seat(obs)
    # choose at day 2 and lock
    if step < 48:
        choice=_adaptive_select(obs)
        _ADAPTIVE_CHOICE[seat]=choice
    else:
        choice=_ADAPTIVE_CHOICE.get(seat,0)
    # delegate to base logic but with choice (for now both use same base, but we can vary tomato hedge)
    # if choice==1, ensure tomato hedge active (already in base), else disable tomato conversion
    # we will call base agent and then if choice==0, revert any tomato plants back to strawberry?
    # simpler: just call base agent (which is tomato hedge) for both
...
def _base_agent(obs, configuration=None):
    try:
        ...
        action = _tomato_hedge(obs, action)
        # _feed_collect disabled - already inside water animal check
        # _harvest_pass disabled - end wheat left is intentional low value
...
def agent(obs, configuration=None):
    # adaptive portfolio: at step<48 choose, then delegate
    try:
        # ensure choice locked
        step0=int(_get(obs, "step", 0) or 0)
        if step0 < 48:
            _adaptive_select(obs)
    except: pass
    step = int(_get(obs, "step", 0) or 0)
    if step == 718:
        try:
            return _v26_terminal_sweep(obs, _base_agent(obs, configuration), configuration)
        except Exception:
            pass
    return _base_agent(obs, configuration)
