...
def _acquire_lock(path, timeout=180):
    """Exclusive create of path.lock. Returns lock path or None."""
    lock = path + ".lock"
...
def atomic_write_json(path, obj):
    """Crash-safe JSON write. Windows-safe: never explode if another worker
    already wrote the same file (WinError 5 on os.replace)."""
    folder = os.path.dirname(path) or "."
    os.makedirs(folder, exist_ok=True)
    if os.path.exists(path) and _json_ok(path):
        return
    ...
    for attempt in range(12):
        ...
        try:
            ...
            time.sleep(0.08 * (attempt + 1))
    try:
        if not (os.path.exists(path) and _json_ok(path)):
...
def cleanup_tmp_files(*folders):
    """Delete leftover .tmp / .lock from a crashed Windows run."""
...
def step_towards(start, goal, style="greedy"):
    """One step from start toward goal. style: greedy (BFS), xfirst, yfirst."""
    sx, sy = start
    gx, gy = goal
    if start == goal:
        return "PASS"
...
def record_reference(seed, seat, mod, variant=None):
    ...
    def market_mod(market, obs, swap_state):
        ...
        if hm is not None:
            ...
            if n > len(hires):
                for _ in range(n - len(hires)):
                    if len(market) < 10:
                        market.append(["HIRE"])
        # v28-quads: extra land purchases at exact steps
        for ls in (variant.get("extra_buy_land") or []):
            if day * 24 + int(obs.get("hour", 0) or 0) == int(ls):
                if len(market) < 10:
        ...
        min_day = int(variant.get("crop_swap_min_day") or 0)
        for (from_crop, to_crop, count) in variant.get("crop_swaps", []):
            if swap_state.get((from_crop, to_crop, "seeds"), 0) >= count:
                continue
            # add to_crop seeds early (d<=1) or in the window around the
            # swap days (min_day-2 .. min_day+2) so the swapped plants have
            # seeds when they execute.  from_crop seed buys are left alone:
            # the leftover units are dead cash but tiny (a few $).
            if ((min_day <= 0 and day <= 1)
                    or (min_day > 0 and min_day - 2 <= day <= min_day + 2)):
                for o in market:
                    ...
                    if len(market) < 10:
                        ...
                        swap_state[(from_crop, to_crop, "seeds")] = swap_state.get((from_crop, to_crop, "seeds"), 0) + 1
        # extra production from swapped plants: bump to_crop SELLs by +1
        # each (spread across the tape's own sell steps) up to a total of
        # ~5 units per swapped plant.  Over-planned sells fail harmlessly.
        # Only bump after the swapped plants can have started yielding.
        bump_total = int(variant.get("crop_swap_sell_bump") or (5 * sum(
            c for (_, _, c) in variant.get("crop_swaps", []))))
        for (from_crop, to_crop, count) in variant.get("crop_swaps", []):
            key = (from_crop, to_crop, "sell_bump")
            if day < min_day + 13 or swap_state.get(key, 0) >= bump_total:
                continue
            for o in market:
                if swap_state.get(key, 0) >= bump_total:
                    break
                if o and o[0] == "SELL" and o[1] == to_crop and len(o) > 2:
                    o[2] = int(o[2]) + 1
...
def extract_plants(obs_history, seat):
    """tile -> {crop, planted_day, first_step, last_alive_day, lifecycles}"""
    plants = {}
        ...
        for y, row in enumerate(tiles):
            for x, t in enumerate(row):
                if isinstance(t, dict) and t.get("kind") == "PLANT":
                    key = (x, y)
                    crop = t.get("crop")
                    pday = int(t.get("planted_day", day))
                    if key not in plants:
                        plants[key] = {
                            ...
                            "last_alive_day": day,
                            "lifecycles": [{
                                "crop": crop, "planted_day": pday,
                                "start_day": day, "end_day": day,
                                "start_step": si, "end_step": si
                            }]
                        }
                    else:
                        plants[key]["last_alive_day"] = day
                        plants[key]["crop"] = crop
                        lcs = plants[key]["lifecycles"]
                        if not lcs or crop != lcs[-1]["crop"] or si - lcs[-1]["end_step"] > 1:
                            lcs.append({
                                "crop": crop, "planted_day": pday,
                                "start_day": day, "end_day": day,
                                "start_step": si, "end_step": si
...
def inject_feed_anchors(anchors, obs_history, seat, days, tile=(7, 4)):
    """Add a FEED anchor at `tile` on the given days, for the worker who
    used to tend that tile (the unit standing on it at the latest reference
    day).  FEED-only, placed at a late hour (h12) after the tender's
    corridor round, so the planner interleaves instead of colliding.
    The tender already PICKUPs wheat from the shed each morning; if their
    inventory is empty the FEED is a harmless no-op (validation decides).

    Fixes the d22 (7,4) cow escape: the v19-compiled route tends (7,4)
    daily through d19, then the late-wheat wave re-routes the tender.
    Alternate-day feeding keeps consecutive_unfed < 2 through d29.
    """
    ...
    for d in range(29, -1, -1):
        for h in range(24):
            ...
            if si >= len(obs_history):
                continue
            farm = (obs_history[si].get("farms") or [{}])[seat]
            pos = [farm.get("farmer"), *list(farm.get("hands") or [])]
            for wid, pp in enumerate(pos):
                if pp and list(pp[:2]) == list(tile):
                    feeder_wid = wid
                    break
            if feeder_wid is not None:
                break
        if feeder_wid is not None:
            break
    if feeder_wid is None:
        return [tuple(a) for a in anchors]
    for day in sorted(set(days)):
        if not (1 <= day <= 28):
            continue
        # skip days where the worker already tends that tile
        if any(a[0] // 24 == day and a[1] == feeder_wid and a[2][0] == "FEED"
               and a[3] == tuple(tile) for a in anchors):
            continue
        anchors.append([day * 24 + 12, feeder_wid, ("FEED", "WHEAT"), tuple(tile)])
...
def compute_water_days(plants, daily=True, switch_day=None):
    ...
    for tile, p in plants.items():
        days = set()
        for lc in p.get("lifecycles", [{"planted_day": p["planted_day"], "last_alive_day": p["last_alive_day"]}]):
            start = lc["planted_day"]
            end = lc.get("end_day", lc.get("last_alive_day", start))
            if daily:
                ...
                days.update(range(start, end + 1, 2))
        schedule[tile] = days
    return schedule
    ...
    for wid, alist in anchors.items():
        ...
        for gs, a, tile in alist:
            ...
            for s in range(start_walk, gs):
                ...
                op = step_towards(cur, tile, path_style)
                if op == "PASS":
                    break
                committed[wid][s] = [op]
        ...
        for wid in list(positions):
                ...
                if si >= e["end"]:
                    ...
                    if e["phase"] == "out":
                        if pos == e["tile"]:
                            ...
                            step_plan[wid] = [step_towards(pos, e["tile"], path_style)]
                        ...
                        if pos == e["home"]:
                            ...
                            step_plan[wid] = [step_towards(pos, e["home"], path_style)]
                            positions[wid] = _apply_move(pos, step_plan[wid][0])
                    continue
            # 2) committed step?
            ...
            if best is not None:
                if pos == best:
                    ...
                    step_plan[wid] = [step_towards(pos, best, path_style)]
                    positions[wid] = _apply_move(pos, step_plan[wid][0])
            else:
                step_plan[wid] = ["PASS"]
...
def _record_sig(variant):
    keys = ("crop_swaps", "hires_mult", "drop_animal_buys",
            "crop_swap_min_day", "crop_swap_sell_bump", "feed74_days",
            "feed74_tile", "extra_buy_land", "se_flood", "se_hands")
...
def _record_from_disk(path):
    ...
    return (d["tape"], plants, anchors, day_starts, hires,
...
def get_record(seed, seat, mod, variant=None):
    ...
    All surgical variants share one base record. On Windows, 8 workers
    racing os.replace on that file is a PermissionError — so we lock,
    and losers just wait and load.
    """
    variant = variant or {}
    ...
    if os.path.exists(path):
        try:
            ...
            try:
                ...
                pass
    lock = _acquire_lock(path, timeout=180)
    try:
        if os.path.exists(path):
            try:
                rec = _record_from_disk(path)
        ...
        fd = variant.get("feed74_days")
        if fd:
            anchors = inject_feed_anchors(anchors, obs_history, seat, fd,
                                          tile=tuple(variant.get("feed74_tile", (7, 4))))
...
def needs_recompile(variant):
    """True only if we must rebuild MOVE paths. Post-only surgery does not."""
    variant = variant or {}
    if variant.get("splice_days") is not None:
        return True
    if variant.get("recompile_all"):
        return True
    if variant.get("plant_fill") or variant.get("early_plant"):
        return True
    if variant.get("feed_repair"):
        return True
    if variant.get("feed74_days"):
        return True
    if variant.get("crop_swaps"):
        return True
    if variant.get("se_flood"):
        return True
    if variant.get("se_hands"):
        return True
    if variant.get("drop_animal_buys"):
        return True
    if (variant.get("hires_mult") or 1.0) != 1.0:
        return True
    return False
...
def compile_seat(seed, seat, mod, jitter=None, variant=None):
    ...
    # FAST PATH: do not rebuild MOVE paths unless a dimension actually needs
    # it. The 10,800-run proved full-route recompiles keep PASS money but
    # lose contested games. Surgical variants (idle/harvest/feed-buffer)
    # patch the proven tape instead.
    if not needs_recompile(variant):
        new_tape = apply_sell_mutations(tape, variant)
        new_tape = apply_extra_cow(new_tape, variant)
        new_tape = apply_surgery(new_tape, plants, visits, variant)
        return new_tape, {"seed": seed, "seat": seat, "ref_reward": ref_reward,
                          "plants": len(plants), "recompiled": False}
    ...
    # se_hands (v29): the user's dedicated-hands SE strawberry program.
    # Unlock SE at d11h1, strawberry seeds 12@d12h1 + 12@d16h1, workers
    # w2/w3 (daily-constant, crop-focused) lose only their CROP anchors on
    # d12-26 (FEED/CARE/PICKUP kept -> animals safe), and get the SE
    # program: plant 12 d12 + 12 d16, water alternating d13-25, harvest
    # d26 (all) + d28 (the d16 tiles' last unit), sells d26h1/d28h1.
    # Anchor placement: feasible-window (prev anchor hour + walk + op < 24).
    seh = variant.get("se_hands")
    if seh:
        import collections as _col
        workers = list(seh.get("workers") or [2, 3])
        n = int(seh.get("n") or 24)
        # market edits
        for step, orders in ((265, [["BUY_LAND"]]),
                             (289, [["BUY_SEED", "STRAWBERRY", 12]]),
                             (385, [["BUY_SEED", "STRAWBERRY", 12]]),
                             (625, [["SELL", "STRAWBERRY", 60]]),
                             (673, [["SELL", "STRAWBERRY", 24]])):
            e = tape[step]
            mkt = [list(o) for o in (e.get("market") or [])]
            for o in orders:
                if len(mkt) < 10:
                    mkt.append(o)
            e["market"] = mkt[:10]
        # delete only CROP anchors of the displaced workers on d12-26
        keep_ops = {"FEED", "CARE", "PICKUP", "COLLECT_FERTILIZER",
                    "PLACE", "BUILD_PASTURE", "BUILD_COOP", "DROP"}
        anchors[:] = [(s, w, a, t) for (s, w, a, t) in anchors
                      if not (12 <= s // 24 <= 26 and w in workers
                              and a[0] not in keep_ops)]
        # per (day, worker) anchor lists for window placement
        per = _col.defaultdict(list)
        for s, w, a, t in anchors:
            per[(s // 24, w)].append((s % 24, t))
        for k in per:
            per[k].sort()
        used = {(s, w) for s, w, a, t in anchors}
        avail = {}
        for (hstep, w), pos in hires.items():
            d = hstep // 24
            avail[(d, w)] = min(avail.get((d, w), 23), hstep % 24)
        for d in range(12, 29):
            avail[(d, 0)] = 0

        def place(d, w, op, tile):
            """feasible-window placement; returns step or None"""
            start = avail.get((d, w))
            if start is None:
                return None
            alist = per.get((d, w))
            if alist:
                prev_h, prev_tile = alist[-1]
                pos = prev_tile
            else:
                prev_h = start
                pos = None
                for (dw, p0) in day_starts.get(d, []):
                    if dw == w:
                        pos = tuple(p0)
                        break
                if pos is None:
                    hp = hires.get((d * 24 + start, w))
                    if hp is not None:
                        pos = tuple(hp)
                if pos is None:
                    return None
            dd = bfs_dist(pos, tile)
            if dd is None:
                return None
            gs = d * 24 + prev_h + dd + 1
            if gs >= (d + 1) * 24 - 1:
                return None
            while (gs, w) in used and gs < (d + 1) * 24 - 1:
                gs += 1
            used.add((gs, w))
            anchors.append((gs, w, (op,), tile))
            per[(d, w)].append((gs % 24, tile))
            per[(d, w)].sort()
            return gs

        se_tiles = [(x, y) for y in range(5, 10) for x in range(5, 10)][:n]
        planted_12 = []
        planted_16 = []
        for i, t in enumerate(se_tiles):
            w = workers[i % 2]
            d = 12 if i < 12 else 16
            gs = place(d, w, "PLANT", t)
            if gs is None:
                continue
            (planted_12 if d == 12 else planted_16).append((t, w))
            plants[t] = {"crop": "STRAWBERRY", "planted_day": d,
                         "first_step": gs, "last_alive_day": 29}
        # water alternating: tiles planted d12 -> water d13,15..25;
        # d16 tiles -> d17,19..25
        for t, w in planted_12 + planted_16:
            d = plants[t]["planted_day"]
            for wd in range(d + 1, 26, 2):
                place(wd, w, "WATER", t)
        # harvests: d26 all; d28 for the d16 tiles (last unit at d27->28)
        for t, w in planted_12 + planted_16:
            place(26, w, "HARVEST", t)
        for t, w in planted_16:
            place(28, w, "HARVEST", t)
        if variant.get("splice_days") is None:
            variant = dict(variant)
            variant["splice_days"] = list(range(12, 30))
        fill_set.update(se_tiles)

    # se_flood (v28-quads): unlock SE and plant N wheat tiles there d20-22,
    # harvest d26-27, with only d20-29 rebuilt (splice d0-19 = byte-identical
    # tape, feeding untouched).
    # Anchor placement rule (plan_day Phase A contract): each SE anchor is
    # placed AFTER the worker's last recorded anchor that day with a full
    # walk budget (gs >= prev_end + dist + 1), so the walk always completes
    # and the position tracker never poisons the FEED choreography.
    se_n = int(variant.get("se_flood") or 0)
    if se_n > 0:
        se_tiles = [(x, y) for y in range(5, 10) for x in range(5, 10)]
        se_tiles = se_tiles[:se_n]
        used = {(s, w) for s, w, a, t in anchors}
        # per (day, worker): sorted (step, tile) of recorded anchors
        per = {}
        for s, w, a, t in anchors:
            per.setdefault((s // 24, w), []).append((s, t))
        for k in per:
            per[k].sort()
        # worker availability per day: farmer (0) from h0; hands from their
        # hire step recorded in day_starts/hires
        avail = {}
        for d in (20, 21, 22, 26, 27):
            avail[(d, 0)] = 0
        for (hstep, w), pos in hires.items():
            d = hstep // 24
            if d in (20, 21, 22, 26, 27):
                avail[(d, w)] = min(avail.get((d, w), 23), hstep % 24)
        plant_pool = list(se_tiles)
        planted = []
        day_of = 20
        for i in range(se_n):
            if not plant_pool:
                break
            day = 20 + (i % 3)
            # best (worker, step, tile): smallest feasible step
            best = None
            for (d2, w), alist in per.items():
                if d2 != day:
                    continue
                start = avail.get((day, w))
                if start is None:
                    continue
                if alist:
                    prev_end = alist[-1][0] % 24   # within-day hour
                    prev_tile = alist[-1][1]
                    pos = prev_tile
                else:
                    # no anchors today: start from spawn position
                    prev_end = start
                    pos = None
                    for (dw, p0) in day_starts.get(day, []):
                        if dw == w:
                            pos = tuple(p0)
                            break
                    if pos is None:
                        hp = hires.get((day * 24 + start, w))
                        if hp is not None:
                            pos = tuple(hp)
                    if pos is None:
                        continue
                # nearest unclaimed SE tile
                bt = bd = None
                for t in plant_pool:
                    dd = bfs_dist(pos, t)
                ...
                if bt is None:
                    continue
                gs = day * 24 + prev_end + bd + 1
                if gs >= (day + 1) * 24 - 1:
                    continue
                if best is None or gs < best[0]:
                    best = (gs, w, bt, prev_end)
            if best is None:
                break
            gs, w, tile, _pe = best
            if (gs, w) in used:
                # slide later (rare: same-step collision)
                while (gs, w) in used and gs < (day + 1) * 24 - 1:
                    gs += 1
            used.add((gs, w))
            anchors.append((gs, w, ("PLANT", "WHEAT"), tile))
            plants[tile] = {"crop": "WHEAT", "planted_day": day,
                            "first_step": gs, "last_alive_day": 29}
            per.setdefault((day, w), []).append((gs, tile))
            per[(day, w)].sort()
            plant_pool.remove(tile)
            planted.append(tile)
        # harvests d26-27 with the same window rule
        harv_pool = list(planted)
        for i, tile in enumerate(harv_pool):
            day = 26 + (i // (max(1, (len(harv_pool) + 1) // 2)))
            best = None
            for (d2, w), alist in per.items():
                if d2 != day:
                    continue
            ...
            if (gs, w) in used:
                while (gs, w) in used and gs < (day + 1) * 24 - 1:
                    gs += 1
            used.add((gs, w))
            anchors.append((gs, w, ("HARVEST",), tile))
            per.setdefault((day, w), []).append((gs, tile))
            per[(day, w)].sort()
        if variant.get("splice_days") is None:
            variant = dict(variant)
            # splice_days = days to REBUILD (all others keep the tape's
            # labor verbatim). We rebuild only d20-29 where the SE program
            # lives; d0-19 stays byte-identical (feeding untouched).
            variant["splice_days"] = list(range(20, 30))
        fill_set.update(se_tiles)
    ...
    # splice: keep the proven tape's labor on days we did NOT ask to rebuild.
    # Workers respawn at the shed every morning, so a single odd day can be
    # swapped without desyncing the next day.
    splice = variant.get("splice_days")
    if splice is not None:
        allowed = set(int(d) for d in splice)
        report["spliced_days"] = sorted(allowed)
        for si, entry in enumerate(new_tape):
            if (si // TURNS_PER_DAY) in allowed:
                continue
            entry["farmer"] = list(tape[si].get("farmer") or ["PASS"])
            entry["hands"] = [list(h) for h in (tape[si].get("hands") or [])]
            keep_compiled_mkt = (
                si // TURNS_PER_DAY == 0
                and (variant.get("plant_fill") or variant.get("early_plant")
                     or int(variant.get("feed_repair") or 0) == 1)
            )
            if not keep_compiled_mkt:
                entry["market"] = [list(o) for o in (tape[si].get("market") or [])]

    # ----- post-compile market + surgical patches -----------------------
...
def _set_unit_action(entry, wid, action):
    """wid 0 = farmer, wid>=1 = hands[wid-1]."""
    if wid == 0:
        entry["farmer"] = list(action)
        return
    hands = [list(h) for h in (entry.get("hands") or [])]
    idx = wid - 1
    while len(hands) <= idx:
        hands.append(["PASS"])
    hands[idx] = list(action)
    entry["hands"] = hands


def _unit_action(entry, wid):
    if wid == 0:
        return entry.get("farmer") or ["PASS"]
    hands = entry.get("hands") or []
    if wid - 1 < len(hands):
        return hands[wid - 1]
    return None
...
def leftover_tiles(plants):
    """One-time crops still on the field on the last day."""
    out = []
    for tile, p in plants.items():
        if int(p.get("last_alive_day", 0) or 0) < 29:
            continue
        if p.get("crop") in ("WHEAT", "CARROT", "MELON"):
            out.append(tile)
    return out


def apply_feed_buffer(tape, extra):
    """Bump the existing day-0 BUY_PRODUCT WHEAT by `extra` (cheap cow buffer)."""
    if extra <= 0:
        return tape
    ...
    for e in out[:4]:
        ...
        for o in mkt:
            if o and o[0] == "BUY_PRODUCT" and o[1] == "WHEAT":
                o[2] = int(o[2]) + int(extra)
                return out
    return out


def apply_feed_stagger(tape):
    """Days 10-20: set each worker's first PICKUP WHEAT qty to (feeds that
    day + 1) and zero extra pickups. Stops the morning shed-drain that
    starves the (7,4) cow without the $40 day-0 wheat bomb."""
    out = copy.deepcopy(tape)
    for day in range(10, 21):
        gs = day * TURNS_PER_DAY
        ge = min(gs + TURNS_PER_DAY, len(out))
        feeds = collections.Counter()
        pickups = []
        for s in range(gs, ge):
            e = out[s]
            for wid in range(0, 1 + len(e.get("hands") or [])):
                a = _unit_action(e, wid)
                if not a:
                    continue
                if a[0] == "FEED":
                    feeds[wid] += 1
                elif a[0] == "PICKUP" and len(a) >= 3 and a[1] == "WHEAT":
                    pickups.append((s, wid, int(a[2])))
        if not pickups:
            continue
        seen = set()
        for s, wid, _qty in pickups:
            if wid in seen:
                _set_unit_action(out[s], wid, ["PICKUP", "WHEAT", 0])
                continue
            seen.add(wid)
            need = max(feeds.get(wid, 0) + 1, 1)
            _set_unit_action(out[s], wid, ["PICKUP", "WHEAT", need])
    return out


def apply_idle_water(tape, plants, visits, mode=1, skip_days=None):
    """PASS -> WATER when a worker is already standing on a live plant.
    mode 1 = on-tile only (timing-safe).
    mode 2 = also take a 1-step side trip onto an adjacent live plant
             if the next step is PASS (go, water, the PASS covers the stay).
    Never touches non-PASS actions. Skip days that were MOVE-recompiled
    (those days already fill free labor with water)."""
    if not mode:
        return tape
    skip_days = set(skip_days or [])
    pos_at = {}
    for tile, vs in visits.items():
        for s, w in vs:
            pos_at[(s, w)] = tile
    alive = {tile: (int(p["planted_day"]), int(p.get("last_alive_day", 29)))
             for tile, p in plants.items()}
    out = copy.deepcopy(tape)
    for s, e in enumerate(out):
        ...
        for wid in range(0, 1 + n_hands):
            ...
            if not a or a[0] != "PASS":
                continue
            tile = pos_at.get((s, wid))
            if tile and tile in alive:
                pd, ld = alive[tile]
                if pd <= day <= ld:
                    _set_unit_action(e, wid, ["WATER"])
                    continue
            if mode < 2 or tile is None:
                continue
            # adjacent live plant, only if next action is also PASS
            nxt = out[s + 1] if s + 1 < len(out) else None
            if nxt is None:
                continue
            na = _unit_action(nxt, wid)
            if not na or na[0] != "PASS":
                continue
            tx, ty = tile
            best = None
            for dx, dy, op in ((1, 0, "EAST"), (-1, 0, "WEST"),
                               (0, 1, "SOUTH"), (0, -1, "NORTH")):
                nb = (tx + dx, ty + dy)
                if nb in alive:
                    pd, ld = alive[nb]
                    if pd <= day <= ld:
                        ...
                        break
            if best:
                _set_unit_action(e, wid, [best])
    return out


def apply_leftover_harvest(tape, plants, visits, mode=1, path_style="greedy"):
    """Harvest the day-26 wheat that is still on the field at game end.
    mode 1: on-tile only — day 28 PASS on leftover -> WATER (yield boost);
            day 29 PASS on leftover -> HARVEST if age >= 2 else WATER.
    mode 2: also walk toward leftovers on day-28 last 6 steps (mostly PASS).
    mode 3: mode 2 + day-29 on-tile HARVEST.
    Never replaces WATER in the wheat yield-boost window with HARVEST.
    """
    if not mode:
        return tape
    leftovers = leftover_tiles(plants)
    if not leftovers:
        return tape
    ...
    for tile, vs in visits.items():
        for s, w in vs:
            pos_at[(s, w)] = tile
    out = copy.deepcopy(tape)
    # on-tile patches (modes 1 and 3, and the on-tile half of 2)
    do_ontile = mode in (1, 2, 3)
    do_walk = mode in (2, 3)
    if do_ontile:
        for s, e in enumerate(out):
            day = s // TURNS_PER_DAY
            ...
            for wid in range(0, 1 + n_hands):
                ...
                if not a or a[0] != "PASS":
                    continue
                tile = pos_at.get((s, wid))
                if tile not in left_set:
                    continue
                # yield-boost WATER on day 28; harvest on day 29
                if day == 28:
                    _set_unit_action(e, wid, ["WATER"])
                else:
                    _set_unit_action(e, wid, ["HARVEST"])
    if do_walk:
        # day 28 last 6 steps are almost all PASS — walk toward leftovers
        for s in range(690, min(696, len(out))):
            e = out[s]
            n_hands = len(e.get("hands") or [])
            for wid in range(0, 1 + n_hands):
                a = _unit_action(e, wid)
                ...
                if best is not None and best_d > 0:
                    op = step_towards(tile, best, path_style)
                    if op != "PASS":
                        _set_unit_action(e, wid, [op])
    return out


def apply_surgery(tape, plants, visits, variant):
    """Post-compile surgical patches that do not rebuild the route."""
    out = tape
    extra = int(variant.get("feed_buffer") or 0)
    if extra:
        out = apply_feed_buffer(out, extra)
    if variant.get("feed_stagger"):
        out = apply_feed_stagger(out)
    idle = int(variant.get("idle_water") or 0)
    splice = variant.get("splice_days") or []
    if idle:
        out = apply_idle_water(out, plants, visits, mode=idle, skip_days=splice)
    hv = int(variant.get("leftover_harvest") or 0)
    if hv:
        out = apply_leftover_harvest(out, plants, visits, mode=hv,
                                     path_style=variant.get("path_style", "greedy"))
    return out
...
def validate_tape(tape, seed, seat, mod):
            ...
            for row in tiles:
                for t in row:
                    ...
                    if t.get("kind") == "WEED":
                        ...
                        crops += 1
                        leftover_end += 1
                        leftover_units += int(t.get("yield_units", 0) or 0)
                    if t.get("animal"):
                        animals_end += 1
        else:
            for row in tiles:
    ...
    return {
        ...
        "leftover_plants": leftover_end,
        "leftover_units": leftover_units,
        "animals_alive": animals_end,
    }
